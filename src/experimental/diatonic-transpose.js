...
export function diatonicNoteEdits(region,values,session){
 ...
 if(v.useProjectKey){if(v.root!==undefined||v.scale!==undefined||v.custom!==undefined)throw Error('Choose project key or an explicit scale, not both.');key=projectKeyAtTime(session);}
 ...
 return selectedMidiNotes(region,v).map(note=>{
  const current=v.useProjectKey?key((region.start??0)+note.start):key,intervals=scaleIntervals(current.scale,current.custom),size=intervals.length;
  const relative=note.pitch-current.root;let octave=Math.floor(relative/12);const within=((relative%12)+12)%12,rawIndex=intervals.findLastIndex(p=>p<=within),index=rawIndex<0?size-1:rawIndex;if(rawIndex<0)octave--;const alteration=relative-(octave*12+intervals[index]);
  if(alteration&&v.accidentals==='reject')throw Error('The selection contains notes outside the chosen scale.');
  const degree=octave*size+index+v.steps,targetOctave=Math.floor(degree/size),targetIndex=((degree%size)+size)%size,pitch=current.root+12*targetOctave+intervals[targetIndex]+alteration;
