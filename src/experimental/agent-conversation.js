...
function entities(session){
 ...
 for(const p of session.keyChanges||[])add('key change',p);
 for(const p of session.scalePresets||[])add('scale preset',p);for(const p of session.tempoChanges||[])add('tempo change',p);for(const p of session.meterChanges||[])add('time signature',p);for(const section of session.sections||[])add('section',section);
