...
function entities(session){
 ...
 add('session',session);effects(session.masterEffects,session.id);for(const p of session.masterAutomation||[])add('master automation',p);for(const m of session.markers||[])add('marker',m);
