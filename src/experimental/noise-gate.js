...
export function connectNoiseGate(context,input,effect,nodes,{position,base,register,registerMeter,registerSidechain}){
 ...
 const node=new AudioWorkletNode(context,'cuestamp-noise-gate',{numberOfInputs:2,numberOfOutputs:1,outputChannelCount:[2],channelCount:2,channelCountMode:'explicit',channelInterpretation:'speakers',processorOptions:{mode:effect.mode,filterEnabled:effect.filterEnabled||Boolean(effect.sidechainTrackId),meter:Boolean(registerMeter)}});
