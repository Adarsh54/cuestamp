...
export function connectNoiseGate(context,input,effect,nodes,{position,base,register,registerMeter}){
 if(!context[stateKey]?.ready)throw Error('Prepare noise gate processing before starting audio.');
 const node=new AudioWorkletNode(context,'cuestamp-noise-gate',{numberOfInputs:2,numberOfOutputs:1,outputChannelCount:[2],channelCount:2,channelCountMode:'explicit',channelInterpretation:'speakers',processorOptions:{filterEnabled:effect.filterEnabled,meter:Boolean(registerMeter)}});
