...
export function connectTremolo(context,input,effect,nodes,{position=0,base=context.currentTime,tempo=120,tempoChanges=[],register}={}){
 ...
 for(let channel=0;channel<2;channel++){
  ...
  if(sync){osc.frequency.setValueAtTime(sync.rate,base);for(const point of sync.changes)osc.frequency.setValueAtTime(point.rate,base+point.time-position);}else schedule(osc.frequency,rateEffect,'rate',position,base);
