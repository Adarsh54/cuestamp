...
export const effectSchema=z.discriminatedUnion('kind',[
 z.object({...base,kind:z.literal('gate'),filterEnabled:z.boolean().default(false),lowCutHz:z.number().finite().min(20).max(20000).default(80),highCutHz:z.number().finite().min(20).max(20000).default(12000),threshold:z.number().finite().min(-96).max(0).default(-40),reductionDb:z.number().finite().min(-96).max(0).default(-60),hysteresis:z.number().finite().min(0).max(24).default(6),attack:z.number().finite().min(0).max(1).default(.003),hold:z.number().finite().min(0).max(2).default(.05),release:z.number().finite().min(.001).max(3).default(.1)}).strict(),
...
export function connectEffects(context,input,effects,nodes,{position=0,base=context.currentTime,tempo=120,tempoChanges=[],register,registerMeter}={}){const schedule=(param,effect,key,position,base,transform=v=>v)=>{register?.(effect,key,param,transform,false);scheduleEffectParameter(param,effect,key,position,base,transform);};let output=input;for(const effect of effects||[]){if(!effect.enabled)continue;
 if(effect.kind==='gate'){output=connectNoiseGate(context,output,effect,nodes,{position,base,register,registerMeter});}
