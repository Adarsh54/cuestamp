...
export function bindTranspose(root,{region,settings,session,execute,guard}){
 ...
 const values=()=>({...(s.mode==='chromatic'?{semitones:s.amount}:{mode:'diatonic',steps:s.amount,root:s.root,scale:s.scale,...(s.scale==='custom'?{custom:s.custom}:{}),accidentals:s.accidentals??'reject'}),...(s.scope==='selected'?{noteIds:(settings.selectedIds||[]).join(',')}:{})});
 const update=()=>{
 ...
 const load=key=>{f.mode.value='diatonic';f.root.value=key.root;f.scale.value=key.scale;s.custom=key.custom;update();};
 ...
 form.querySelectorAll('[data-transpose-octave]').forEach(button=>button.onclick=guard(()=>{f.amount.value=Number(button.dataset.transposeOctave)*(s.mode==='chromatic'?12:scaleIntervals(s.scale,s.scale==='custom'?s.custom:undefined).length);update();}));
