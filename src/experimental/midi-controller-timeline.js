...
export function compileMidiControllers(events,range=2){
 ...
 return {events:ordered,levels,pitch,pitchIntegral:compilePitchTimeIntegral(pitch),sustainedEnd(note,duration){const end=note.start+note.duration,index=upperMidiTime(pedals,end)-1;if(index<0||pedals[index].value<64)return end;return Math.min(duration,releases[upperMidiTime(releases,end)]?.time??duration);}};
}
