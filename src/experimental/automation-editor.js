...
export function automationView(track,esc,{gainOnly=false,inheritedOff=false,parameters=gainOnly?{gainDb:defaultParameters.gainDb}:defaultParameters}={}){
 return `<section class="daw-automation"><h4>Automation · ${esc(track.name)}</h4><label>Automation mode<select data-auto-mode><option value="read" ${track.automationMode!=='off'?'selected':''}>Read saved curves</option><option value="off" ${track.automationMode==='off'?'selected':''}>Off · use static settings</option></select></label><p class="muted">${inheritedOff?'The parent channel has automation Off. These curves remain saved but are not playing.':track.automationMode==='off'?'Automation is Off. Saved points remain editable; playback uses static settings.':'Read plays the saved curves. This control does not record fader movements.'}</p><label>Parameter<select data-auto-parameter>${Object.entries(parameters).map(([key,spec])=>`<option value="${key}">${esc(spec.label)}</option>`).join('')}</select></label><label>Selected curve<select data-auto-curve-mode><option value="on">On · read saved points</option><option value="off">Off · use static value</option></select></label><output data-auto-curve-status></output><p class="muted">Click to add a point; drag a point to move it. Focus a point and use arrow keys to adjust it, or Delete to remove it. In Read mode, curves override the static control. Each point’s shape controls the transition to the next point; Hold keeps its value until the next point.</p><svg data-auto-graph data-parent-automation-off="${inheritedOff}" data-automation-off="${inheritedOff||track.automationMode==='off'}" viewBox="0 0 600 160" preserveAspectRatio="none" aria-label="Automation curve"></svg><form data-auto-form="${track.id}"><label>Time · seconds<input type="number" name="time" min="0" max="86400" step=".01" value="0" required></label><label>Value<input type="number" name="value" step=".1" value="0" required></label>${shapeSelect()}<button type="submit">Add point</button><button type="button" data-auto-clear="${track.id}">Clear curve</button></form><div data-auto-points></div><div data-auto-range-host></div></section>`;
}
export function bindAutomation(root,{track,session,rangeTarget=track.id,rangeBusId,execute,guard,duration,mode=value=>({op:'track.set',target:track.id,values:{automationMode:value}}),automationParameter='gainDb',onAutomationParameter=()=>{},
 ...
 if(!root)return;
 root.querySelector('[data-auto-mode]')?.addEventListener('change',guard(e=>execute([mode(e.target.value)],'Changed automation mode')));
 const parameter=root.querySelector('[data-auto-parameter]');if(!parameter)return;
 root.querySelector('[data-auto-curve-mode]').onchange=guard(e=>execute([{op:'automation.parameter',target:rangeTarget,values:{parameter:parameter.value,enabled:e.target.value==='on',...(rangeBusId?{busId:rangeBusId}:{})}}],'Changed curve playback'));
