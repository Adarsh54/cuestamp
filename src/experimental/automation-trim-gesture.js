...
export function automationTrimGesturePlan(session,target,values){
 const v=options.parse(values),offsets=z.array(sample).min(2).max(2000).parse(JSON.parse(v.samples)),track=session.tracks.find(t=>t.id===target);
 if(target!==session.id&&(!track||track.kind==='video'))throw Error('Choose a mixer track or Master for trim recording.');
 ...
 const points=target===session.id?session.masterAutomation:track.automation,fallback=target===session.id?(v.parameter==='gainDb'?session.masterDb:session.masterPan):track[v.parameter];
 ...
 return automationGesturePlan(session,target,{parameter:v.parameter,samples:JSON.stringify(samples),returnSeconds:0});
}
