...
export function samplerOffset(note,root,events,at,settings={},compiledPoints){
 const rate=samplerPlaybackRate(note.pitch,root,settings),points=compiledPoints??pitchBendTimeline(events,settings.pitchBendRange);let previous=note.start,cents=points[upperMidiTime(points,previous)-1]?.cents??0,offset=0;
...
export function scheduleSampler(context,destination,buffer,root,note,events,relative,when,end,nodes,settings={},compiledPoints){
 ...
 const loop=samplerLoop(buffer,settings),offset=loopedSampleOffset(samplerOffset(note,root,events,at,settings,compiledPoints),loop);if(offset>=buffer.duration)return;
