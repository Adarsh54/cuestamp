...
export function samplerOffset(note,root,events,at,settings={},compiledPoints,compiledIntegral){
 const rate=samplerPlaybackRate(note.pitch,root,settings);if(compiledIntegral)return rate*compiledIntegral(note.start,at);const points=compiledPoints??pitchBendTimeline(events,settings.pitchBendRange);let previous=note.start,cents=points[upperMidiTime(points,previous)-1]?.cents??0,offset=0;
