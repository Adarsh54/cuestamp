...
export function copyChannelSettings(destination,source,options){
 ...
 const keys={effects:['effects'],mix:['gainDb','pan'],automation:['automation','automationMode','automationMuted'],routing:['output','sends'],instrument:['midiSwitches','sampleArticulations',...Object.keys(copy).filter(key=>key==='instrument'||key==='pitchBendRange'||(key.startsWith('sample')&&key!=='sampleArticulations'))]};
