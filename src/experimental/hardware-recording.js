...
export const hardwareRecordingSchema=z.object({midiFingerprint:z.string().regex(/^[a-f0-9]{64}$/).optional(),kind:z.literal('midi-hardware'),deviceName:z.string().max(200),scope:z.enum(['track','arrangement']),revision:z.number().int().nonnegative(),capturedAt:z.string().datetime(),start:z.number().finite().min(0).max(86400),end:z.number().finite().max(86400),tracks:z.array(z.object({id:sourceId,name:z.string().max(200)}).strict()).min(1).max(128)}).strict().refine(v=>v.end>v.start,'Capture end must follow its start.');
