...
export const transportStateSchema=z.object({sessionId:z.string().min(1).max(100),revision:z.number().int().nonnegative(),epoch:z.number().int().nonnegative(),position,playing:z.boolean(),scene:sceneObservationSchema.optional(),mode:z.enum(['stopped','arrangement','cycle','audioRange','regionSelection','scene','scenePerformance','comp','warp','preview']).optional()}).strict();
...
export function transportSummary(operation,state){
 ...
 const labels={scenePerformance:'Recorded take preview',scene:'Scene audition',cycle:'Cycle playback',audioRange:'Audio range audition',regionSelection:'Selected clip audition',comp:'Comp audition',warp:'Warp preview',preview:'Preview playback'};
