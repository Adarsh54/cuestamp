...
export function startSceneTransport(context,preview,buffers,{schedule=scheduleSession}={}){
 ...
 let current,pending=null,stopped=false;
 ...
 current=prepare(preview,base);current.output.gain.setValueAtTime(1,base);
 const transport={base,position:0,preview:true,scenePreview:preview.sceneId,
  ...
  queue(plan,quantization){
   ...
   current.output.gain.setValueAtTime(0,when);next.output.gain.setValueAtTime(1,when);pending=next;
   return {position,sceneId:plan.sceneId};
  },
  stop(){if(stopped)return;stopped=true;current.stop();pending?.stop();pending=null;}
 };
 return transport;
}
