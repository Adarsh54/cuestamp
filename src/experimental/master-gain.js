...
export function offsetMasterGain(session,deltaDb){
 ...
 return {masterDb:shift(session.masterDb),masterAutomation:(session.masterAutomation||[]).map(p=>session.masterAutomationMode!=='off'&&!(session.masterAutomationMuted||[]).includes('gainDb')&&p.parameter==='gainDb'?{...p,value:shift(p.value)}:{...p})};
}
