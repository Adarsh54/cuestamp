...
export function createTouchRecording({getState,commit,getMode=()=> 'touch'}){
 ...
 function finish(gestures=[...active.values()]){
  ...
  try{
   ...
   const commands=valid.map(g=>({op:g.mode.startsWith('trim')?'automation.trimRecord':'automation.record',target:g.target,values:{parameter:g.parameter,samples:JSON.stringify(g.capture.finish(state.position)),returnSeconds:.1}}));
   ...
   for(const g of gestures){
    if(valid.includes(g)){
     ...
     g.playback.automation.replace(g.target,g.parameter,owner.automation,owner[g.parameter]);if(g.mode.startsWith('trim'))g.playback.automation.resume(g.target,g.parameter);else g.playback.automation.release(g.target,g.parameter,.1);
 ...
 const api={
  ...
  beforePaint(){
   ...
   if(['latch','write','trimLatch'].includes(getMode())){try{for(const g of active.values())release(g.target,g.parameter);}catch{cancel();}}else cancel();
  },
  beginWrite(target){
  ...
  input(target,parameter,value){
   ...
   try{
    ...
    state.playback.automation[mode.startsWith('trim')?'trim':'set'](target,parameter,value);
   }catch(error){cancel();throw error;}
  },
 };
 return api;
