...
export function readMidi(buffer){
 ...
 const tempos=[{tick:0,microseconds:500000}],tracks=[],markers=[],signatures=[],keys=[];
 ...
 const keySignatures=[...new Map(keys.map(point=>[point.tick,point])).values()].sort((a,b)=>a.tick-b.tick).map(({tick,...point})=>({...point,beat:tick/ppq,time:seconds(tick)}));
...
export function writeMidi(session,{includeMuted=false}={}){
 ...
 for(const point of compileKeyMap(session).points){if(map.timeAtBeat(point.beat)>86400)throw Error('Key signatures must fit within the 24-hour timeline.');conductorEvents.push({tick:Math.round(point.beat*ppq),priority:-1,data:[255,89,2,point.sharps&255,point.mode==='minor'?1:0]});}
