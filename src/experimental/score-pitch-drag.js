...
export function scorePitchStep(pitch,steps,key){
 if(!Number.isInteger(pitch)||pitch<0||pitch>127||!Number.isInteger(steps))throw Error('Use a MIDI pitch and whole staff steps.');
 ...
 if(next<0||next>127)throw Error('That staff position is outside MIDI pitch 0–127.');return next;
}
export function bindScorePitchDrag(element,{session,reference,zoom,execute,guard,onSelect}){
 ...
 const key=compileKeyMap(session).keyAtBeat(compileTempoMap(session).beatAtTime(region.start+note.start));
 ...
 element.onpointermove=e=>{if(!gesture||gesture.id!==e.pointerId)return;e.preventDefault();e.stopPropagation();const steps=Math.round((gesture.y-e.clientY)/(5*zoom));gesture.steps=steps;
  try{gesture.pitch=scorePitchStep(note.pitch,steps,key);element.dataset.scoreDragPitch=String(gesture.pitch);element.style.translate=`0 ${-steps*5}px`;element.setAttribute('aria-label',`Move to MIDI note ${gesture.pitch}`);}catch{gesture.pitch=null;element.setAttribute('aria-label','Outside MIDI pitch range');}
 };
