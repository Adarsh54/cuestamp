import {snapArrangementTime} from './arrangement-snap.js';
export function scissorsCut(session,clickedId,selection,time,{snap=true,grid='sixteenth'}={}){
 if(!Number.isFinite(time)||time<0||time>86400)throw Error('Choose a cut position within the timeline.');
 const at=Math.min(86400,snapArrangementTime(session,time,grid,!snap));
...
export function bindScissors(root,{session,zoom,selection,split,select,guard,blocked,grid='sixteenth'}){
 ...
 root.querySelectorAll('[data-region]').forEach(el=>{
  ...
  const update=e=>{
   ...
   try{plan=scissorsCut(session,el.dataset.region,selection,time,{snap:!e.shiftKey,grid});}catch(error){plan={time,count:0,regionIds:[],error:error.message};}
