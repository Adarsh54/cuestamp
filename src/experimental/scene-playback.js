...
export function sceneAudition(session,values){
 ...
 const audible=new Set(renderingSources(session).flatMap(t=>t.regions.map(r=>r.id)));
 const cells=new Map(scene.cells.filter(c=>audible.has(c.regionId)).map(c=>[c.regionId,c]));
 if(!cells.size)throw Error('This scene has no audible or sidechain trigger clips. Assign clips and check mute and solo settings.');
