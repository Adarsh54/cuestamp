export function shortcutAction(event){
 ...
 if(mod){if(key==='z')return event.shiftKey?'redo':'undo';if(key==='y'&&event.ctrlKey&&!event.metaKey&&!event.shiftKey)return 'redo';return null;}
 if(event.shiftKey)return null;
 return {' ':'play',home:'stop',s:'split',d:'duplicate',delete:'delete',backspace:'delete'}[key]||null;
}
export function shortcutsView(enabled=true){return `<details class="daw-shortcuts"><summary>Keyboard shortcuts</summary><label class="daw-shortcuts-toggle"><input type="checkbox" data-shortcuts-enabled ${enabled?'checked':''}>Enable workspace shortcuts</label><dl><dt>Space</dt><dd>Play / pause</dd><dt>Home</dt><dd>Stop and return to start</dd><dt>⌘ / Ctrl Z</dt><dd>Undo</dd><dt>⌘ / Ctrl Shift Z</dt><dd>Redo (also Ctrl Y)</dd><dt>S</dt><dd>Split selected region at playhead</dd><dt>D</dt><dd>Duplicate selected regions or piano-roll notes</dd><dt>Delete / Backspace</dt><dd>Delete selected regions or piano-roll notes</dd></dl><p class="muted">Click the arrangement or piano roll to choose the editing area. S applies in the arrangement or region inspector. Text fields and focused buttons keep their normal keys. Shortcuts pause during recording, loading and dialogs.</p></details>`;}
export function bindWorkspaceShortcuts(root,{buttonFor,blocked}){
 ...
 const keydown=event=>{
  ...
  if(control&&(action==='play'||(['split','duplicate','delete'].includes(action)&&!control.matches('[data-region],[data-track],[data-note]'))))return;
  if(['split','duplicate','delete'].includes(action)&&(!area||(action==='split'&&area!=='arrangement')))return;
