...
export function bindWorkspaceShortcuts(root,{buttonFor,blocked}){
 ...
 const keydown=event=>{
  ...
  if(['copy','cut','paste'].includes(action)&&path.some(node=>node.matches?.('[data-score-preview]')))return;
