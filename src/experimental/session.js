import {savedMelodySchema,setSavedMelody,pruneSavedMelodies} from './melody-storage.js';
import {retunedRegionTrack} from './melody-retune-plan.js';
import {samplerPresetsSchema,editSamplerPreset} from './sampler-presets.js';
import {samplerGroupsSchema,editSamplerGroup,validateSamplerGroups} from './sampler-groups.js';
import {samplerZonesSchema,editSamplerZone} from './sampler-zones.js';
import {samplerArticulationsSchema,setSamplerArticulation,removeSamplerArticulation} from './sampler-articulations.js';
import {assignNoteArticulation} from './note-articulations.js';
import {midiSwitchSchema,midiSwitchesSchema,saveMidiSwitch,removeMidiSwitch,insertMidiSwitch} from './midi-switches.js';
import {importedMarkers} from './marker-import.js';
import {markersFromRegions} from './region-markers.js';
import {markerCyclePlan} from './marker-cycle.js';
import {hardwareRecordingSchema} from './hardware-recording.js';
import {commitScenePerformance} from './scene-performance.js';
import {placeScene} from './scene-arrangement.js';
import {sceneSchema,editScene,validateScenes,pruneSceneReferences} from './scenes.js';
import {renamedRegions} from './region-rename.js';
import {batchRegionFades} from './region-batch-fades.js';
import {sequencedRegions} from './region-sequence.js';
import {effectPresetSchema,editEffectPreset} from './effect-presets.js';
import {createCycleTakes} from './cycle-takes.js';
import {takeGroupSchema,editTakeGroup} from './audio-takes.js';
import {warpedRegionTrack} from './audio-warp-region.js';
import {quantizeAudioRegions} from './region-quantize.js';
import {attackMarkersSchema,setAttackMarkers} from './attack-markers.js';
import {splitAudioAtTimes} from './region-split-times.js';
import {splitAudioRange} from './region-split-range.js';
import {silenceRegionTrack} from './audio-silence-regions.js';
import {stretchedRegionTrack,pitchedRegionTrack} from './audio-stretch-region.js';
import {fitTempoPlan} from './tempo-fit.js';
import {midiTuningPlan} from './midi-tuning.js';
import {midiBendRangePlan} from './midi-bend-range.js';
import {constantTempoPlan} from './tempo-constant.js';
import {scaleTempoPlan} from './tempo-scale.js';
import {tempoRampPlan} from './tempo-ramp.js';
import {thinControllerPlan} from './controller-thin.js';
import {allocateMidiChannels} from './midi-channel-allocation.js';
import {remapMidiChannel} from './midi-channel-remap.js';
import {pitchBendRangeSchema} from './pitch-bend.js';
import {applyMidiImportKey} from './midi-import-key.js';
import {applyAutomationTrimGesture} from './automation-trim-gesture.js';
import {applyAutomationTrim} from './automation-trim.js';
import {applyAutomationGesture} from './automation-gesture.js';
import {scalePresetSchema} from './scale-presets.js';
...
import {alignedRegions,setSelectedTempoFollow,movedRegions,duplicatedRegions,regionIdsForDeletion} from './region-selection.js';
...
const note=z.object({articulation:midiSwitchSchema.optional(),id:ident,mute:z.boolean().default(false),channel:z.number().int().min(0).max(15).default(0),pitch:z.number().int().min(0).max(127),start:time,duration:z.number().positive().max(3600),velocity:z.number().min(0).max(1)});
const region=z.object({melodyDraft:savedMelodySchema.optional(),hardwareRecording:hardwareRecordingSchema.optional(),takeGroup:takeGroupSchema.optional(),attackMarkers:attackMarkersSchema.optional(),id:ident,tempoFollow:z.boolean().optional(),name:z.string().max(200),assetId:ident.nullable(),mute:z.boolean().default(false),start:time,offset:time,duration:z.number().positive().max(86400),gainDb:db,fadeIn:time,fadeOut:time,fadeInShape:z.enum(['linear','equalPower']).default('linear'),fadeOutShape:z.enum(['linear','equalPower']).default('linear'),reverse:z.boolean(),notes:z.array(note).max(20000),events:z.array(midiEventSchema).max(20000).default([])});
const track=z.object({scoreInstrument:z.enum(['concert','bb','eb','f']).optional(),scoreClef:z.enum(['treble','bass','alto','tenor']).optional(),sampleGroups:samplerGroupsSchema.optional(),sampleZones:samplerZonesSchema.optional(),inputArticulation:midiSwitchSchema.nullable().optional(),sampleArticulations:samplerArticulationsSchema.optional(),midiSwitches:midiSwitchesSchema.optional(),id:ident,color:z.string().regex(/^#[0-9a-fA-F]{6}$/).nullable().optional(),name:z.string().max(200),kind:z.enum(['audio','midi','video','bus']),collapsed:z.boolean().optional(),protected:z.boolean().optional(),automationMode:automationModeSchema.optional(),automationMuted:automationMutedSchema(['gainDb','pan']),gainDb:db,pan:z.number().min(-1).max(1),mute:z.boolean(),solo:z.boolean(),pitchBendRange:pitchBendRangeSchema,instrument:z.enum(['sine','triangle','square','sawtooth','drumKit','sampler']),sampleAssetId:ident.nullable().default(null),sampleRoot:z.number().int().min(0).max(127).default(60),sampleAttack:z.number().min(0).max(10).default(.005),sampleDecay:z.number().min(0).max(10).default(0),sampleSustain:z.number().min(0).max(1).default(1),sampleRelease:z.number().min(0).max(30).default(.02),...samplerTuningSchema.shape,...samplerFilterSchema.shape,sampleLoop:z.boolean().default(false),sampleLoopStart:time.default(0),sampleLoopEnd:time.nullable().default(null),regions:z.array(region).max(1000),compAlternatives:z.array(compAlternativeSchema).max(32).default([]),output:ident.nullable().default(null),sends:z.array(z.object({busId:ident,gainDb:db,automationMode:automationModeSchema.optional(),automationMuted:automationMutedSchema(['gainDb']),tap:z.enum(['preFader','postFader','postPan']).default('postPan'),automation:z.array(automationSchema.refine(p=>p.parameter==='gainDb','Send automation supports gain only.')).max(2000).default([])})).max(16).default([]),effects:z.array(effectSchema).max(16).default([]),automation:z.array(automationSchema).max(2000).default([])}).superRefine((t,ctx)=>{validateSamplerGroups(t,ctx);if(t.kind!=='audio'&&t.regions.some(r=>r.hardwareRecording))ctx.addIssue({code:'custom',message:'Hardware recording metadata requires an audio region.'});if(t.collapsed&&t.kind!=='bus')ctx.addIssue({code:'custom',message:'Only bus groups can be collapsed.'});if(t.kind!=='audio'&&t.compAlternatives.length)ctx.addIssue({code:'custom',message:'Comp alternatives require an audio track.'});if(t.sampleLoopEnd!==null&&t.sampleLoopEnd<=t.sampleLoopStart)ctx.addIssue({code:'custom',message:'Sampler loop end must follow its start.'});});
const meterPoint=z.object({id:ident,bar:z.number().int().min(2).max(10000000),numerator:z.number().int().min(1).max(32),denominator:z.union([z.literal(1),z.literal(2),z.literal(4),z.literal(8),z.literal(16),z.literal(32),z.literal(64)])});
const keyPoint=z.object({id:ident,beat:z.number().finite().positive().max(432000),sharps:z.number().int().min(-7).max(7),mode:z.enum(['major','minor'])});
const tempoPoint=z.object({id:ident,beat:z.number().finite().positive().max(432000),bpm:z.number().finite().min(20).max(300)});
export const sessionSchema=z.object({samplerPresets:samplerPresetsSchema,scenes:z.array(sceneSchema).max(64).default([]),version:z.literal(1),id:ident,title:z.string().max(200),revision:z.number().int().nonnegative(),tempo:z.number().min(20).max(300),tempoChanges:z.array(tempoPoint).max(256).default([]),meterChanges:z.array(meterPoint).max(256).default([]),keyChanges:z.array(keyPoint).max(256).default([]),effectPresets:z.array(effectPresetSchema).max(32).default([]),scalePresets:z.array(scalePresetSchema).max(64).default([]),keySignature:z.object({sharps:z.number().int().min(-7).max(7),mode:z.enum(['major','minor'])}).strict().nullable().default(null),meter:z.number().int().min(1).max(16),meterDenominator:z.union([z.literal(1),z.literal(2),z.literal(4),z.literal(8),z.literal(16),z.literal(32),z.literal(64)]).default(4),metronomeEnabled:z.boolean().default(false),metronomeRecordEnabled:z.boolean().default(false),countInBars:z.number().int().min(0).max(2).default(0),midiPunchEnabled:z.boolean().default(false),midiPunchStart:time.default(0),midiPunchEnd:time.default(4),audioPunchEnabled:z.boolean().default(false),audioPunchStart:time.default(0),audioPunchEnd:time.default(4),audioRecordMode:z.enum(['layer','takes','cycle']).default('layer'),recordWithPlayback:z.boolean().default(false),midiMonitorEnabled:z.boolean().default(false),audioMonitorEnabled:z.boolean().default(false),audioMonitorDb:z.number().min(-60).max(0).default(-18),metronomeDb:z.number().min(-60).max(0).default(-18),masterDb:db,masterAutomationMode:automationModeSchema.optional(),masterAutomationMuted:automationMutedSchema(['gainDb','pan']),masterPan:z.number().min(-1).max(1).default(0),masterAutomation:z.array(automationSchema).max(2000).default([]),masterEffects:z.array(effectSchema).max(16).default([]),loopEnabled:z.boolean().default(false),loopStart:time.default(0),loopEnd:time.default(4),timecodeOffset:time.default(0),dropFrame:z.boolean().default(false),frameRate:z.number().refine(value=>frameRates.includes(value),'Unsupported frame rate.').default(24),tracks:z.array(track).max(128),sections:arrangementSectionsSchema,markers:z.array(z.object({id:ident,name:z.string().max(200),time})).max(1000)}).superRefine((s,ctx)=>{if(s.dropFrame&&!supportsDropFrame(s.frameRate))ctx.addIssue({code:'custom',path:['dropFrame'],message:'Drop-frame timecode requires 29.97 or 59.94 fps.'});try{const keys=compileKeyMap(s),tempo=compileTempoMap(s);if(keys.points.some(p=>tempo.timeAtBeat(p.beat)>86400))throw Error('Key change exceeds the 24-hour timeline.');if(new Set(s.keyChanges.map(p=>p.id)).size!==s.keyChanges.length)throw Error('Key change IDs must be unique.');}catch(error){ctx.addIssue({code:'custom',path:['keyChanges'],message:error.message});}try{const meter=compileMeterMap(s),tempo=compileTempoMap(s);if(meter.points.some(p=>tempo.timeAtBeat(p.beat)>86400))throw Error('Signature change exceeds the 24-hour timeline.');if(new Set(s.meterChanges.map(p=>p.id)).size!==s.meterChanges.length)throw Error('Signature point IDs must be unique.');}catch(error){ctx.addIssue({code:'custom',path:['meterChanges'],message:error.message});}try{compileTempoMap(s);}catch(error){ctx.addIssue({code:'custom',path:['tempoChanges'],message:error.message});}if(new Set(s.tempoChanges.map(p=>p.id)).size!==s.tempoChanges.length)ctx.addIssue({code:'custom',path:['tempoChanges'],message:'Tempo point IDs must be unique.'});});
...
export const operations=['region.saveMelody','region.clearMelody','notes.addMany','samplerZone.duplicate','samplerZone.deleteMany','samplerZone.addMany','samplerZone.automap','samplerPreset.import','samplerPreset.save','samplerPreset.update','samplerPreset.apply','samplerPreset.rename','samplerPreset.delete','samplerGroup.set','samplerGroup.members','samplerGroup.delete','samplerZone.adjust','samplerZone.range','samplerZone.set','samplerZone.delete','midiSwitch.input','samplerArticulation.set','samplerArticulation.delete','notes.articulation','midiSwitch.save','midiSwitch.delete','midiSwitch.insert','markers.import','markers.fromRegions','scene.performance','scene.place','scene.add','scene.duplicate','scene.rename','scene.delete','scene.move','scene.cell.set','scene.cell.delete','regions.rename','regions.fades','regions.sequence','regions.align','effectPreset.import','effectPreset.save','effectPreset.apply','effectPreset.rename','effectPreset.delete','region.cycleTakes','takes.append','takes.create','takes.select','takes.ungroup','region.commitWarp','regions.quantizeAudio','region.attackMarkers','region.splitAtTimes','region.splitRange','region.commitSilence','region.commitRetune','region.commitPitch','region.commitStretch','regions.tempoFollow','tempo.fit','midi.tuning','midi.bendRange','tempo.constant','tempo.scale','tempo.ramp','event.thin','midi.allocateChannels','midi.remapChannel','keyChange.add','keyChange.set','keyChange.delete','automation.trimRecord','automation.trim','automation.record','scalePreset.add','scalePreset.set','scalePreset.delete','notes.diatonicTranspose','key.set','key.clear','meter.add','meter.set','meter.delete','notes.repeat','tempo.add','tempo.set','tempo.delete','section.add','section.set','section.delete','section.editContent','session.transferSection','session.repeatSection','session.deleteTime','regions.split','session.insertTime','midi.import','master.gain.offset','session.set','track.add','track.createGroup','track.duplicate','track.copySettings','track.comp','comp.save','comp.delete','comp.createTrack','track.move','track.set','track.delete','send.set','send.delete','send.automation.point','send.automation.clear','track.commitBounce','region.commitBounce','region.separateMidi','region.joinMidi','region.timeScale','region.crossfade','region.add','region.extractAudio','region.trim','region.set','regions.paste','regions.duplicate','regions.delete','regions.move','region.move','region.delete','region.split','region.duplicate','region.repeat','event.ramp','event.add','event.set','event.delete','note.add','note.set','note.delete','notes.arpeggiate','notes.timeScale','notes.reverse','notes.legato','notes.scale','notes.chord','notes.velocity','notes.velocityRamp','notes.move','notes.resize','notes.delete','notes.duplicate','notes.quantize','notes.humanize','notes.transpose','notes.invert','notes.mute','notes.split','notes.divide','notes.applySustain','notes.removeDuplicates','notes.join','notes.transfer','marker.add','marker.set','marker.delete','marker.cycle','effect.automation.point','effect.automation.set','effect.automation.delete','effect.automation.clear','effect.add','effect.set','effect.delete','effect.move','automation.parameter','automation.range','automation.point','automation.set','automation.delete','automation.clear'];
...
export function applyCommands(input,commands,expectedRevision=input.revision){
 const validatedCommands=batchSchema.parse(commands);if(validatedCommands.some(c=>c.op==='scene.performance')&&validatedCommands.length!==1)throw Error('Save a scene performance as a separate edit.');
 const session=sessionSchema.parse(structuredClone(input));validateScenes(session);if(session.revision!==expectedRevision)throw Error('The session changed. Run the instruction again.');
 pruneSavedMelodies(session);
 for(const {op,target,values:rawValues} of validatedCommands){
  let v=rawValues;
  ...
  switch(op){
   case 'scalePreset.add':session.scalePresets.push(scalePresetSchema.parse({id:crypto.randomUUID(),...pick(v,['id','name','root','custom'])}));break;
   case 'scalePreset.set':{const p=need(session.scalePresets.find(p=>p.id===target),'Scale preset');Object.assign(p,scalePresetSchema.parse({...p,...pick(v,['name','root','custom'])}));break;}
   case 'scalePreset.delete':need(session.scalePresets.find(p=>p.id===target),'Scale preset');pick(v,[]);session.scalePresets=session.scalePresets.filter(p=>p.id!==target);break;
   case 'keyChange.add':case 'keyChange.set':case 'keyChange.delete':{
    if(op==='keyChange.add')session.keyChanges.push(keyPoint.parse({id:crypto.randomUUID(),...pick(v,['id','beat','sharps','mode'])}));
    else {const point=need(session.keyChanges.find(p=>p.id===target),'Key change');if(op==='keyChange.set')Object.assign(point,keyPoint.parse({...point,...pick(v,['beat','sharps','mode'])}));else {pick(v,[]);session.keyChanges=session.keyChanges.filter(p=>p.id!==target);}}
    session.keyChanges.sort((a,b)=>a.beat-b.beat);compileKeyMap(session);break;
   }
   case 'key.set':session.keySignature=pick(v,['sharps','mode']);break;
   case 'key.clear':pick(v,[]);session.keySignature=null;break;
   case 'meter.add':case 'meter.set':case 'meter.delete':{
    if(op==='meter.add')session.meterChanges.push(meterPoint.parse({id:crypto.randomUUID(),...pick(v,['id','bar','numerator','denominator'])}));
    else {const point=need(session.meterChanges.find(p=>p.id===target),'Signature point');if(op==='meter.set')Object.assign(point,meterPoint.parse({...point,...pick(v,['bar','numerator','denominator'])}));else {pick(v,[]);session.meterChanges=session.meterChanges.filter(p=>p.id!==target);}}
    session.meterChanges.sort((a,b)=>a.bar-b.bar);compileMeterMap(session);break;
   }
   case 'tempo.fit':case 'tempo.constant':case 'tempo.scale':case 'tempo.ramp':{if(target!==undefined&&target!==session.id)throw Error('Target the current session for a tempo curve.');const previous={tempo:session.tempo,tempoChanges:structuredClone(session.tempoChanges)},plan=op==='tempo.fit'?fitTempoPlan(session,v):op==='tempo.constant'?constantTempoPlan(session,v):op==='tempo.scale'?scaleTempoPlan(session,v):tempoRampPlan(session,v);session.tempo=plan.tempo;session.tempoChanges=plan.tempoChanges;retimeMidiTracks(session.tracks,previous,session);break;}
   ...
   case 'midi.import':{
    ...
    const articulation=v.articulation===undefined?undefined:midiSwitchSchema.parse(JSON.parse(v.articulation));
    const imported=midi.tracks.map(source=>{
     ...
     return track.parse({id:crypto.randomUUID(),name:source.name.slice(0,200),kind:'midi',gainDb:0,pan:0,mute:false,solo:false,instrument:'triangle',
      regions:[{id:crypto.randomUUID(),name:source.name.slice(0,200),assetId:null,start:0,offset:0,duration,gainDb:0,fadeIn:0,fadeOut:0,reverse:false,notes:source.notes.map(n=>articulation?{...n,articulation:structuredClone(articulation)}:n),events:source.events}],
     });
    });
   ...
   case 'session.set':{const previousTiming={tempo:session.tempo,tempoChanges:structuredClone(session.tempoChanges)};Object.assign(session,pick(v,['title','tempo','meter','meterDenominator','metronomeEnabled','metronomeRecordEnabled','countInBars','recordWithPlayback','audioRecordMode','audioPunchEnabled','audioPunchStart','audioPunchEnd','midiPunchEnabled','midiPunchStart','midiPunchEnd','midiMonitorEnabled','audioMonitorEnabled','audioMonitorDb','metronomeDb','masterDb','masterPan','masterAutomationMode','timecodeOffset','dropFrame','frameRate','loopEnabled','loopStart','loopEnd']));if(v.tempo!==undefined)retimeMidiTracks(session.tracks,previousTiming,session);break;}
   case 'track.add':session.tracks.push(track.parse({id:crypto.randomUUID(),name:'New track',kind:'audio',gainDb:0,pan:0,mute:false,solo:false,instrument:'triangle',regions:[],...pick(v,['id','name','color','kind','scoreInstrument','scoreClef','instrument','pitchBendRange','sampleAssetId','sampleRoot','sampleTune','sampleFineTune','sampleAttack','sampleDecay','sampleSustain','sampleRelease','sampleLoop','sampleLoopStart','sampleLoopEnd','sampleFilterType','sampleFilterCutoff','sampleFilterResonance','sampleFilterKeyTrack'])}));break;
   ...
   case 'region.cycleTakes':createCycleTakes(session,target,v);break;
   case 'takes.append':case 'takes.create':case 'takes.select':case 'takes.ungroup':editTakeGroup(t,op.slice(6),v);break;
   ...
   case 'track.set':Object.assign(need(t,'Track'),pick(v,['name','scoreInstrument','scoreClef','color','gainDb','pan','mute','solo','collapsed','protected','automationMode','instrument','pitchBendRange','sampleAssetId','sampleRoot','sampleTune','sampleFineTune','sampleAttack','sampleDecay','sampleSustain','sampleRelease','sampleLoop','sampleLoopStart','sampleLoopEnd','sampleFilterType','sampleFilterCutoff','sampleFilterResonance','sampleFilterKeyTrack','output']));break;
   case 'track.delete':need(t,'Track');for(const e of [...session.masterEffects,...session.tracks.flatMap(t=>t.effects)])if(e.kind==='gate'&&e.sidechainTrackId===target)e.sidechainTrackId=null;session.tracks=session.tracks.filter(x=>x!==t);for(const other of session.tracks){if(other.output===target)other.output=null;other.sends=other.sends.filter(s=>s.busId!==target);}break;
   ...
   case 'region.add':if(v.tempoFollow!==undefined&&need(t,'Track').kind!=='midi')throw Error('Tempo following is a MIDI region setting.');need(t,'Track').regions.push(region.parse({id:crypto.randomUUID(),name:'Region',assetId:null,start:0,offset:0,duration:4,gainDb:0,fadeIn:0,fadeOut:0,reverse:false,notes:[],...pick(v,['id','name','assetId','start','offset','duration','tempoFollow','hardwareRecording']),...(v.hardwareRecording===undefined?{}:{hardwareRecording:hardwareRecordingSchema.parse(JSON.parse(v.hardwareRecording))})}));break;
   case 'region.set':if(v.tempoFollow!==undefined&&need(owner,'Track').kind!=='midi')throw Error('Tempo following is a MIDI region setting.');Object.assign(need(r,'Region'),pick(v,['name','start','offset','duration','gainDb','fadeIn','fadeOut','fadeInShape','fadeOutShape','reverse','mute','tempoFollow']));break;
   ...
   case 'regions.tempoFollow':setSelectedTempoFollow(session,v);break;
   ...
   case 'regions.quantizeAudio':quantizeAudioRegions(session,v);break;
   case 'scene.performance':commitScenePerformance(session,v);break;
   case 'scene.place':placeScene(session,target,v);break;
   case 'scene.add':case 'scene.duplicate':case 'scene.rename':case 'scene.delete':case 'scene.move':case 'scene.cell.set':case 'scene.cell.delete':editScene(session,op,target,v);break;
   case 'regions.rename':{const edits=new Map(renamedRegions(session,v).map(e=>[e.id,e.name]));for(const track of session.tracks)for(const region of track.regions)if(edits.has(region.id))region.name=edits.get(region.id);break;}
   case 'regions.fades':{const edits=new Map(batchRegionFades(session,v).map(e=>[e.id,e.values]));for(const track of session.tracks)for(const region of track.regions)if(edits.has(region.id))Object.assign(region,edits.get(region.id));break;}
   case 'regions.sequence':{const edits=new Map(sequencedRegions(session,v).map(e=>[e.id,e.start]));for(const track of session.tracks)for(const region of track.regions)if(edits.has(region.id))region.start=edits.get(region.id);break;}
   case 'regions.align':{const edits=new Map(alignedRegions(session,v).map(e=>[e.id,e.start]));for(const track of session.tracks)for(const region of track.regions)if(edits.has(region.id))region.start=edits.get(region.id);break;}
   ...
   case 'region.commitSilence':{need(r,'Region');const {track:copy,sourceTrackId}=silenceRegionTrack(session,r.id,v);session.tracks.splice(session.tracks.findIndex(t=>t.id===sourceTrackId)+1,0,copy);r.mute=true;break;}
   case 'region.commitWarp':{need(r,'Region');const {track:copy,sourceTrackId}=warpedRegionTrack(session,r.id,v);session.tracks.splice(session.tracks.findIndex(t=>t.id===sourceTrackId)+1,0,copy);r.mute=true;break;}
   case 'region.commitRetune':
   case 'region.commitPitch':
   case 'region.commitStretch':{need(r,'Region');const {track:copy,sourceTrackId}=(op==='region.commitRetune'?retunedRegionTrack:op==='region.commitPitch'?pitchedRegionTrack:stretchedRegionTrack)(session,r.id,v);session.tracks.splice(session.tracks.findIndex(t=>t.id===sourceTrackId)+1,0,copy);r.mute=true;break;}
   ...
   case 'region.saveMelody':need(r,'Region');setSavedMelody(owner,r,v);break;
   case 'region.clearMelody':need(r,'Region');pick(v,[]);delete r.melodyDraft;break;
   case 'region.attackMarkers':need(r,'Region');setAttackMarkers(owner,r,v);break;
   case 'region.splitAtTimes':need(r,'Region');splitAudioAtTimes(owner,r,v);break;
   case 'region.splitRange':need(r,'Region');splitAudioRange(owner,r,v);break;
   case 'region.split':{
   ...
   case 'midi.tuning':case 'midi.bendRange':{need(r,'Region');if(owner.kind!=='midi')throw Error('Choose a MIDI region.');const plan=op==='midi.tuning'?midiTuningPlan(r,v):midiBendRangePlan(r,v,owner.pitchBendRange);r.events.push(...plan.events.map(e=>({...e,id:crypto.randomUUID()})));break;}
   case 'samplerPreset.import':case 'samplerPreset.save':case 'samplerPreset.update':case 'samplerPreset.apply':case 'samplerPreset.rename':case 'samplerPreset.delete':editSamplerPreset(session,op,target,v);break;
   case 'samplerGroup.set':case 'samplerGroup.members':case 'samplerGroup.delete':editSamplerGroup(t,op,v);break;
   case 'samplerZone.duplicate':case 'samplerZone.deleteMany':case 'samplerZone.addMany':case 'samplerZone.automap':case 'samplerZone.adjust':case 'samplerZone.range':case 'samplerZone.set':case 'samplerZone.delete':editSamplerZone(t,op,v);break;
   case 'samplerArticulation.set':setSamplerArticulation(t,v);break;
   case 'samplerArticulation.delete':removeSamplerArticulation(t,v);break;
   case 'notes.articulation':assignNoteArticulation(owner,r,v);break;
   case 'midiSwitch.input':{need(t,'Track');if(t.kind!=='midi')throw Error('Choose an instrument track.');pick(v,['id']);if(v.id===null)t.inputArticulation=null;else t.inputArticulation=structuredClone(need(t.midiSwitches?.find(p=>p.id===v.id),'MIDI switch'));break;}
   case 'midiSwitch.save':saveMidiSwitch(t,v);break;
   case 'midiSwitch.delete':removeMidiSwitch(t,v);break;
   case 'midiSwitch.insert':insertMidiSwitch(owner,r,v);break;
   ...
   case 'notes.addMany':{need(r,'Region');if(owner.kind!=='midi')throw Error('Choose a MIDI region.');const values=z.object({notes:z.string().max(1000000)}).strict().parse(v),notes=z.array(note.strict()).min(1).max(20000).parse(JSON.parse(values.notes));r.notes.push(...notes);break;}
   ...
   case 'event.thin':need(r,'Region');if(owner.kind!=='midi')throw Error('Choose a MIDI region.');r.events=thinControllerPlan(r,v).events;break;
   case 'midi.allocateChannels':if(target!==undefined&&target!==session.id)throw Error('Target the current session for channel allocation.');pick(v,[]);allocateMidiChannels(session);break;
   case 'midi.remapChannel':{const targetTrack=t??owner;need(targetTrack,'Track or region');if(targetTrack.kind!=='midi')throw Error('Choose a MIDI track or region.');remapMidiChannel(t?t.regions:[need(r,'Region')],v);break;}
   ...
   case 'notes.scale':{need(r,'Region');if(owner.kind!=='midi')throw Error('Choose a MIDI region.');const edits=new Map(scaleNoteEdits(r,v,session).map(e=>[e.id,e.pitch]));for(const note of r.notes)if(edits.has(note.id))note.pitch=edits.get(note.id);break;}
   ...
   case 'notes.diatonicTranspose':{need(r,'Region');if(owner.kind!=='midi')throw Error('Choose a MIDI region.');const edits=new Map(diatonicNoteEdits(r,v,session).map(e=>[e.id,e.pitch]));for(const note of r.notes)if(edits.has(note.id))note.pitch=edits.get(note.id);break;}
   case 'notes.transpose':{need(r,'Region');if(owner.kind!=='midi')throw Error('Choose a MIDI region.');const edits=new Map(transposeNoteEdits(r,v,session).map(e=>[e.id,e.pitch]));for(const note of r.notes)if(edits.has(note.id))note.pitch=edits.get(note.id);break;}
   ...
   case 'effectPreset.import':case 'effectPreset.save':case 'effectPreset.apply':case 'effectPreset.rename':case 'effectPreset.delete':editEffectPreset(session,op,target,v);break;
   ...
   case 'automation.trimRecord':applyAutomationTrimGesture(session,target,v);break;
   case 'automation.trim':applyAutomationTrim(session,target,v);break;
   case 'automation.record':applyAutomationGesture(session,target,v);break;
   ...
   case 'markers.fromRegions':session.markers.push(...markersFromRegions(session,v));break;
   case 'markers.import':session.markers.push(...importedMarkers(session,v));break;
   ...
   case 'marker.cycle':Object.assign(session,markerCyclePlan(session,target,pick(v,['endMarkerId'])));break;
  ...
  pruneSavedMelodies(session);pruneSceneReferences(session);
 ...
 pruneSceneReferences(session);validateScenes(session);
 session.revision++;sessionSchema.parse(session);audioRecordingWindow({...session,audioRecordMode:'layer',audioPunchEnabled:true});midiRecordingWindow({...session,midiPunchEnabled:true});if(session.loopEnd<=session.loopStart)throw Error('Cycle end must follow its start.');validateRouting(session);
 const ids=[session.id,...session.samplerPresets.map(p=>p.id),...session.scenes.map(s=>s.id),...session.effectPresets.flatMap(p=>[p.id,...p.effects.flatMap(e=>[e.id,...e.automation.map(a=>a.id)])]),...session.keyChanges.map(p=>p.id),...session.scalePresets.map(p=>p.id),...session.meterChanges.map(p=>p.id),...session.tempoChanges.map(p=>p.id),...session.sections.map(s=>s.id),...session.masterAutomation.map(p=>p.id),...session.masterEffects.flatMap(e=>[e.id,...e.automation.map(p=>p.id)]),...session.tracks.flatMap(t=>[t.id,...(t.compAlternatives||[]).map(c=>c.id),...t.effects.flatMap(e=>[e.id,...e.automation.map(p=>p.id)]),...t.automation.map(p=>p.id),...t.sends.flatMap(s=>s.automation.map(p=>p.id)),...t.regions.flatMap(r=>[r.id,...r.notes.map(n=>n.id),...r.events.map(e=>e.id)])]),...session.markers.map(m=>m.id)];if(new Set(ids).size!==ids.length)throw Error('IDs must be unique.');
...
export class SessionHistory{
 constructor(session=newSession()){this.session=sessionSchema.parse(session);pruneSavedMelodies(this.session);validateRouting(this.session);validateScenes(this.session);this.past=[];this.future=[];}
