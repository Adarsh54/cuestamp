...
export function applyCommands(input,commands,expectedRevision=input.revision){
 ...
 for(const {op,target,values:rawValues} of batchSchema.parse(commands)){
  ...
  switch(op){
   ...
   case 'tempo.constant':case 'tempo.scale':case 'tempo.ramp':{if(target!==undefined&&target!==session.id)throw Error('Target the current session for a tempo curve.');const previous={tempo:session.tempo,tempoChanges:structuredClone(session.tempoChanges)},plan=op==='tempo.constant'?constantTempoPlan(session,v):op==='tempo.scale'?scaleTempoPlan(session,v):tempoRampPlan(session,v);session.tempo=plan.tempo;session.tempoChanges=plan.tempoChanges;retimeMidiTracks(session.tracks,previous,session);break;}
