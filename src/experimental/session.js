...
export function applyCommands(input,commands,expectedRevision=input.revision){
 ...
 for(const {op,target,values:rawValues} of validatedCommands){
  ...
  switch(op){
   ...
   case 'region.add':if(v.tempoFollow!==undefined&&need(t,'Track').kind!=='midi')throw Error('Tempo following is a MIDI region setting.');need(t,'Track').regions.push(region.parse({id:crypto.randomUUID(),name:'Region',assetId:null,start:0,offset:0,duration:4,gainDb:0,fadeIn:0,fadeOut:0,reverse:false,notes:[],...pick(v,['id','name','assetId','start','offset','duration','tempoFollow'])}));break;
