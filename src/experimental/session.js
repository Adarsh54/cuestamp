...
export function applyCommands(input,commands,expectedRevision=input.revision){
 ...
 for(const {op,target,values:v} of batchSchema.parse(commands)){
  ...
  switch(op){
   ...
   case 'track.set':Object.assign(need(t,'Track'),pick(v,['name','gainDb','pan','mute','solo','instrument','sampleAssetId','sampleRoot','sampleAttack','sampleDecay','sampleSustain','sampleRelease','sampleLoop','sampleLoopStart','sampleLoopEnd','output']));break;
