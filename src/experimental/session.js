...
import {bouncedRegionTrack} from './bounce-in-place.js';
...
const track=z.object({id:ident,name:z.string().max(200),kind:z.enum(['audio','midi','video','bus']),gainDb:db,pan:z.number().min(-1).max(1),mute:z.boolean(),solo:z.boolean(),instrument:z.enum(['sine','triangle','square','sawtooth','drumKit','sampler']),sampleAssetId:ident.nullable().default(null),sampleRoot:z.number().int().min(0).max(127).default(60),sampleAttack:z.number().min(0).max(10).default(.005),sampleDecay:z.number().min(0).max(10).default(0),sampleSustain:z.number().min(0).max(1).default(1),sampleRelease:z.number().min(0).max(30).default(.02),...samplerTuningSchema.shape,...samplerFilterSchema.shape,sampleLoop:z.boolean().default(false),sampleLoopStart:time.default(0),sampleLoopEnd:time.nullable().default(null),regions:z.array(region).max(1000),output:ident.nullable().default(null),sends:z.array(z.object({busId:ident,gainDb:db,tap:z.enum(['preFader','postFader','postPan']).default('postPan'),automation:z.array(automationSchema.refine(p=>p.parameter==='gainDb','Send automation supports gain only.')).max(2000).default([])})).max(16).default([]),effects:z.array(effectSchema).max(16).default([]),automation:z.array(automationSchema).max(2000).default([])}).superRefine((t,ctx)=>{if(t.sampleLoopEnd!==null&&t.sampleLoopEnd<=t.sampleLoopStart)ctx.addIssue({code:'custom',message:'Sampler loop end must follow its start.'});});
export const sessionSchema=z.object({version:z.literal(1),id:ident,title:z.string().max(200),revision:z.number().int().nonnegative(),tempo:z.number().min(20).max(300),meter:z.number().int().min(1).max(16),metronomeEnabled:z.boolean().default(false),metronomeRecordEnabled:z.boolean().default(false),countInBars:z.number().int().min(0).max(2).default(0),audioPunchEnabled:z.boolean().default(false),audioPunchStart:time.default(0),audioPunchEnd:time.default(4),recordWithPlayback:z.boolean().default(false),midiMonitorEnabled:z.boolean().default(false),audioMonitorEnabled:z.boolean().default(false),audioMonitorDb:z.number().min(-60).max(0).default(-18),metronomeDb:z.number().min(-60).max(0).default(-18),masterDb:db,masterPan:z.number().min(-1).max(1).default(0),masterAutomation:z.array(automationSchema).max(2000).default([]),masterEffects:z.array(effectSchema).max(16).default([]),loopEnabled:z.boolean().default(false),loopStart:time.default(0),loopEnd:time.default(4),frameRate:z.number().refine(value=>frameRates.includes(value),'Unsupported frame rate.').default(24),tracks:z.array(track).max(128),markers:z.array(z.object({id:ident,name:z.string().max(200),time})).max(1000)});
...
export const operations=['midi.import','master.gain.offset','session.set','track.add','track.duplicate','track.copySettings','track.move','track.set','track.delete','send.set','send.delete','send.automation.point','send.automation.clear','region.commitBounce','region.separateMidi','region.joinMidi','region.timeScale','region.crossfade','region.add','region.extractAudio','region.trim','region.set','regions.paste','regions.duplicate','regions.delete','regions.move','region.move','region.delete','region.split','region.duplicate','region.repeat','event.ramp','event.add','event.set','event.delete','note.add','note.set','note.delete','notes.arpeggiate','notes.timeScale','notes.reverse','notes.legato','notes.scale','notes.chord','notes.velocity','notes.velocityRamp','notes.move','notes.resize','notes.delete','notes.duplicate','notes.quantize','notes.humanize','notes.transpose','notes.invert','notes.mute','notes.removeDuplicates','notes.transfer','marker.add','marker.set','marker.delete','effect.automation.point','effect.automation.set','effect.automation.delete','effect.automation.clear','effect.add','effect.set','effect.delete','effect.move','automation.point','automation.set','automation.delete','automation.clear'];
...
export function applyCommands(input,commands,expectedRevision=input.revision){
 ...
 for(const {op,target,values:v} of batchSchema.parse(commands)){
  ...
  switch(op){
   case 'midi.import':{
    pick(v,['data','start','trackId']);const destination=v.trackId===undefined?null:need(session.tracks.find(t=>t.id===v.trackId),'Destination track');if(destination&&destination.kind!=='midi')throw Error('MIDI takes require an instrument track.');const start=time.parse(v.start??0),midi=readMidi(decodeMidiImport(v.data));
   ...
   case 'track.set':Object.assign(need(t,'Track'),pick(v,['name','gainDb','pan','mute','solo','instrument','sampleAssetId','sampleRoot','sampleTune','sampleFineTune','sampleAttack','sampleDecay','sampleSustain','sampleRelease','sampleLoop','sampleLoopStart','sampleLoopEnd','sampleFilterType','sampleFilterCutoff','sampleFilterResonance','sampleFilterKeyTrack','output']));break;
   ...
   case 'notes.timeScale':{need(r,'Region');if(owner.kind!=='midi')throw Error('Choose a MIDI region.');const plan=timeScaleNotes(r,v),edits=new Map(plan.edits.map(n=>[n.id,n]));for(const note of r.notes)if(edits.has(note.id))Object.assign(note,edits.get(note.id));r.duration=plan.duration;break;}
   case 'notes.reverse':{need(r,'Region');if(owner.kind!=='midi')throw Error('Choose a MIDI region.');const edits=new Map(reverseNoteEdits(r,v).map(e=>[e.id,e.start]));for(const note of r.notes)if(edits.has(note.id))note.start=edits.get(note.id);break;}
   case 'notes.arpeggiate':{need(r,'Region');if(owner.kind!=='midi')throw Error('Choose a MIDI region.');const plan=arpeggioPlan(r,session.tempo,v),removed=new Set(plan.removedIds);r.notes=[...r.notes.filter(n=>!removed.has(n.id)),...plan.notes.map(n=>({...n,id:crypto.randomUUID()}))];break;}
   case 'notes.legato':{need(r,'Region');if(owner.kind!=='midi')throw Error('Choose a MIDI region.');const edits=new Map(legatoEdits(r,v).map(e=>[e.id,e.duration]));for(const note of r.notes)if(edits.has(note.id))note.duration=edits.get(note.id);break;}
 ...
 session.revision++;sessionSchema.parse(session);audioRecordingWindow({...session,audioPunchEnabled:true});if(session.loopEnd<=session.loopStart)throw Error('Cycle end must follow its start.');validateRouting(session);
 const ids=[session.id,...session.masterAutomation.map(p=>p.id),...session.masterEffects.flatMap(e=>[e.id,...e.automation.map(p=>p.id)]),...session.tracks.flatMap(t=>[t.id,...t.effects.flatMap(e=>[e.id,...e.automation.map(p=>p.id)]),...t.automation.map(p=>p.id),...t.sends.flatMap(s=>s.automation.map(p=>p.id)),...t.regions.flatMap(r=>[r.id,...r.notes.map(n=>n.id),...r.events.map(e=>e.id)])]),...session.markers.map(m=>m.id)];if(new Set(ids).size!==ids.length)throw Error('IDs must be unique.');
