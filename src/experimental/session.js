import {createAudioComp} from './audio-comp.js';
...
export const sessionSchema=z.object({version:z.literal(1),id:ident,title:z.string().max(200),revision:z.number().int().nonnegative(),tempo:z.number().min(20).max(300),meter:z.number().int().min(1).max(16),metronomeEnabled:z.boolean().default(false),metronomeRecordEnabled:z.boolean().default(false),countInBars:z.number().int().min(0).max(2).default(0),midiPunchEnabled:z.boolean().default(false),midiPunchStart:time.default(0),midiPunchEnd:time.default(4),audioPunchEnabled:z.boolean().default(false),audioPunchStart:time.default(0),audioPunchEnd:time.default(4),recordWithPlayback:z.boolean().default(false),midiMonitorEnabled:z.boolean().default(false),audioMonitorEnabled:z.boolean().default(false),audioMonitorDb:z.number().min(-60).max(0).default(-18),metronomeDb:z.number().min(-60).max(0).default(-18),masterDb:db,masterPan:z.number().min(-1).max(1).default(0),masterAutomation:z.array(automationSchema).max(2000).default([]),masterEffects:z.array(effectSchema).max(16).default([]),loopEnabled:z.boolean().default(false),loopStart:time.default(0),loopEnd:time.default(4),frameRate:z.number().refine(value=>frameRates.includes(value),'Unsupported frame rate.').default(24),tracks:z.array(track).max(128),markers:z.array(z.object({id:ident,name:z.string().max(200),time})).max(1000)});
