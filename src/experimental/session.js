...
export function applyCommands(input,commands,expectedRevision=input.revision){
 ...
 const ids=[session.id,...session.scenes.map(s=>s.id),...session.effectPresets.flatMap(p=>[p.id,...p.effects.flatMap(e=>[e.id,...e.automation.map(a=>a.id)])]),...session.keyChanges.map(p=>p.id),...session.scalePresets.map(p=>p.id),...session.meterChanges.map(p=>p.id),...session.tempoChanges.map(p=>p.id),...session.sections.map(s=>s.id),...session.masterAutomation.map(p=>p.id),...session.masterEffects.flatMap(e=>[e.id,...e.automation.map(p=>p.id)]),...session.tracks.flatMap(t=>[t.id,...(t.compAlternatives||[]).map(c=>c.id),...t.effects.flatMap(e=>[e.id,...e.automation.map(p=>p.id)]),...t.automation.map(p=>p.id),...t.sends.flatMap(s=>s.automation.map(p=>p.id)),...t.regions.flatMap(r=>[r.id,...r.notes.map(n=>n.id),...r.events.map(e=>e.id)])]),...session.markers.map(m=>m.id)];if(new Set(ids).size!==ids.length)throw Error('IDs must be unique.');
