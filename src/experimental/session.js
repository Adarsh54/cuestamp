import {stretchedRegionTrack} from './audio-stretch-region.js';
...
export function applyCommands(input,commands,expectedRevision=input.revision){
 ...
 for(const {op,target,values:rawValues} of batchSchema.parse(commands)){
  ...
  switch(op){
   ...
   case 'region.commitStretch':{need(r,'Region');const {track:copy,sourceTrackId}=stretchedRegionTrack(session,r.id,v);session.tracks.splice(session.tracks.findIndex(t=>t.id===sourceTrackId)+1,0,copy);r.mute=true;break;}
