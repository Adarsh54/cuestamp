...
export function applyCommands(input,commands,expectedRevision=input.revision){
 ...
 for(const {op,target,values:rawValues} of batchSchema.parse(commands)){
  ...
  switch(op){
   ...
   case 'effectPreset.save':case 'effectPreset.apply':case 'effectPreset.rename':case 'effectPreset.delete':editEffectPreset(session,op,target,v);break;
