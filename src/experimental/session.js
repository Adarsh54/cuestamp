...
export function applyCommands(input,commands,expectedRevision=input.revision){
 ...
 for(const {op,target,values:rawValues} of validatedCommands){
  ...
  switch(op){
   ...
   case 'track.add':session.tracks.push(track.parse({id:crypto.randomUUID(),name:'New track',kind:'audio',gainDb:0,pan:0,mute:false,solo:false,instrument:'triangle',regions:[],...pick(v,['id','name','color','kind','scoreClef','instrument','pitchBendRange','sampleAssetId','sampleRoot','sampleTune','sampleFineTune','sampleAttack','sampleDecay','sampleSustain','sampleRelease','sampleLoop','sampleLoopStart','sampleLoopEnd','sampleFilterType','sampleFilterCutoff','sampleFilterResonance','sampleFilterKeyTrack'])}));break;
