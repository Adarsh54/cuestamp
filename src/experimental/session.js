...
export function applyCommands(input,commands,expectedRevision=input.revision){
 ...
 for(const {op,target,values:rawValues} of batchSchema.parse(commands)){
  ...
  switch(op){
   ...
   case 'midi.bendRange':{need(r,'Region');if(owner.kind!=='midi')throw Error('Choose a MIDI region.');const plan=midiBendRangePlan(r,v,owner.pitchBendRange);r.events.push(...plan.events.map(e=>({...e,id:crypto.randomUUID()})));break;}
