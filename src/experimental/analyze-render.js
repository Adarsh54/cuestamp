...
export function analyzeRender(buffer,{signal,loudness=false,truePeak=false}={}){
 ...
 return new Promise((resolve,reject)=>{
  ...
  worker.onmessage=({data})=>finish(data.error?new Error(data.error):null,{channels:data.channels,stereo:data.stereo,stereoWindows:data.stereoWindows,...(data.truePeak?{truePeak:data.truePeak}:{}),...(data.loudness?{loudness:data.loudness}:{})});worker.onerror=()=>finish(new Error('Audio analysis worker failed.'));
