...
export function scorePitchView(session,track,mode='written'){
 if(!['written','concert'].includes(mode))throw Error('Choose written or concert pitch.');
 if(mode==='written')return {session,track};
 // Display-only copies preserve the document's instrument settings and original region/note identities.
 const tracks=session.tracks.map(t=>({...t,scoreInstrument:'concert'}));
 return {session:{...session,tracks},track:track?tracks.find(t=>t.id===track.id):track};
}
