import {scoreTimingToolsView,bindScoreTimingTools} from './score-timing-tools.js';
import {scoreClipboardView,bindScoreClipboard} from './score-clipboard.js';
import {rememberScoreSelection,rememberedScoreSelection} from './score-agent-selection.js';
...
export const scoreNoteEditorView=()=>`<form data-score-note-editor hidden><p data-score-note-label></p>${scoreRhythmView()}<div class="button-row"><label>Sounding MIDI pitch<input name="pitch" type="number" min="0" max="127" step="1" required></label><label>Start · seconds in region<input name="start" type="number" min="0" step="any" required></label><label>Duration · seconds<input name="duration" type="number" min="0.001" step="any" required></label><label>Velocity<input name="velocity" type="number" min="0" max="1" step="any" required></label><button type="submit">Apply note edit</button><button type="button" data-score-note-chord>Add chord tone</button><button type="button" data-score-note-play>Play selection</button><button type="button" data-score-note-copy>Copy notes</button><button type="button" data-score-note-cut>Cut notes</button><button type="button" data-score-note-duplicate>Duplicate after</button><button type="button" data-score-note-delete>Delete note</button><button type="button" data-score-note-close>Cancel</button></div>${scoreSelectionToolsView()}${scoreTimingToolsView()}</form>${scoreClipboardView()}`;
...
export function bindScoreNotes(panel,renderer,{session,track,region,scope,execute,guard,selectionRoot,getSession,scoreShortcutsBlocked,notationSession=session,notationTrack=track,notationRegion=region,auditionNote,auditionSelection}){
 ...
 const select=(reference,extend=false,preserve=false)=>{
  ...
  if(!restoring)rememberScoreSelection(selectionRoot,session.id,note.id?{regionId:sourceRegion.id,noteId:note.id,noteIds:[...selectedIds]}:null);
  ...
  for(const action of ['duplicate','delete','chord','copy','cut','play'])form.querySelector(`[data-score-note-${action}]`).hidden=!note.id;
  ...
  for(const input of form.querySelectorAll('input,select'))if(!input.closest('[data-score-selection-tools],[data-score-timing-tools]'))input.closest('label').hidden=multiple;
  form.querySelector('[data-score-selection-tools]').hidden=!multiple;form.querySelector('[data-score-timing-tools]').hidden=!note.id;
 ...
 for(const [element,target] of targets){
  ...
  bindScorePitchDrag(element,{session,reference,zoom:renderer.Zoom,execute,guard,onSelect:e=>{select(reference,Boolean(e?.shiftKey));if(selectedIds.has(reference.noteId))auditionNote?.(reference);},onDelete:()=>{const ids=selectedIds.has(reference.noteId)?[...selectedIds]:[reference.noteId],r=session.tracks.flatMap(t=>t.regions).find(r=>r.id===reference.regionId);execute([scoreNoteGroupAction(session,r,ids,'delete')],'Deleted score notes');}});
 }
 form.querySelector('[data-score-note-play]').onclick=guard(()=>auditionSelection?.({regionId:selectedRegion?.id,noteIds:[...selectedIds]}));
 ...
 bindScoreTimingTools(form,{session,selection:()=>({region:selectedRegion,ids:[...selectedIds]}),execute,guard});
 ...
 form.querySelector('[data-score-note-close]').onclick=()=>{form.hidden=true;selected=null;selectedIds.clear();rememberScoreSelection(selectionRoot,session.id,null);delete panel.dataset.selectedScoreNote;delete panel.dataset.selectedScoreRegion;delete panel.dataset.selectedScoreNotes;panel.querySelectorAll('[data-score-note]').forEach(el=>{el.style.fill='';el.setAttribute('aria-pressed','false');});};
 const memory=rememberedScoreSelection(selectionRoot,session.id);
 if(memory){
  const visible=new Set([...targets.values()].filter(Boolean).filter(t=>t.reference.regionId===memory.regionId).map(t=>t.reference.noteId));
  const ids=memory.noteIds.filter(id=>visible.has(id));
  if(ids.length){
   selectedRegion=session.tracks.flatMap(t=>t.regions).find(r=>r.id===memory.regionId);
   selectedIds=new Set(ids);restoring=true;
   select({regionId:memory.regionId,noteId:ids.includes(memory.noteId)?memory.noteId:ids[0]},true,true);
   restoring=false;
  }
 }
