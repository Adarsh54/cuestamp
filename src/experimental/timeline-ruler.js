...
import {formatSessionTimecode,parseSessionTimecode,exactFrameRate} from './timecode.js';
export const rulerModes=[['seconds','Seconds'],['musical','Bars / beats'],['timecode','Timecode']];
...
export function timelinePosition(time,session,mode){return mode==='musical'?formatMusicalPosition(time,session):mode==='timecode'?formatSessionTimecode(time,session):time.toFixed(2)+' s';}
export function parseTimelinePosition(text,session,mode){const time=mode==='musical'?parseMusicalPosition(text,session):mode==='timecode'?parseSessionTimecode(text.trim(),session):text.trim()?Number(text):NaN;if(!Number.isFinite(time)||time<0||time>86400)throw Error('Choose a position between 0 and 86,400 seconds.');return time;}
...
export function timelineTicks(session,mode,zoom,width){
 ...
 const count=Math.min(1000,Math.ceil(end/step));return Array.from({length:count},(_,i)=>{const time=i*step,label=mode==='musical'?formatMusicalPosition(time,session).split(':').slice(0,2).join('|'):mode==='timecode'?formatSessionTimecode(time,session):`${Number(time.toFixed(6))}s`;return {time,label};});
}
...
export function positionJump(session,position,mode,disabled){const label=mode==='musical'?'Bar : beat : tick':mode==='timecode'?'Timecode':'Position · seconds',value=mode==='seconds'?position.toFixed(3):timelinePosition(position,session,mode);return `<form data-position-jump class="daw-position-jump"><label>${label}<input name="position" aria-label="Go to position" value="${value}" required ${disabled?'disabled':''}></label><button ${disabled?'disabled':''}>Go to position</button>${mode==='musical'?'<small>Bars and beats start at 1; 960 ticks per beat.</small>':''}</form>`;}
