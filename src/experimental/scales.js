...
const options=z.object({root:z.number().int().min(0).max(11),scale:z.enum([...scales.map(([id])=>id),'custom']),custom:z.string().max(40).optional(),direction:z.enum(['nearest','up','down']).default('nearest'),tie:z.enum(['down','up']).default('down'),noteId:z.string().optional(),noteIds:z.string().max(2020000).optional()}).strict();
...
export function scalePitchClasses(root,scale,custom){const parsed=options.parse({root,scale,custom});return new Set(scaleIntervals(parsed.scale,parsed.custom).map(interval=>(interval+root)%12));}
export function scaleNoteEdits(region,values){
 const v=options.parse(values),classes=scalePitchClasses(v.root,v.scale,v.custom),allowed=Array.from({length:128},(_,i)=>i).filter(p=>classes.has(p%12));
