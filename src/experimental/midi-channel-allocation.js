...
export function channelAllocationView(session,esc,{disabled=false,heading='Organize MIDI channels for export'}={}){
 if(!session.tracks.some(t=>t.kind==='midi'))return '';
 let content;try{const plan=midiChannelAllocation(session),changed=plan.filter(p=>p.from!==p.to);content=changed.length?`<ul>${changed.map(p=>`<li>${esc(p.name)} · Channel ${p.from+1} → ${p.to+1}</li>`).join('')}</ul><button type="button" data-allocate-midi-channels ${disabled?'disabled':''}>Assign independent channels</button>`:'<p class="muted">Instrument parts already use independent channels.</p>';}catch(error){content=`<p role="status">${esc(error.message)}</p>`;}
 return `<details class="daw-midi-channel-allocation"><summary>${esc(heading)}</summary>${content}<p class="muted">Each original channel on each instrument track gets an independent channel, including muted parts and controller-only streams. Drum tracks stay unchanged; their channels and channel 10 are reserved. Protected tracks stay on their current channels. Notes and controllers move together in one undo step.</p></details>`;
}
export function bindChannelAllocation(root,{execute,guard,blocked=()=>false}){root.querySelectorAll('[data-allocate-midi-channels]').forEach(button=>button.addEventListener('click',guard(()=>{if(blocked())throw Error('Finish the current operation before assigning MIDI channels.');execute([{op:'midi.allocateChannels'}],'Assigned independent MIDI channels');})));}
