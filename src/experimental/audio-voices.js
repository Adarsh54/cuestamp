...
export function validateTrackSources(track,buffers){
 ...
 if(track.kind==='midi'&&track.instrument==='sampler'){const sources=new Map();for(const r of track.regions)for(const n of r.notes)if(!n.mute&&n.velocity>0){const settings=sampleForNote(track,n);sources.set(n.articulation?.id||'',settings);}for(const settings of sources.values()){if(!buffers.has(settings.sampleAssetId))throw Error(`Missing sampler source for ${track.name}. Assign the required sample.`);samplerLoop(buffers.get(settings.sampleAssetId),settings);}}
}
