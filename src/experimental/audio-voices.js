...
export function validateTrackSources(track,buffers){
 ...
 if(track.kind==='midi'&&track.instrument==='sampler'&&track.regions.some(r=>r.notes.some(n=>!n.mute&&n.velocity>0))){if(!buffers.has(track.sampleAssetId))throw Error(`Missing sampler source for ${track.name}. Assign an audio sample.`);samplerLoop(buffers.get(track.sampleAssetId),track);}
}
