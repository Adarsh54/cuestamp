  ...
  for(const measure of measures){
   ...
   for(const item of measure.children){
    if(item.localName==='attributes'){
     ...
     for(const [attribute,parameter] of [['damper-pedal',64],['sostenuto-pedal',66]])if(sound?.hasAttribute(attribute)){if(events.length>=20000)throw Error('Import up to 20,000 pedal changes per part.');events.push({start:at+cursor+offset,parameter,value:musicxmlPedalValue(sound.getAttribute(attribute))});}
