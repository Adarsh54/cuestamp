...
export const audibleAssets=session=>[...new Set(audibleSources(session).flatMap(t=>t.kind==='audio'?t.regions.map(r=>r.assetId):t.kind==='midi'&&t.instrument==='sampler'?t.regions.flatMap(r=>r.notes.filter(n=>!n.mute&&n.velocity>0).flatMap(n=>samplesForNote(t,n).map(s=>s.sampleAssetId))):[]).filter(Boolean))];
