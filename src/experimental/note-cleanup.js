...
export function duplicateNotePlan(region,values={}){
 ...
 for(const n of region.notes){if(!selected.has(n.id))continue;const key=JSON.stringify([n.channel??0,n.pitch,n.start,...(v.match==='identical'?[n.duration,n.velocity,Boolean(n.mute)]:[])]);if(!groups.has(key))groups.set(key,[]);groups.get(key).push(n);}
