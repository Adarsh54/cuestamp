...
export async function startRecording({signal,deviceId,inputChannels='stereo',session=null,buffers=new Map(),position=0,onProgress=()=>{},onLimit=()=>{}}={}){
 ...
 try{
  ...
  const timing=recordingTiming(session,context.currentTime,context.sampleRate,position),startFrame=timing.startFrame+Math.round((window.start-position)*context.sampleRate),captureTime=startFrame/context.sampleRate,endFrame=startFrame+(cycleFrames?.maxFrames??Math.max(1,Math.round(window.duration*context.sampleRate)));
