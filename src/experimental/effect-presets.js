...
const cloneEffects=effects=>effects.map(effect=>({...structuredClone(effect),...(effect.kind==='gate'?{sidechainTrackId:null}:{}),id:crypto.randomUUID(),automation:effect.automation.map(point=>({...point,id:crypto.randomUUID()}))}));
