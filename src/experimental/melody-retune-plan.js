...
export function melodyRetunePlan(session,analysis,options={}){
 ...
 const plan=audioStretchPlan(session,analysis.regionId,1,options),corrections=[];
...
export function retunedRegionTrack(session,regionId,values,options={}){
 ...
 const {corrections:ignored,...rendered}=v,result=stretchedRegionTrack(session,regionId,{...rendered,ratio:1},options),name=region.name.slice(0,180)+' tuned';result.track.name=name;result.track.regions[0].name=name;return result;
}
