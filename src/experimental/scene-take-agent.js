...
export const sceneTakeActionSchema=z.object({operation:z.enum(['select','rename','discard','place']),takeId:id,name:z.string().trim().min(1).max(100).nullable(),position:z.number().finite().min(0).max(86400).nullable(),overlap:z.enum(['reject','allow']).nullable()}).strict().superRefine((v,ctx)=>{
