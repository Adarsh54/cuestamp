...
export const meterObservationSchema=z.object({sessionId:z.string().min(1).max(100),revision:z.number().int().nonnegative(),measuredAt:z.number().int().nonnegative(),position:z.number().finite().min(0).max(1000000000),mode:z.enum(['linear','cycle']),sampleRate:z.number().int().min(8000).max(192000),windowFrames:z.literal(2048),loudness:liveLoudnessSchema.optional(),compressors:z.array(z.object({id:z.string().min(1).max(100),reductionDb:z.number().finite().min(-1000).max(0)}).strict()).max(2064).optional(),channels:z.array(z.object({id:z.string().min(1).max(100),leftPeakDb:db,rightPeakDb:db,heldPeakDb:db}).strict()).min(1).max(129)}).strict();
...
export function captureMeterObservation(session,values,{position,mode,sampleRate,now=Date.now(),effectValues}){
 ...
 const compressors=mode==='linear'&&effectValues?[...effectValues].filter(([,v])=>Number.isFinite(v.reductionDb)&&v.reductionDb<=0&&v.reductionDb>=-1000).map(([id,v])=>({id,reductionDb:v.reductionDb})):[];
 return validateMeterObservation({...(compressors.length?{compressors}:{}),...(loudness?{loudness}:{}),sessionId:session.id,revision:session.revision,measuredAt:now,position,mode,sampleRate,windowFrames:2048,channels},session,now);
}
