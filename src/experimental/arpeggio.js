...
export function arpeggioPlan(region,tempo,values={}){
 const v=options.parse(values),bpm=typeof tempo==='number'?tempo:tempo?.tempo;if(!Number.isFinite(bpm)||bpm<20||bpm>300)throw Error('Choose a valid project tempo.');
 ...
 const clock=regionBeatTiming(region,tempo),mapped=Boolean(tempo?.tempoChanges?.length),interval=v.rate*60/bpm,notes=[],capacity=20000-(region.notes.length-chosen.length);
 for(const groups of channels.values()){
  ...
  for(let g=0;g<starts.length;g++){
   ...
   const startBeat=clock.beatAtTime(start),span=mapped?clock.beatsInDuration(start,end-start)/v.rate:(end-start)/interval,count=Math.max(1,Math.ceil(span-1e-10));
