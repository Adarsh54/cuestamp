...
export function joinNotesPlan(region,values={}){
 ...
 for(const notes of buckets.values()){
  ...
  for(const n of notes){if(group.length&&(n.start>end+v.gap||articulationIdentity(n.articulation)!==articulationIdentity(group[0].articulation))){flush();group=[];}if(!group.length)end=n.start+n.duration;else end=Math.max(end,n.start+n.duration);group.push(n);}flush();
 }
