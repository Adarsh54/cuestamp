import {samplerZoneBatchView,bindSamplerZoneBatch} from './sampler-zone-batch.js';
...
export function bindSamplerAutomap(root,{track,files,execute,guard,state={}}){
 ...
 if(state.batchZoneIds!==undefined)for(const input of form.querySelectorAll('[name=zoneIds]'))input.checked=state.batchZoneIds.includes(input.value);
 ...
 const preview=()=>{state.batchZoneIds=ids();for(const button of form.querySelectorAll('[data-zone-batch]'))button.disabled=!state.batchZoneIds.length;output.hidden=mode.value!=='filenames';if(output.hidden)return;try{const plan=filenameMapPlan(track,files,options());output.textContent=plan.rows.map(r=>`${r.filename} → MIDI ${r.root}`).join('\n');output.dataset.valid='true';}catch(error){output.textContent=error.message;output.dataset.valid='false';}};
 ...
 bindSamplerZoneBatch(form,{root,track,execute,guard,state,ids});
