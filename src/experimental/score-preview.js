...
export function bindScorePreview(root,{session,track,region,execute,guard,download,getSession}){
 ...
 const draw=async()=>{
  ...
  try{
   ...
   renderer.Zoom=Number(zoom.value);renderer.render();bindScoreNotes(panel,renderer,{session:display.session,track:display.track,region,scope:selectedScope,execute,guard,selectionRoot:root,getSession});indexScorePosition(root,panel);scroll.scrollTop=view.top;scroll.scrollLeft=view.left;status.textContent='Score preview ready.';
