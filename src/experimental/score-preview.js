...
import {scoreDisplayTiming,scoreDisplayTimingView} from './score-display-timing.js';
...
export function scorePreviewView(track,region,session){
 ...
 return `<details data-score-preview tabindex="-1"><summary>Score preview</summary><div class="button-row"><label>Show<select data-score-scope><option value="region" ${!hasRegion?'disabled':''}>Selected MIDI region</option><option value="arrangement" ${!hasRegion?'selected':''}>Arrangement parts</option></select></label>${hasRegion?`<label>Selected track notation<select data-score-instrument>${Object.entries(scoreInstruments).map(([id,item])=>`<option value="${id}" ${id===(track.scoreInstrument??'concert')?'selected':''}>${item.label}</option>`).join('')}</select></label><label>Selected track clef<select data-score-clef>${["treble","bass","alto","tenor"].map(c=>`<option value="${c}" ${c===(track.scoreClef??"treble")?"selected":""}>${c[0].toUpperCase()+c.slice(1)}</option>`).join('')}</select></label>`:''}<label>Pitch display<select data-score-pitch-view><option value="written">Written pitch</option><option value="concert">Concert pitch</option></select></label>${scoreDisplayTimingView()}<label>Score zoom<select data-score-zoom><option value="0.75">75%</option><option value="1" selected>100%</option><option value="1.25">125%</option><option value="1.5">150%</option></select></label><button type="button" data-score-download>Download displayed score</button><label><input type="checkbox" data-score-audition> Audition notes</label><output data-score-audition-status aria-live="polite"></output><label>Print paper<select data-score-paper><option>A4</option><option>Letter</option></select></label><button type="button" data-score-print>Print score</button></div><p class="muted">Click a note to edit it or a rest inside a MIDI region to add a note. Shift-click or Shift+Enter toggles notes within one region for duplicate/delete actions and agent requests. Selecting another region starts a new selection. With the score focused, ⌘/Ctrl C copies, X cuts and V pastes using the destination and beat in the paste panel. Text fields keep their normal clipboard shortcuts. Drag vertically or use ↑/↓ to move by staff steps in the project key; Shift+↑/↓ moves an octave. Escape cancels a drag. Delete removes the selection when the focused note belongs to it. Notation instrument changes written pitches only; note fields show sounding MIDI pitches. Display grid simplifies note starts and lengths for notation and Download displayed score only; it does not alter playback. Note fields and edits still use the original performance. Audition notes plays up to half a second through the current instrument and mixer, with tails capped at four seconds; muted notes stay silent. Blue highlights mark the transport position. Export MusicXML to continue in notation software. Arrangement view shows selected pitched MIDI tracks, including muted tracks and regions. Percussion and audio tracks are excluded.</p>${session?scorePartsView(session):''}${scoreNoteEditorView()}<output data-score-status aria-live="polite"></output><div class="daw-score-scroll"><div data-score-sheet role="group" aria-label="Score"></div></div></details>`;
}
const scoreViews=new WeakMap();
export function bindScorePreview(root,{session,track,region,execute,guard,download,getSession,scoreShortcutsBlocked,auditionScoreNote,auditionScoreSelection,stopScoreAudition}){
 ...
 let view=scoreViews.get(root);if(!view||view.sessionId!==session.id){view={sessionId:session.id,scope:scope.value,zoom:zoom.value,pitchMode:'written',displayGrid:'off',paper:'A4',audition:false,partIds:null,top:0,left:0};scoreViews.set(root,view);}
 if([...scope.options].some(o=>o.value===view.scope&&!o.disabled))scope.value=view.scope;zoom.value=view.zoom;pitchView.value=view.pitchMode??'written';displayGrid.value=view.displayGrid??'off';const paper=panel.querySelector('[data-score-paper]');paper.value=view.paper??'A4';paper.onchange=()=>{view.paper=paper.value;};const audition=panel.querySelector('[data-score-audition]');audition.checked=Boolean(view.audition);audition.onchange=()=>{view.audition=audition.checked;if(!view.audition){stopScoreAudition?.();panel.querySelector('[data-score-audition-status]').textContent='';}};
 ...
 const draw=async()=>{
  ...
  const request=++generation,selectedScope=scope.value,pitchMode=pitchView.value,cacheKey=selectedScope+':'+pitchMode+':'+JSON.stringify(view.partIds)+':'+displayGrid.value;status.textContent='Preparing score…';
  try{
   const sourceSession=displayedSession(),display=scorePitchView(sourceSession,track,pitchMode),notationSession=scoreDisplayTiming(display.session,displayGrid.value),notationTrack=notationSession.tracks.find(t=>t.id===track?.id),notationRegion=notationTrack?.regions.find(r=>r.id===region?.id);
   if(!renderer||loadedScope!==cacheKey){
    const document=scorePreviewDocument(notationSession,notationTrack,notationRegion,selectedScope,'written'),module=await import('opensheetmusicdisplay');
   ...
   renderer.Zoom=Number(zoom.value);renderer.render();bindScoreNotes(panel,renderer,{session:display.session,track:display.track,region,notationSession,notationTrack,notationRegion,scope:selectedScope,execute,guard,selectionRoot:root,getSession,scoreShortcutsBlocked,auditionSelection:auditionScoreSelection,auditionNote:reference=>{if(view.audition)void auditionScoreNote?.(reference);}});indexScorePosition(root,panel);scroll.scrollTop=view.top;scroll.scrollLeft=view.left;status.textContent='Score preview ready.';
 ...
 if(partsPanel)partsPanel.hidden=scope.value!=='arrangement';
 displayGrid.onchange=()=>{view.displayGrid=displayGrid.value;void draw();};
 pitchView.onchange=()=>{view.pitchMode=pitchView.value;void draw();};
 panel.querySelector('[data-score-download]').onclick=guard(()=>{const display=scorePitchView(displayedSession(),track,pitchView.value),notationSession=scoreDisplayTiming(display.session,displayGrid.value),notationTrack=notationSession.tracks.find(t=>t.id===track?.id),notationRegion=notationTrack?.regions.find(r=>r.id===region?.id),xml=scope.value==='arrangement'?exportScoreMusicxml(notationSession):exportRegionMusicxml(notationSession,notationTrack,notationRegion);download(new Blob([xml],{type:'application/vnd.recordare.musicxml+xml'}),`${scope.value==='arrangement'?session.title:region.name||'Score'}-${pitchView.value}.musicxml`);});
 panel.querySelector('[data-score-print]').onclick=guard(async()=>{const display=scorePitchView(displayedSession(),track,pitchView.value),notation=scoreDisplayTiming(display.session,displayGrid.value),part=notation.tracks.find(t=>t.id===track?.id),clip=part?.regions.find(r=>r.id===region?.id),xml=scope.value==='arrangement'?exportScoreMusicxml(notation):exportRegionMusicxml(notation,part,clip);await openScorePrint(root,{xml,title:scope.value==='arrangement'?session.title:region.name,paper:panel.querySelector('[data-score-paper]').value});});
 ...
 panel.addEventListener('toggle',()=>{if(panel.open)void draw();else stopScoreAudition?.();});zoom.onchange=()=>{view.zoom=zoom.value;void draw();};scope.onchange=()=>{view.scope=scope.value;view.top=0;view.left=0;void draw();};if(panel.open)void draw();
}
