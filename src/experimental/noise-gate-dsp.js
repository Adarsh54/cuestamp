...
export class NoiseGate {
 constructor(rate,mode='gate'){this.duck=mode==='duck';this.rate=rate;this.envelope=0;this.gain=null;this.open=false;this.hold=0;this.detectorDecay=Math.exp(-1/(.01*rate));}
 process(input,output,parameters,detector=input){
  ...
  for(let i=0;i<output[0].length;i++){
   ...
   const floor=10**(value('reductionDb',i)/20),target=this.duck?(this.open?floor:1):(this.open?1:floor);
   this.gain??=this.duck?1:floor;
   const time=value((this.duck?target<this.gain:target>this.gain)?'attack':'release',i),coefficient=time===0?0:Math.exp(-1/(time*this.rate));
