...
export async function exportArchive(document,files,takeBundle,{format='cuestamp-daw'}={}){
 if(!['cuestamp-daw','cuestamp-sampler'].includes(format))throw Error('Unsupported archive format.');
 ...
 if(format==='cuestamp-sampler'&&(session.tracks.length||session.samplerPresets.length!==1||sceneTakes.takes.length))throw Error('Sampler archives must contain one preset and no tracks or takes.');
 const manifest=JSON.stringify({format,version:1,session,assets,sceneTakes});if(new TextEncoder().encode(manifest).length>DOCUMENT_LIMIT)throw Error('Project document is too large.');
...
export async function importArchive(data,{format='cuestamp-daw'}={}){
 if(!['cuestamp-daw','cuestamp-sampler'].includes(format))throw Error('Unsupported archive format.');
 ...
 if(value.format!==format||value.version!==1||!Array.isArray(value.assets))throw Error(format==='cuestamp-sampler'?'Choose a Cuestamp sampler instrument archive.':'Unsupported project archive.');
