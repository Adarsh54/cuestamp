...
export function parseTimecode(text,rate=24,dropFrame=false){
 ...
 const [h,m,s,f]=text.split(/[:;]/).map(Number),nominal=Math.round(rate);if(h>23||m>59||s>59||f>=nominal)throw Error('Timecode is outside the selected frame rate.');
