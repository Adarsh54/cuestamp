...
export const mixAnalysisSchema=z.object({sessionId:z.string().min(1).max(100),revision:z.number().int().nonnegative(),measuredAt:z.number().int().nonnegative(),sampleRate:z.union([z.literal(44100),z.literal(48000),z.literal(96000)]),frames:z.number().int().positive().max(57600000),channels:z.array(channel).length(2),stereo:stereo.optional(),truePeak:z.object({oversample:z.literal(4),peaksDbtp:z.array(db).length(2)}).strict().optional(),loudness:z.object({integratedLufs:db,blocks:z.number().int().nonnegative().max(6000),gatedBlocks:z.number().int().nonnegative().max(6000)}).strict().optional()}).strict();
