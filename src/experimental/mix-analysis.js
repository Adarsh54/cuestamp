...
const stereo=z.object({correlation:z.number().finite().min(-1).max(1).nullable(),midRmsDb:db,sideRmsDb:db}).strict();
...
export const mixAnalysisSchema=z.object({sessionId:z.string().min(1).max(100),revision:z.number().int().nonnegative(),measuredAt:z.number().int().nonnegative(),sampleRate:z.union([z.literal(44100),z.literal(48000),z.literal(96000)]),frames:z.number().int().positive().max(57600000),channels:z.array(channel).length(2),stereo:stereo.optional(),truePeak:z.object({oversample:z.literal(4),peaksDbtp:z.array(db).length(2)}).strict().optional(),loudness:z.object({integratedLufs:db,dynamics:z.object({momentaryMaxLufs:db,shortTermMaxLufs:db,rangeLu:z.number().finite().min(0).max(2000).nullable()}).strict().optional(),blocks:z.number().int().nonnegative().max(6000),gatedBlocks:z.number().int().nonnegative().max(6000)}).strict().optional()}).strict();
export function validateMixAnalysis(value,session,now=Date.now()){
 ...
 if(result.stereo){const power=db=>db===null?0:10**(db/10),[left,right]=result.channels.map(c=>power(c.rmsDb)),mid=power(result.stereo.midRmsDb),side=power(result.stereo.sideRmsDb),total=(left+right)/2;
  if(Math.abs(mid+side-total)>Math.max(total,1e-100)*1e-6||((left===0||right===0)!==(result.stereo.correlation===null)))throw Error('Invalid stereo analysis.');
  if(left&&right&&Math.abs((mid-side)-result.stereo.correlation*Math.sqrt(left*right))>Math.max(total,1e-100)*1e-6)throw Error('Inconsistent stereo correlation.');
 }
 if(result.truePeak){for(let i=0;i<2;i++){const measured=result.truePeak.peaksDbtp[i],sample=result.channels[i].peakDb;if((measured===null)!==(sample===null)||(measured!==null&&measured<sample-1e-9))throw Error('Invalid true-peak analysis.');}}
 if(result.loudness?.dynamics){const d=result.loudness.dynamics;if((result.loudness.integratedLufs!==null&&(d.momentaryMaxLufs===null||d.momentaryMaxLufs<result.loudness.integratedLufs-1e-6))||(result.frames<Math.round(.4*result.sampleRate)&&d.momentaryMaxLufs!==null)||(result.frames<3*result.sampleRate&&(d.shortTermMaxLufs!==null||d.rangeLu!==null))||(d.rangeLu!==null&&d.shortTermMaxLufs===null))throw Error('Invalid loudness dynamics.');}
 if(result.loudness){const l=result.loudness,block=Math.round(.4*result.sampleRate),expected=result.frames<block?0:Math.floor((result.frames-block)/(block/4))+1;if(l.blocks!==expected||l.gatedBlocks>l.blocks||(l.integratedLufs===null)!==(l.gatedBlocks===0)||(l.integratedLufs!==null&&l.integratedLufs<=-70))throw Error('Invalid loudness analysis.');}
 return result;
}
...
export function mixAnalysisView(value,session,busy,targetPeakDb=-1,loudnessSettings={target:-14,ceiling:-1}){
 ...
 return `<section class="daw-mix-analysis"><p data-live-loudness>Live loudness · Play to measure</p><small>Current playback, including cycle repeats. Linear playback excludes the click; cycle playback includes it. Readings reset when playback restarts.</small><div class="section-title"><h3>Mix analysis</h3><button data-analyze-mix ${busy?'disabled':''}>${value?'Analyze again':'Analyze mix'}</button></div>${value?`<p class="muted">${fresh?'Full stereo render':'Out of date — analyze again after edits'} · ${(value.frames/value.sampleRate).toFixed(2)} s · ${value.sampleRate/1000} kHz</p><div class="daw-analysis-values"><div><span>Sample peak</span><strong>${format(Number.isFinite(peak)?peak:null)}</strong></div><div><span>Average level · RMS</span><strong>${format(Number.isFinite(rms)?rms:null)}</strong></div><div><span>Samples over 0 dBFS</span><strong>${value.channels.reduce((sum,c)=>sum+c.overSamples,0)}</strong></div></div><div class="daw-analysis-channels">${value.channels.map((c,i)=>`<p>${i?'Right':'Left'} · Peak ${format(c.peakDb)} · RMS ${format(c.rmsDb)}</p>`).join('')}</div>${value.loudness?`<div class="daw-analysis-values"><div><span>Integrated loudness</span><strong>${value.loudness.integratedLufs===null?'Not measurable':value.loudness.integratedLufs.toFixed(2)+' LUFS'}</strong></div></div><p class="muted">Gated loudness of the full stereo mix. Requires at least 400 ms above the measurement gate.</p>`:''}${loudnessDynamicsView(value.loudness?.dynamics)}${truePeakView(value.truePeak)}${stereoAnalysisView(value.stereo)}<button data-analysis-peak ${!fresh||!Number.isFinite(peak)?'disabled':''}>Go to loudest sample</button>${peakNormalizationView(session,value,busy,targetPeakDb)}${loudnessNormalizationView(session,value,busy,loudnessSettings)}`:'<p class="muted">Measure the full mix, including effects and automation, before exporting.</p>'}<small>Measures every rendered sample. RMS is average signal level, not LUFS. Sample peaks do not measure inter-sample true peaks. Analysis respects mute and solo and excludes the metronome.</small></section>`;
}
...
function normalizationPreview(session,analysis,target){try{const plan=peakNormalizationPlan(session,analysis,target),changed=Math.abs(plan.deltaDb)>=.001;return {plan,changed,text:changed?`${plan.deltaDb>=0?'+':''}${plan.deltaDb.toFixed(2)} dB on master volume${session.masterAutomationMode==='off'||(session.masterAutomationMuted||[]).includes('gainDb')?'':' and its active automation'}. The mix will be analyzed again afterward.`:'The measured sample peak already matches this target.'};}catch(error){return {changed:false,text:error.message};}}
...
function truePeakView(value){if(!value)return '';const format=db=>db===null?'Silence':db.toFixed(2)+' dBTP';return `<div class="daw-analysis-values"><div><span>Estimated true peak · Left</span><strong>${format(value.peaksDbtp[0])}</strong></div><div><span>Estimated true peak · Right</span><strong>${format(value.peaksDbtp[1])}</strong></div></div><p class="muted">4× oversampling estimates peaks between samples. This measurement does not apply a limiter.</p>`;}
