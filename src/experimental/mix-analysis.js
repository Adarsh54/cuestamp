...
function normalizationPreview(session,analysis,target){try{const plan=peakNormalizationPlan(session,analysis,target),changed=Math.abs(plan.deltaDb)>=.001;return {plan,changed,text:changed?`${plan.deltaDb>=0?'+':''}${plan.deltaDb.toFixed(2)} dB on master volume${session.masterAutomationMode==='off'?'':' and its active automation'}. The mix will be analyzed again afterward.`:'The measured sample peak already matches this target.'};}catch(error){return {changed:false,text:error.message};}}
