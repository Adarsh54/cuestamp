import {tempoFitView,bindTempoFit} from './tempo-fit.js';
import {tempoConstantView,bindTempoConstant} from './tempo-constant.js';
import {tempoScaleView,bindTempoScale} from './tempo-scale.js';
import {tempoRampView,bindTempoRamp} from './tempo-ramp.js';
...
export function tempoView(session){
 ...
 return `<details class="daw-bounce-settings" data-tempo-panel><summary>Tempo map</summary><p class="muted">BPM holds until the next change. Beat positions count quarter notes from 1. MIDI follows the tempo unless its Follow tempo changes setting is off; audio, video, automation and markers keep their time positions.</p><div>${points.map(p=>`<form class="daw-cycle" data-tempo-point="${p.id}"><label>Beat<input name="beat" aria-label="Tempo change beat" type="number" min="1.000001" step="any" required value="${p.beat+1}"></label><label>BPM<input name="bpm" aria-label="Tempo change BPM" type="number" min="20" max="300" step="any" required value="${p.bpm}"></label><span>${p.time.toFixed(2)} s</span><button type="submit">Update</button><button type="button" data-tempo-delete="${p.id}" aria-label="Remove tempo change at beat ${p.beat+1}">Remove</button></form>`).join('')||'<p class="muted">One tempo throughout. Change the starting BPM in the toolbar.</p>'}</div><form class="daw-cycle" data-tempo-add><label>Beat<input name="beat" aria-label="New tempo change beat" type="number" min="1.000001" step="any" required value="${(points.at(-1)?.beat??0)+session.meter*4/(session.meterDenominator??4)+1}"></label><label>BPM<input name="bpm" aria-label="New tempo change BPM" type="number" min="20" max="300" step="any" required value="${points.at(-1)?.bpm??session.tempo}"></label><button type="submit">Add tempo change</button></form>${tempoRampView(session)}${tempoScaleView()}${tempoConstantView()}${tempoFitView()}</details>`;
}
export function bindTempo(root,{session,execute,guard}){
 bindTempoFit(root,{session,execute,guard});
 bindTempoConstant(root,{session,execute,guard});
 bindTempoScale(root,{session,execute,guard});
 bindTempoRamp(root,{session,execute,guard});
