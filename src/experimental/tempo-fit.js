...
export function bindTempoFit(root,{session,execute,guard}){
 ...
 let previousFormat=f.targetFormat.value;const exact=()=>f.targetExact.value===''?null:Number(f.targetExact.value);
 const read=k=>f[k].value.trim()?Number(f[k].value):NaN,endpoint=()=>{const marker=session.markers.find(m=>m.id===f.marker.value);if(marker)return {targetTime:marker.time};if(exact()!==null)return {targetTime:exact()};return f.targetFormat.value==='timecode'?{targetTimecode:f.targetTime.value.trim()}:{targetTime:read('targetTime')};},values=()=>({startBeat:read('startBeat')-1,endBeat:read('endBeat')-1,...endpoint()});
 ...
 f.targetFormat.onchange=()=>{try{const time=exact()??(previousFormat==='timecode'?parseSessionTimecode(f.targetTime.value.trim(),session):read('targetTime'));if(!Number.isFinite(time)||time<0||time>86400)throw Error('Enter a valid endpoint before changing format.');f.targetExact.value=String(time);f.targetTime.value=f.targetFormat.value==='timecode'?formatSessionTimecode(time,session):String(time);previousFormat=f.targetFormat.value;update();}catch(error){f.targetFormat.value=previousFormat;output.textContent=error.message;}};f.marker.onchange=()=>{const marker=session.markers.find(m=>m.id===f.marker.value);if(marker){f.targetExact.value=String(marker.time);f.targetTime.value=f.targetFormat.value==='timecode'?formatSessionTimecode(marker.time,session):String(marker.time);}update();};f.targetTime.oninput=()=>{f.marker.value='';f.targetExact.value='';};form.oninput=update;
 form.addEventListener('daw-drafts-restored',()=>{previousFormat=f.targetFormat.value;update();});
