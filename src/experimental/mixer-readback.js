...
export function updateMixerReadback(root,session,time,liveValue=()=>undefined){
 ...
 for(const input of root.querySelectorAll('[data-mix-gain],[data-mix-pan],[data-master-gain],[data-master-pan],[data-send-gain]')){
  ...
  const parameter=('mixGain' in input.dataset||'masterGain' in input.dataset||'sendGain' in input.dataset)?'gainDb':'pan',source=input.dataset.sendSource,busId=input.dataset.sendGain,target=source?sendAutomationTarget(source,busId):input.dataset.mixGain||input.dataset.mixPan||session.id;
