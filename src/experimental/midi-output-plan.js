...
export function midiOutputPlan(session,trackId,start=0){
 if(!Number.isFinite(start)||start<0||start>86400)throw Error('Choose a valid playback position.');
 const track=session.tracks.find(t=>t.id===trackId&&t.kind==='midi');if(!track)throw Error('Choose an instrument track.');if(track.mute)throw Error('Unmute this track before playing it on the device.');
 const messages=[],spans=new Map();let end=start;
 const add=(time,bytes,priority)=>{if(messages.length>=200000)throw Error('This track has too many MIDI messages for one playback.');messages.push({time,bytes,priority});};
 for(const region of track.regions.filter(r=>!r.mute&&r.start+r.duration>start)){
  ...
  for(const channel of channels){const prior=spans.get(channel)||[];if(prior.some(([a,b])=>at<b&&limit>a))throw Error('Overlapping regions share a MIDI channel. Assign separate channels before hardware playback.');prior.push([at,limit]);spans.set(channel,prior);
 ...
 messages.sort((a,b)=>a.time-b.time||a.priority-b.priority);return {trackId,start,end,messages};
}
