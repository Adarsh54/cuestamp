...
const velocityGainSchema=z.number().finite().min(0).max(1).default(1);
export function sampleZoneLevel(settings={}){return velocityGainSchema.parse(settings.sampleZoneVelocityGain)*10**((zoneVoiceSchema.shape.gainDb.parse(settings.sampleZoneGainDb)+zoneVoiceSchema.shape.gainDb.parse(settings.sampleGroupGainDb))/20);}
