...
export function createMidiInput({beforeRecord,onTake,onChange,clock=()=>performance.now(),requestAccess=()=>navigator.requestMIDIAccess({sysex:false})}){
 let inputChannel=null,captureChannel=null;
 ...
 function message(event){const status=event.data?.[0];if(captureChannel!==null&&status>=0x80&&status<0xf0&&(status&15)!==captureChannel)return;monitor?.push(event.data);if(capture&&!capture.push(event.data,event.timeStamp))finish('Recording limit reached.');}
 ...
 async function record(){
  ...
  opening=true;captureChannel=inputChannel;const token=++epoch;notice='Opening MIDI input…';refresh();
