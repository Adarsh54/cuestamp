...
Import checks take ownership, unique IDs, selection, and the device's 32-take capacity before storing media or replacing the current project. Existing device takes remain intact. Failed local take storage leaves the imported takes in memory with a warning; the original archive remains the recovery copy. Finish scene recording before importing a project. Plain session JSON exports and cloud saves still exclude this device-local take library.
