...
This follows the stuck-note recovery workflow in [Apple’s MIDI monitoring/reset guide](https://support.apple.com/en-ca/guide/logicpro/lgcpc412346d/mac). It is a manual hardware control; agent device selection and panic commands remain unfinished. Tests cover all pitches/channels, cancellation, failed opens, playback cancellation, and browser controls with a simulated device. Physical-device behavior is not yet verified.
