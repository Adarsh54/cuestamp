...
test('MIDI input channel filters recording and monitoring before capture and keeps event channels',async()=>{
 ...
 try{
  await controls.get('[data-midi-connect]')();controls.get('[data-midi-channel]')({target:{value:'2'}});controls.get('[data-midi-transpose]')({target:{value:'12'}});controls.get('[data-midi-velocity-mode]')({target:{value:'fixed'}});controls.get('[data-midi-velocity-value]')({target:{value:'80'}});await controls.get('[data-midi-record]')();assert.match(controller.view(x=>x),/data-midi-channel disabled/);
  // Programmatic changes while armed cannot switch away from held notes.
  controls.get('[data-midi-channel]')({target:{value:'0'}});controls.get('[data-midi-transpose]')({target:{value:'-12'}});controls.get('[data-midi-velocity-value]')({target:{value:'30'}});
  ...
  for(const data of [[0x80,60,0],[0x82,64,0]])handlers.get('midimessage')({data,timeStamp:time});controls.get('[data-midi-finish]')();assert.equal(take.notes.length,1);assert.equal(take.notes[0].pitch,76);assert.equal(take.notes[0].velocity,80/127);assert.equal(take.notes[0].channel,2);assert.equal(take.events.length,2);assert.ok(monitored.every(data=>(data[0]&15)===2));
