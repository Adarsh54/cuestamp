...
test('agent marker navigation uses stable IDs and skips coincident positions',async()=>{const s={...session,markers:[{id:'end',name:'End',time:20},{id:'start',name:'Start',time:2},{id:'same',name:'Same',time:2}]},state={...transport,position:2};const run=(args,extra={})=>planDawEdit({session:s,instruction:'Go to cue',allowTransport:true,transport:state,...extra},adapter([output(args,'navigate_markers')]));assert.equal((await run({operation:'next',markerId:null})).transport.position,20);assert.equal((await run({operation:'jump',markerId:'same'})).transport.position,2);assert.equal((await run({operation:'previous',markerId:null},{transport:{...state,position:20}})).transport.position,2);for(const args of [{operation:'previous',markerId:null},{operation:'jump',markerId:'missing'},{operation:'next',markerId:'end'},{operation:'jump',markerId:null}])await assert.rejects(run(args));await assert.rejects(run({operation:'next',markerId:null},{allowTransport:false}),/Unexpected/);});
