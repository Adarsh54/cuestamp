...
test('workspace shortcuts map transport/editing keys while preserving browser modifier combinations',()=>{
 assert.equal(key(' '),'play');assert.equal(key('Home'),'stop');assert.equal(key('m'),'marker');assert.equal(key('S'),'split');assert.equal(key('d'),'duplicate');assert.equal(key('Delete'),'delete');assert.equal(key('Backspace'),'delete');
