...
(async()=>{
 ...
 AudioBufferSourceNode.prototype.stop=function(...args){if(!args.length||args[0]<=this.context.currentTime)window.activeClicks.delete(this);return stopBuffer.apply(this,args);};
  ...
  assert.deepEqual(await page.evaluate(()=>window.midiOptions),{sysex:false});await page.locator('[data-midi-channel]').selectOption({value:'2'});
  ...
  let saved=await session();assert.equal(saved.tracks.length,1);assert.equal(saved.tracks[0].regions[0].notes.length,1);await page.locator('[data-midi-channel]').selectOption({value:''});assert.equal(saved.tracks[0].regions[0].notes[0].channel,2);assert.equal(saved.tracks[0].regions[0].events.length,2);
