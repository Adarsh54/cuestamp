...
(async()=>{const browser=await chromium.launch({executablePath:process.env.PLAYWRIGHT_EXECUTABLE,headless:true});try{
 ...
 // Live Trim controls display offsets and commit the existing curve plus those offsets.
 await page.locator('[data-touch-mode]').selectOption('trimTouch');assert.equal(await page.locator('[data-mix-gain="t"]').inputValue(),'0');const beforeLiveTrim=await session();await input(22.05,3);assert.deepEqual((await session()).tracks,beforeLiveTrim.tracks);assert.equal(await page.locator('[data-mix-gain="t"]').inputValue(),'0');
 await page.locator('[data-action=play]').click();await page.getByRole('button',{name:'Pause',exact:true}).waitFor();await input(23,3);await input(24,6);await page.evaluate(()=>{window.testAudioTime=25;document.querySelector('[data-mix-gain="t"]').dispatchEvent(new Event('change',{bubbles:true}));});assert.equal(await page.locator('[data-mix-gain="t"]').inputValue(),'0');
 const delta=await page.evaluate(async original=>{const {curveAutomationValue}=await import('/src/experimental/automation-curves.js');const current=JSON.parse(localStorage.getItem('cuestamp-experimental:touch-test'));return curveAutomationValue(current.tracks[0].automation,'gainDb',2,0)-curveAutomationValue(original.tracks[0].automation,'gainDb',2,0);},beforeLiveTrim);assert.ok(Math.abs(delta-6)<1e-8);await page.locator('[data-action=stop]').click();await page.locator('[data-action=undo]').click();assert.deepEqual((await session()).tracks,beforeLiveTrim.tracks);
 await page.locator('[data-touch-mode]').selectOption('trimLatch');await page.locator('[data-action=play]').click();await page.getByRole('button',{name:'Pause',exact:true}).waitFor();await input(26,3);await page.locator('[data-mix-gain="t"]').dispatchEvent('change');assert.equal(await page.locator('[data-mix-gain="t"]').inputValue(),'3');await page.getByText('1 automation lane recording',{exact:true}).waitFor();await page.evaluate(()=>window.testAudioTime=28);await page.locator('[data-action=stop]').click();assert.equal(await page.locator('[data-mix-gain="t"]').inputValue(),'0');assert.equal((await session()).revision>beforeLiveTrim.revision,true);await page.locator('[data-action=undo]').click();assert.deepEqual((await session()).tracks,beforeLiveTrim.tracks);
 ...
 const sendRatios=await page.evaluate(async()=>{
  const {scheduleSession}=await import('/src/experimental/audio-engine.js'),{newSession,SessionHistory}=await import('/src/experimental/session.js'),{sendAutomationTarget}=await import('/src/experimental/automation-recording-lane.js');const h=new SessionHistory(newSession());
  h.execute([{op:'track.add',values:{id:'source',kind:'audio'}},{op:'track.add',values:{id:'dry',kind:'bus'}},{op:'track.add',values:{id:'wet',kind:'bus'}},{op:'track.set',target:'source',values:{output:'dry'}},{op:'track.set',target:'dry',values:{mute:true}},{op:'send.set',target:'source',values:{busId:'wet',gainDb:-12}},{op:'region.add',target:'source',values:{assetId:'signal',start:0,duration:1}}]);
  const ctx=new OfflineAudioContext(2,48000,48000),buffer=ctx.createBuffer(1,48000,48000);buffer.getChannelData(0).fill(.2);const player=scheduleSession(ctx,h.session,new Map([['signal',buffer]]),0,{baseTime:0}),id=sendAutomationTarget('source','wet'),first=ctx.suspend(.25),rendering=ctx.startRendering();await first;player.automation.set(id,'gainDb',-6);const second=ctx.suspend(.5);await ctx.resume();await second;player.automation.cancel(id,'gainDb');await ctx.resume();const result=await rendering,data=result.getChannelData(0);return [data[19200]/data[4800],data[38400]/data[4800]];
 });assert.ok(Math.abs(sendRatios[0]-10**(6/20))<.001);assert.ok(Math.abs(sendRatios[1]-1)<.001);
 console.log('PASS live Trim Touch/Latch UI, range Trim UI/undo; Write start/stop/undo/discard; automated fader playback/seek/drag protection; Touch/Latch UI release, held readback, multi-lane stop/undo/discard, Escape; actual offline audio override/return and changing/live Trim/send renders. Physical/realtime audio not tested.');
