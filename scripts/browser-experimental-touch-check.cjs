...
(async()=>{const browser=await chromium.launch({executablePath:process.env.PLAYWRIGHT_EXECUTABLE,headless:true});try{
 ...
 console.log('PASS live Trim Touch/Latch UI, range Trim UI/undo; Write start/stop/undo/discard; automated fader playback/seek/drag protection; Touch/Latch UI release, held readback, multi-lane stop/undo/discard, Escape; actual offline audio override/return and changing/live Trim renders. Physical/realtime audio not tested.');
