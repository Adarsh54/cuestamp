...
(async()=>{
 ...
 try{
  ...
  const rendered=await page.evaluate(async()=>{
   ...
   const sr=48000;
   const run=async(values={},position=0,duration=1,tone=220,timing={})=>{
    ...
    connectEffects(context,source,[effect],nodes,{position,base:0,...timing,register:(e,key)=>bindings.push(key)}).connect(context.destination);
   ...
   const timing={tempo:120,tempoChanges:[{beat:2,bpm:90},{beat:3,bpm:180}]};
   const retained=[{id:'ignored',parameter:'rate',time:0,value:7}],mapped=[{id:'a',parameter:'rate',time:0,value:2,shape:'hold'},{id:'b',parameter:'rate',time:1,value:1.5,shape:'hold'},{id:'c',parameter:'rate',time:5/3,value:3,shape:'hold'}];
   const synced=await run({sync:true,beats:1,automation:retained},0,3,220,timing),reference=await run({automation:mapped},0,3),syncedSeek=await run({sync:true,beats:1,automation:retained},1.25,1.75,220,timing);
   let syncError=0,syncSeekError=0;for(let i=0;i<3*sr;i++)for(let c=0;c<2;c++)syncError=Math.max(syncError,Math.abs(synced.pcm[c][i]-reference.pcm[c][i]));
   for(let i=12000;i<1.75*sr;i++)for(let c=0;c<2;c++)syncSeekError=Math.max(syncSeekError,Math.abs(synced.pcm[c][i+1.25*sr]-syncedSeek.pcm[c][i]));
   return {dryError,wetRatio,notchRatio,stereo,seekError,syncError,syncSeekError,syncRateBindings:synced.bindings.filter(k=>k==='rate').length,bindings:dry.bindings};
  });
  ...
  assert.ok(rendered.syncError<1e-6,JSON.stringify(rendered));assert.ok(rendered.syncSeekError<.005,JSON.stringify(rendered));assert.equal(rendered.syncRateBindings,0);
