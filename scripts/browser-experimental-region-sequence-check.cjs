...
(async()=>{
 ...
 try{
  ...
  const before=await read(),form=page.locator('[data-region-sequence]');await form.locator('[name=gap]').fill('0.5');await form.getByRole('button',{name:'Sequence regions',exact:true}).click();assert.deepEqual((await read()).tracks.map(t=>t.regions[0].start),[0,4.5]);await page.getByRole('button',{name:'Undo',exact:true}).click();assert.deepEqual((await read()).tracks,before.tracks);
  await page.locator('[data-ruler-mode]').selectOption('seconds');await page.getByRole('textbox',{name:'Go to position',exact:true}).fill('10');await page.getByRole('button',{name:'Go to position',exact:true}).click();await form.locator('[name=anchor]').selectOption('playhead');await form.locator('[name=order]').selectOption('selection');await form.getByRole('button',{name:'Sequence regions',exact:true}).click();assert.deepEqual((await read()).tracks.map(t=>t.regions[0].start),[10,14]);
  assert.deepEqual(errors,[]);console.log('PASS sequence controls, gap, playhead anchor, selection order, cross-track layout and undo.');
