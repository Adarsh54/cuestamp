...
(async()=>{
 ...
 try{
  ...
  await page.locator('#daw-json').fill(JSON.stringify([{op:'track.add',values:{id:'t',kind:'midi',instrument:'sine'}},{op:'region.add',target:'t',values:{id:'r',duration:4}},...[60,64,67].map((pitch,i)=>({op:'note.add',target:'r',values:{id:'n'+i,pitch,start:0,duration:2,velocity:.7,channel:0}})),{op:'midiSwitch.save',target:'t',values:{id:'p',name:'Plucked',type:'keyswitch',channel:0,parameter:24,value:100}},{op:'notes.articulation',target:'r',values:{id:'p',noteIds:'n0,n1'}}]));
  ...
  await page.locator('.daw-arpeggio').screenshot({path:'/tmp/cuestamp-arpeggio.png'});const original=await read();await apply.click();let current=await read();assert.ok(current.tracks[0].regions[0].notes.every(n=>(n.articulation?.id??null)===(n.pitch===67?null:'p')));assert.deepEqual(current.tracks[0].regions[0].notes.map(n=>n.pitch),[60,64,67,60,64,67,60,64]);
  ...
  await page.getByRole('button',{name:'Undo',exact:true}).click();await page.locator('[data-note=n0]').click();await page.locator('[data-note=n1]').click({modifiers:['Meta']});await form.locator('[name=scope]').selectOption('selected');await form.locator('[name=order]').selectOption('down');await form.locator('[name=octaves]').fill('2');await form.locator('[name=gate]').fill('50');await apply.click();current=await read();assert.deepEqual(current.tracks[0].regions[0].notes[0],original.tracks[0].regions[0].notes[2]);assert.deepEqual(current.tracks[0].regions[0].notes.slice(1,5).map(n=>n.pitch),[76,72,64,60]);assert.ok(current.tracks[0].regions[0].notes.slice(1).every(n=>n.duration===.125&&n.articulation?.id==='p'));
