...
(async()=>{
 ...
 try{
  ...
  await page.locator('#daw-instruction').fill('How much compression is applied right now?');await page.getByRole('button',{name:'Run instruction',exact:true}).click();await page.getByText('Compression reading received.',{exact:true}).first().waitFor();const observed=requests.at(-1).meterObservation;assert.ok(observed.compressors.some(e=>e.reductionDb < -1));assert.equal(observed.compressors[0].id,before.tracks[0].effects[0].id);assert.equal(observed.revision,before.revision);
  ...
  assert.deepEqual(errors,[]);console.log('PASS real-time compressor reduction, validated agent request, unchanged document, stop reset, bypass state and explicit unavailable cycle readout.');
