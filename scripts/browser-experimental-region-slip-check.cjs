...
(async()=>{
 ...
 try{
  ...
  // Every region gesture must cancel without leaving a preview or history entry.
  for(const mode of ['move','trim-end','fadeOut','slip']){
   await clip.scrollIntoViewIfNeeded();const handle=mode==='move'||mode==='slip'?clip:clip.locator(`[data-region-handle=${mode}]`);const rect=await handle.boundingBox(),initial=await read();
   if(mode==='slip')await page.keyboard.down('Alt');await page.keyboard.down('Shift');await page.mouse.move(rect.x+rect.width/2,rect.y+rect.height/2);await page.mouse.down();await page.mouse.move(rect.x+rect.width/2+12,rect.y+rect.height/2,{steps:3});await page.keyboard.press('Escape');await page.mouse.up();await page.keyboard.up('Shift');if(mode==='slip')await page.keyboard.up('Alt');assert.deepEqual(await read(),initial,mode+' cancellation');assert.equal(await clip.evaluate(e=>e.style.left),initial.tracks[0].regions[0].start*32+'px');
  }
