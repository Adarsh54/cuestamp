...
(async()=>{
 ...
 try{
  ...
  // Error rendering replaces form fields, so set the requested values again.
  await form.locator('[name=fadeIn]').fill('1');await form.locator('[name=fadeOut]').fill('2');await form.locator('[name=fit]').selectOption('scale');await form.locator('[name=shape]').selectOption('equalPower');await form.getByRole('button',{name:'Apply audio fades',exact:true}).click();let saved=await read();assert.equal(saved.tracks[0].regions[0].fadeIn,1/3);assert.equal(saved.tracks[0].regions[1].fadeOut,2);assert.equal(saved.tracks[0].regions[1].fadeInShape,'equalPower');assert.deepEqual(saved.tracks[1],before.tracks[1]);await page.getByRole('button',{name:'Undo',exact:true}).click();assert.deepEqual((await read()).tracks,before.tracks);
