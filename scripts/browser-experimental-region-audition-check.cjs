...
(async()=>{
 ...
 try{
  ...
  await page.route('**/api/daw',r=>{if(r.request().method()==='GET')return r.fulfill({json:{configured:true}});const body=r.request().postDataJSON();requests.push(body);return r.fulfill({json:body.instruction==='Describe playback'?{revision:body.session.revision,commands:[],summary:'Observed playback context.'}:{revision:body.session.revision,commands:[],action:'audition_selected_regions',regionIds:body.selectedRegionIds,transportEpoch:body.transport.epoch,summary:'Untrusted model summary'}});});
  ...
  await page.getByRole('button',{name:'Audition selected clips',exact:true}).click();await page.getByRole('button',{name:'Stop audition',exact:true}).waitFor();await page.locator('#daw-instruction').fill('Describe playback');await page.getByRole('button',{name:'Run instruction',exact:true}).click();await page.getByText('Observed playback context.',{exact:true}).waitFor();assert.equal(requests.at(-1).transport.mode,'regionSelection');assert.equal(requests.at(-1).transport.playing,true);await page.getByRole('button',{name:'Stop audition',exact:true}).click();
