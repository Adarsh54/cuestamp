...
(async()=>{
 ...
 try{
  ...
  const before=await read();await page.getByRole('button',{name:'Audition selected clips',exact:true}).click();await page.getByRole('button',{name:'Stop audition',exact:true}).waitFor();await page.getByRole('button',{name:'Pause',exact:true}).waitFor();assert.deepEqual(await read(),before);await page.getByRole('button',{name:'Stop audition',exact:true}).click();await page.getByText('Selected clip audition stopped.',{exact:true}).waitFor();assert.deepEqual(await read(),before);
  ...
  assert.deepEqual(errors,[]);console.log('PASS selected-clips audition, stop feedback, unchanged session and native audio render through bus excluding missing unselected source.');
