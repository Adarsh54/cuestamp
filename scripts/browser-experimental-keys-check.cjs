...
(async()=>{
 ...
 try{
  ...
  await page.locator('#daw-json').fill(JSON.stringify([{op:'track.add',values:{id:'second-part',kind:'midi'}},{op:'region.add',target:'second-part',values:{id:'second-region',duration:4}},{op:'note.add',target:'second-region',values:{pitch:60,start:0,duration:1}}]));await page.locator('[data-action=json]').click();
  await page.getByText('Organize MIDI channels for export',{exact:true}).click();await page.locator('[data-allocate-midi-channels]').click();assert.deepEqual((await session()).tracks.map(t=>t.regions[0].notes[0].channel),[0,1]);assert.equal(await page.locator('[data-allocate-midi-channels]').count(),0);await page.locator('[data-action=undo]').click();assert.deepEqual((await session()).tracks.map(t=>t.regions[0].notes[0].channel),[0,0]);assert.deepEqual(errors,[]);
  console.log('PASS: MIDI key-map import, key-change edit/add/remove, undo, reload persistence and note-onset key-following controls and MIDI channel reassignment/allocation.');
