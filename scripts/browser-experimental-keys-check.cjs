...
(async()=>{
 ...
 try{
  ...
  await page.locator('#daw-json').fill(JSON.stringify([50,50,51,51,50].map((value,i)=>({op:'event.add',target:'follow-region',values:{type:'controlChange',parameter:11,channel:0,value,start:i*.1}}))));await page.locator('[data-action=json]').click();await page.locator('[data-region=follow-region]').click();
  await page.getByText('Thin controller data',{exact:true}).click();const thin=page.locator('[data-controller-thin]');await thin.locator('[name=tolerance]').fill('1');await thin.getByRole('button',{name:'Thin this lane',exact:true}).click();assert.equal((await session()).tracks[0].regions[0].events.length,2);await page.locator('[data-action=undo]').click();assert.equal((await session()).tracks[0].regions[0].events.length,5);assert.deepEqual(errors,[]);
  await page.locator('[data-tempo-panel] > summary').click();await page.locator('[data-tempo-ramp-panel] > summary').click();const ramp=page.locator('[data-tempo-ramp]');await ramp.locator('[name=step]').fill('1');await ramp.locator('[name=toBpm]').fill('60');await ramp.locator('[name=curve]').selectOption('easeOut');assert.ok((await ramp.locator('path').getAttribute('d')).includes(' H'));await ramp.getByRole('button',{name:'Create tempo curve',exact:true}).click();assert.equal((await session()).tempoChanges.length,16);assert.equal((await session()).tempoChanges.at(-1).bpm,60);await page.locator('[data-action=undo]').click();assert.equal((await session()).tempoChanges.length,0);assert.deepEqual(errors,[]);
  await page.locator('[data-tempo-scale-panel] > summary').click();const tempoScale=page.locator('[data-tempo-scale]');await tempoScale.locator('[name=scope]').selectOption('range');await tempoScale.locator('[name=startBeat]').fill('5');await tempoScale.locator('[name=endBeat]').fill('13');await tempoScale.locator('[name=percent]').fill('50');assert.match(await tempoScale.locator('output').textContent(),/4.00 → 8.00/);await tempoScale.getByRole('button',{name:'Scale tempo',exact:true}).click();assert.deepEqual((await session()).tempoChanges.map(p=>[p.beat,p.bpm]),[[4,60],[12,120]]);await page.locator('[data-action=undo]').click();assert.equal((await session()).tempoChanges.length,0);assert.deepEqual(errors,[]);
  console.log('PASS: MIDI key-map import, key-change edit/add/remove, undo, reload persistence and note-onset key-following controls and MIDI channel reassignment/allocation controller thinning tempo curves and tempo scaling.');
