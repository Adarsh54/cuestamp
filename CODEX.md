...
The dedicated agent analysis action is not yet connected; raw `region.commitSilence` commands from the model are blocked so it cannot invent detected ranges. Unit tests verify stereo activity, exact ranges, silence, gap merging, padding, invalid data, reversed offsets and atomic undo. `scripts/browser-experimental-silence-check.cjs` imports a WAV and verifies actual worker analysis, preview invalidation, apply, undo/redo and reload. Reference: [Logic Remove Silence controls](https://support.apple.com/en-ie/guide/logicpro/lgcp86ee7672/10.7/mac/11.0). This implementation does not yet offer zero-crossing adjustment or claim exact Logic detection parity.
