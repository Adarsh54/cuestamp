...
A finished take is stored under the account's local `:scene-performance` key and restored after refresh. The panel offers Save performance (destination start and overlap choice) or Discard performance. Saving creates independent editable regions through the shared placement logic in one undo step; short final fragments are supported. Originals remain. This captures clip timing, not rendered audio, restarted mixer automation or tail cutoffs. Saving requires the same session ID and revision; edits after recording invalidate placement rather than silently using changed source clips. Save/discard a pending take before starting another recording. A take is persisted on stop, so closing the page during active recording still loses that unfinished take.

Shared `scene.performance` values are `performance` (JSON-encoded sessionId/revision/events receipt), `position`, and `overlap` (reject default or allow). Each event has sceneId, start relative to the performance, and duration. It must be the only command in its batch. Server validation enforces ordered, non-overlapping events, current revision, source scenes, track protection/limits and timeline bounds. The agent receives a fresh pending take as recordedScenePerformance and can save it on request; recording itself starts manually. Unit tests cover delayed ticks, pending launches canceled before playback, short clips, atomic late failure, stale revisions and mocked agent saving. Browser checks cover recording, automatic stop, local refresh recovery, saving and undo. All 1,059 tests and build pass.
