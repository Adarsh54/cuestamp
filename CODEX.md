...
### Split at detected attacks

After attack detection, **Split at detected attacks** slices the region at all measured interior attacks. The source file is reused, reversed offsets remain correct, original outer fades are retained (clamped to their piece), and internal boundaries have no added fades. This uses one `region.splitAtTimes` command and one undo step; it does not create a new audio file. Detection settings still control which attacks are included. Listen to the result and use fades where necessary.

The shared command accepts `values.times` as a JSON array of strictly increasing, distinct absolute timeline seconds. All positions must be inside the source region. It validates track capacity (1,000 regions) and saved comp capacity (64 sections) before splitting and remaps comp references to the resulting pieces. Protected tracks reject the edit. Invalid cuts and persistence failures leave the previous document/history intact. Detected selection data is tied to the current session/revision and cannot be applied after intervening edits.

The agent `detect_audio_attacks` tool now requires `operation: preview | split`. Split always performs actual worker analysis first and commits those measured cuts through the same command. No attacks means no edit. It remains a standalone operation, with cancellation/session checks before applying and successful document commits recorded in agent history. Raw `region.splitAtTimes` also supports explicitly supplied cut times; model instructions direct detected-attack requests through the measurement tool rather than guessed times.

Verification covers forward/reversed source coverage, comp remapping/reconstruction, edge fades, undo/redo, invalid/protected/stale requests, more than 100 cuts in one command, and track/comp capacity rejection. The editor browser regression checks manual detection/splitting and mocked-agent detection/splitting with real worker analysis, then restores the original regions through undo. Flex warping and manually editable attack marker sets remain unfinished.

### Saved and corrected attack markers

Attack detection remains a preview until **Save markers**, **Move marker**, **Delete marker**, or **Add attack at cursor** commits the current marker set. The numbered marker form selects a marker and edits its position in seconds relative to the region. Each correction is one undoable document edit. Saved markers reappear after reload when the detailed waveform is loaded and are available for navigation and splitting. Running detection again previews a replacement; saving it replaces that region's saved set.

`region.attackMarkers` accepts `values.times` as a JSON array of sorted distinct region-local seconds strictly inside the region; an empty array clears the saved set. The document stores the optional `attackMarkers` array in original source-file seconds. Display maps those coordinates through the region offset and reverse flag and hides markers outside the current trimmed interval. Moving, splitting, trimming and duplicating preserve their physical source positions. Reversing reflects positions; it does not rerun onset detection. Newly stretched/pitch-rendered assets discard inherited marker coordinates; original source regions retain theirs. Bounce output starts with no saved markers.

Protected tracks reject marker edits. Invalid/duplicate/out-of-range markers are rejected atomically. The agent can use the same shared command for explicitly supplied corrections; its instructions distinguish region-local command times from source-coordinate storage and direct automatic detection to the PCM analysis tool.

Verification: `test/experimental-attack-markers.test.js` covers serialization, forward/reverse mapping, split/move behavior, validation/protection, render invalidation, agent command application and undo. The browser editor check detects/saves, moves, adds and deletes markers, verifies undo/redo, reloads and confirms saved marker restoration. Live model configuration was rechecked: local `OPENAI_API_KEY` and `DAW_AGENT_MODEL` are still absent, so live inference remains unverified. Manual marker sets are now persisted; Flex warping remains unfinished.

### Quantize audio-region timing

Select one or more audio regions and open **Quantize audio timing** in the inspector. Choose a musical grid, strength (0–100%), swing delay (0–75%), and alignment at region starts or the first saved attack. First-attack alignment requires a saved marker in every selected audio region. **Each region independently** moves every anchor toward its closest grid point. **Together, keeping spacing** uses the earliest anchor to calculate one common shift, preserving relative timing among selected audio clips.

`regions.quantizeAudio` is the shared manual/agent command. Values are comma-separated `regionIds`, `grid` in quarter-note beats (0.01–16), `strength` (0–1), `swing` (0–0.75), `anchor: start | firstAttack`, and `mode: individual | group`. Swing delays alternate grid points. Strength interpolation occurs in beats through the complete tempo map, then converts back to seconds. Other track kinds in a mixed selection stay unchanged. Region durations, trims, fades, source files and saved source-coordinate attack markers stay unchanged; this is region movement, not stretching or Flex warping. Gaps and overlapping audio can result; overlapping audio plays together.
...
### Time-warp rendering foundation

`audio-warp-options.js`, `audio-warp.js`, and `renderAudioWarp` in the existing audio worker client implement a piecewise duration warp. An anchor is `{source, target}` in seconds within the supplied decoded audio. Start and end are fixed, so output has exactly the original frame count. Interior anchors are rounded to source samples, must be strictly ordered on both timelines, and number 1–64. Each source interval must be at least 50 ms and each interval's target/source ratio must stay within 0.5–2. Existing mono/stereo, 8–192 kHz, 10-minute and 250 MB source-plus-output limits apply.

Intervals use the existing SoundTouch WSOLA duration processor with neighboring source context. Complementary linear crossfades blend 5 ms on each side of destination anchors; stereo uses the same mapping and weights. An entirely identity map returns exact independent PCM copies. Input buffers remain unchanged; rendering runs in the cancelable module worker. This is lossy signal processing and may alter transients or produce texture artifacts. Anchor frame placement is exact in the render plan; perceived musical attacks may shift due to WSOLA's alignment choices. This is not full Logic Flex parity.

Verification: `test/experimental-audio-warp.test.js` covers endpoint/ratio/order bounds, identity copies, finite output, approximate sine pitch, stereo phase and source preservation. `scripts/browser-experimental-warp-check.cjs` changes a fixture's 440-to-880 Hz transition from 1 s toward 1.25 s in a real worker, samples frequency windows on both sides, verifies exact total length, and tests cancellation. Existing stretch-worker regression also passes.

This commit provides the render path only. Region commit/undo integration, graphical warp controls and the agent warp tool remain to be connected; no user-facing time-warp control is exposed yet. Attack-marker dragging still edits markers only and does not invoke this renderer.

### Time-warp region editor and agent

The audio inspector now exposes **Time warp** with source/destination timing-point rows, Add/Remove controls and **Render warped audio**. Points use seconds relative to the trimmed region in playback direction (including reverse). Start/end stay fixed. Enter at least one changed anchor; source/target points must be ordered, source intervals at least 50 ms, and each interval ratio 0.5–2. Sample-rounded identity edits reject without creating a new asset.

Rendering uses the existing guarded audio-processing transaction: trim/reverse the selected source, run the cancelable warp worker, save a 32-bit float WAV in local media storage, and commit `region.commitWarp` in one undo step. A new track retains source routing/effects/automation, gain and outer fades while the original region is muted and retained. Source attack markers are cleared from the rendered asset. Source offset becomes zero and reverse becomes false because both were rendered. Region duration stays fixed to the decoded source frame count; automation remains at its existing absolute times.

Cancellation and stale-session checks occur before committing. Invalid frame counts/assets/anchors and protected sources reject. Persistence errors roll back session/history and uncommitted new media is removed. Undo restores the original region, and redo reuses the saved render.

The agent tool `warp_audio_region` accepts a nullable selected region ID and explicit source/target anchors through the existing audio-processing capability. It shares the manual render path, validates captured context, runs as a standalone action and cannot execute during an editing continuation. Direct model-generated `region.commitWarp` commands are rejected to prevent fabricated render assets. Live provider inference remains unverified without local model credentials.

Tests: `test/experimental-audio-warp-region.test.js` checks source/processing preservation, frame/anchor/asset validation, undo/redo and agent tool guards. The stretch-editor browser regression now also renders warps manually and through mocked agent responses with real worker audio, verifies original restoration via undo, redoes/reloads, and decodes the stored render. These are numeric anchor controls; graphical warp-handle editing and richer Flex modes remain unfinished. Attack-marker dragging still only changes attack markers.

### Graphical time-warp draft

**Load warp waveform** adds a timing preview to the Time warp panel. The original source waveform is faint; colored waveform columns show the piecewise timing projection. Drag a top handle to change its destination time, then use **Render warped audio** to process the draft. The source point stays fixed. Start/end stay fixed, targets align to samples, and each drag is bounded by both neighboring intervals' 50–200% stretch limits. This is a timing projection, not a pre-render of WSOLA output; the final waveform can differ.

Dragging updates the numeric destination field without changing the project or audio. Escape, pointer cancellation and lost capture restore the pre-drag draft. Numeric source/destination fields and Add/Remove controls remain available. Unfinished rows survive inspector redraws and waveform loading for the active source geometry; source/offset/duration/reverse/session changes invalidate the draft. Drafts are temporary UI state, not saved audio edits or reload-persistent project data.

The graphical preview uses the source's detailed min/max index and respects trim and reverse. Invalid anchor drafts show a validation message. Out-of-source waveform bounds show an error in the preview rather than breaking the inspector. Rendering continues through the existing manual/agent worker transaction and undo path.

Verification: `test/experimental-audio-warp-graphic.test.js` covers piecewise projection, sample-aligned two-sided drag constraints and draft identity. The stretch-editor browser regression loads the waveform while retaining entered values, drags a destination, cancels a second drag, verifies unchanged project state, and then renders successfully. Preview screenshots were inspected. Full tests/build pass; richer Flex algorithms and real-music listening validation remain unfinished.

### Warp saved attacks to the musical grid

The Time warp panel includes **Align saved attacks to grid**, with grid, strength and swing controls. It replaces the timing-point draft with source positions from the region's saved attack markers and calculated destination positions. Review or drag those points before **Render warped audio**. Generating the draft does not modify the document.

Targets use global musical beats through the complete project tempo map, including the region's timeline start. Strength interpolates in beats; swing delays alternate grid points. Endpoints remain fixed. Source markers are mapped through trim and reverse using the same source-coordinate logic as the waveform editor. The plan requires 1–64 saved interior attacks and validates all warp intervals. Missing markers, coincident targets, sub-50 ms source intervals, out-of-bounds points and ratios outside 50–200% fail without rendering. Use fewer corrected markers, a finer grid or lower strength instead of silently dropping attacks.

The agent tool `quantize_audio_region_warp` accepts a nullable selected region ID and `settings: {grid, strength, swing}` in quarter-note beats and 0–1 fractions (swing maximum .75). The server derives anchors from actual saved markers rather than model-supplied guesses, then returns the existing guarded `warp_audio_region` render action. It shares source preservation, cancellation, undo and standalone-tool restrictions. `regions.quantizeAudio` continues to move whole clips; this tool changes timing inside one continuous region.

Verification: `test/experimental-audio-warp-grid.test.js` checks saved-marker targeting, tempo changes, swing, partial strength, invalid/colliding/identity plans, protection and agent capability guards. The browser editor regression checks generated targets, unchanged project state, editable draft rows, and the existing render path. Live provider execution and real-music listening validation remain separate unfinished work.

### Audition a time-warp draft before saving

**Preview warp** renders the current timing-point draft to a temporary AudioBuffer and plays it through a cloned track with its fades, gain, routing, bus effects and master processing. It excludes other source regions, disables cycle/metronome, and uses the scheduled output gate to stop at the region end. **Stop warp preview** stops early. It does not write a media file, change the project/revision, or create an undo entry. The temporary buffer is removed from the decode cache once the playback graph owns it. Effect history before the start is not reconstructed; tails are cut at the end.

The render stage uses the same bounded cancelable worker as applying a warp. Session revision and transport epoch changes invalidate a pending preview. The shared playback engine checks its own epoch during startup. Preview rendering may take time and is currently repeated for each audition; it is not a real-time Flex engine. Draft values remain available after playback. **Render warped audio** still performs the persistent, undoable operation separately.
...
### Sequence selected regions

The multi-region inspector includes Sequence selected regions. Choose timeline order (ties use session track/region order) or selection order, earliest selected start or playhead, and a nonnegative gap in seconds. Clips remain on their tracks, retain all source/MIDI/fade content, and move end to end. Unselected clips and track automation remain in place; this does not ripple the project or prevent overlaps with unselected material.

Manual UI and agent use `regions.sequence` with comma-separated `regionIds`, optional `position`, `gap` (default 0), and `order` (`timeline` default or `selection`). Up to 1,000 selected clips can move in one undoable operation. Invalid IDs, negative gaps, protected edits, or resulting starts beyond 86,400 seconds reject the entire command. Validation: `test/experimental-region-sequence.test.js` and `scripts/browser-experimental-region-sequence-check.cjs` cover ordering, preservation, cross-track movement, gaps, playhead anchoring, undo, and invalid operations.

### Batch audio fades

The multi-region inspector offers fade-in/out lengths and linear/equal-power curves for its selected audio clips. MIDI/video selections remain unchanged. Require fades to fit rejects the whole edit if either clip is too short; Scale to fit reduces the combined fade lengths proportionally per short clip. Zero removes a fade. These replace existing fades; they do not create overlaps or move clips.

The shared `regions.fades` command takes audio-only comma-separated `regionIds`, required nonnegative `fadeIn` and `fadeOut` seconds, optional `shape` (`linear` default or `equalPower`), and `fit` (`reject` default or `scale`). Protection and atomic undo apply. Playback and exports consume the existing region fade envelope. Validation: `test/experimental-region-batch-fades.test.js` checks preservation, envelope values, proportional fit, undo, malformed inputs, protection, and mixed-media rejection; `scripts/browser-experimental-batch-fades-check.cjs` checks UI filtering, fit rejection, curve selection, scaling and undo.
...
### Audition selected clips

The multi-region inspector has Audition selected clips / Stop audition. A temporary playback document retains only selected, currently audible audio/MIDI regions while preserving all mixer tracks, routes, effects, and automation. Track/region mute and track solo remain respected. Cycle and metronome are disabled for the audition; playback includes the existing routed/master tail calculation. An entirely inaudible selection produces an actionable message. Video regions do not produce audition audio. Saved regions, mute states, revisions, and undo history are unchanged, and audition meter readings are not captured as whole-project agent context.

Tests: `test/experimental-region-audition.test.js` checks isolation, routing, muting/solo, missing IDs and preservation. `scripts/browser-experimental-region-audition-check.cjs` checks controls, stop feedback, unchanged session and native OfflineAudioContext output through a bus while excluding an unavailable unselected source. Agent invocation uses the selection-audition action documented below; waveform-range audition remains separate.


### Agent selection audition

The `audition_selected_regions` tool accepts `regionIds: null` for the captured selection or an explicit array of up to 1,000 clip IDs. It is offered only when the client enables `allowSelectionAudition` and supplies current transport context; it is unavailable during editing continuations. Server validation uses the same selection-audition planner as manual playback. The browser revalidates session/revision, transport epoch, IDs and audibility before starting, honors request cancellation, and reports actual playback status rather than the model's summary. It makes no project edits and cannot be combined with edit commands or follow-up actions.

Tests: `test/experimental-agent-selection-audition.test.js` covers captured/explicit IDs, disabled capability, empty/missing/duplicate IDs, muted tracks, transport context and continuation rejection. The region-audition browser check exercises a mocked agent response through real playback and verifies unchanged saved state. This does not establish live model inference; local provider configuration is still absent.
...
### Agent playback scope

Transport observations now include `mode`: stopped, arrangement, cycle, audioRange, regionSelection, comp, warp, or generic preview. This is read-only context, not session state. Validation rejects a playing/stopped mismatch and cycle observations without an enabled cycle; older clients may omit the field. Agent instructions distinguish temporary auditions from full-project listening and explain that Play without a new position leaves an already running audition intact. Transport summaries identify auditions/previews rather than calling them ordinary playback. Tests cover mode mapping, validation before provider calls, provider payloads, summaries, and a real active audition's browser request. Provider inference remains mocked in these checks.

### Saved scene definitions — playback integration pending

Sessions now save up to 64 ordered scenes, each with a name and up to 128 cells. A cell references an existing audio/MIDI region by `regionId` and stores `loop` (default true); its track and mixer come from that region. One clip per track is allowed in each scene. These are references rather than copied recordings. Editing a source therefore affects its scene cell. Deleting a source/track prunes scene references in the same undo step. Moving a referenced region so two cells would occupy one track rejects the edit unless the scene is repaired within the batch.
...
### Timed scene audition

Scenes now offer an audition duration (0.1–600 seconds), Audition scene, and Stop scene audition. `scene-playback.js` builds a temporary arrangement: cells start at zero, loop cells repeat at their existing duration in seconds, and one-shot cells play once. The original arrangement and saved scene are unchanged. Audio offsets/reverse/fades, MIDI notes/events, instruments, mute/solo, bus/send routing, and effects are retained. Mixer/effect automation follows the audition timeline from zero; this does not retime clips to tempo changes or reproduce automation at each original arrangement location. Playback is gated at the requested end, including effect tails. Cycle and click are disabled. Plans exceeding 10,000 expanded clips or 200,000 MIDI notes/events reject before expansion.

The agent receives `audition_scene` only with the `allowSceneAudition` capability and current transport context. Arguments are sceneId and duration. Server and browser validate the same plan; session/revision/transport epoch checks prevent stale starts. This action is separate from edits and unavailable during editing continuations. Transport mode `scene` distinguishes scene listening from arrangement playback. Auditions cannot record automation or contribute full-mix meter observations.

Native browser audio checks verify repeated audio, one-shot silence, bus gain, exclusion of unselected missing media, and the exact end gate. Browser controls cover manual/agent start, manual/automatic stop and unchanged saved state. Provider calls are mocked; live inference remains unverified. All 1,044 tests and build pass. Quantized live scene switching, independent cell launching, unlimited performance playback and performance capture remain unfinished.

### Place a scene into the arrangement

The Scenes panel now includes Place scene in arrangement, with start/length in seconds and an explicit overlap choice. Shared `scene.place` targets a scene ID with `position`, `duration` (0.1–600 seconds), and optional `overlap` (`reject` default, or `allow`). It creates independent clips on each source track, repeating loop cells and placing one-shots once. The final copy is trimmed to the selected duration using existing audio/MIDI crop logic, including reversed source offsets and note cropping. Region/note/event IDs are regenerated; copies are detached from take groups. Originals, scene references, source files, routing and automation remain unchanged. Muted cells are included with their mute state preserved; placement does not depend on current solo state. Existing clips are never deleted or shifted. Automation remains at its original absolute times, so this does not reproduce audition automation from zero at the destination.

Default placement rejects any overlap on a destination track; Allow overlap layers new clips with existing ones. Protection, timeline bounds, the 1,000-region per-track limit and expansion limits reject atomically. Placement is one undoable edit; the agent uses the same command. Unit tests cover reverse cropping, partial MIDI notes, original preservation, unique IDs, mute/protection, overlap, limits, undo/reload and mocked agent commands. Browser tests use the actual form for rejection, corrected placement, loop/one-shot lengths, undo/redo and reload. All 1,050 tests and build pass. Live scene performance capture and quantized switching are still unfinished.

### Quantized scene handoff

While a scene audition is running, select another scene and use Cue scene with Immediately, Next beat, or Next bar. The existing duration field controls the next scene's length. `scene-transport.js` schedules both graphs against `AudioContext.currentTime`; complementary output gates switch at the same boundary. The UI tick only updates the active scene and releases old nodes. Beat/bar boundaries use the project tempo and meter maps on a continuous scene clock, including denominator and signature changes. Scheduling reserves 100 ms after media preparation, so it uses the next eligible boundary rather than a beat too close to prepare safely. If preparation misses its deadline, the new graph stays silent and is released.

Only one pending scene is allowed. Stop cancels both current and queued graphs. The current audition must extend past the requested boundary; otherwise queueing rejects. Each scene retains its bounded audition duration, and the next scene extends the transport end. Handoffs cut old effect tails; each new scene's clips and mixer automation begin at local time zero. This does not implement independent cell launching, crossfaded scene transitions, tempo-following audio stretch, or performance capture. Scene auditions no longer play the arrangement video in the background.

The agent's `queue_scene` tool is offered only for observed active scene playback with `allowSceneAudition`. It takes sceneId, duration and quantization; server scene validation and browser revision/transport checks apply. Unit tests cover tempo/meter boundary calculation, pending and stopped states, future graph cleanup and missed deadlines. Browser tests cover manual and agent queueing; native offline audio proves the sign-changing handoff occurs on the precise boundary without any UI advance callback. All 1,055 tests and build pass; provider responses remain mocked.

### Record scene performances

Use Record performance in Scenes to start an audition that captures scene launch timing. Cue scene works as before; Stop recording or the final scene's timed end finishes the take. The audio-clock log includes only scenes whose scheduled start preceded the stop time, trims each scene at the actual next launch/stop, and remains accurate if UI ticks are delayed. Up to 100 launches are retained. Ordinary auditions do not create a recorded take. Arrangement commands are blocked during active recording.

A finished take is stored under the account's local `:scene-performance` key and restored after refresh. The panel offers Save performance (destination start and overlap choice) or Discard performance. Saving creates independent editable regions through the shared placement logic in one undo step; short final fragments are supported. Originals remain. This captures clip timing, not rendered audio, restarted mixer automation or tail cutoffs. Saving requires the same session ID and matching source clips; new takes use source fingerprints as described below, while older takes require their original revision. Save/discard a pending take before starting another recording. A take is persisted on stop, so closing the page during active recording still loses that unfinished take.

Shared `scene.performance` values are `performance` (JSON-encoded sessionId/revision/events receipt), `position`, and `overlap` (reject default or allow). Each event has sceneId, start relative to the performance, and duration. It must be the only command in its batch. Server validation enforces ordered, non-overlapping events, matching recorded sources, source scenes, track protection/limits and timeline bounds. The agent receives a fresh pending take as recordedScenePerformance and can save it on request; recording itself starts manually. Unit tests cover delayed ticks, pending launches canceled before playback, short clips, atomic late failure, stale revisions and mocked agent saving. Browser checks cover recording, automatic stop, local refresh recovery, saving and undo. All 1,059 tests and build pass.

### Source-aware performance recovery

New scene performance events include a SHA-256 `sourceSignature`, generated from canonical scene membership/loop settings and referenced clip content, track ownership and kind. Scene/clip names, original arrangement positions, take-group labels and source attack markers are excluded because they do not determine the placed performance's clip timing/content. Mixer settings, project labels and unrelated tracks/regions are also excluded. Therefore a take stays saveable after those edits. Changes to notes/events, duration, source offset/asset, gain/fades/reverse/mute, scene membership/loop state or source track invalidate it. Undoing the source edit restores saveability even though the document revision increases.

This checks the saved clip definition, not original media bytes or authenticity. SHA-256 uses the pinned @noble/hashes 2.4.0 package; imports follow its [official usage documentation](https://github.com/paulmillr/noble-hashes#usage). Canonical key ordering and deterministic cell sorting make signatures stable across JSON reloads. Legacy events without sourceSignature retain strict revision matching. The UI and agent context share the same validator, and command application revalidates every source. Tests cover allowed/blocked edits, deletion, wrong sessions, altered signatures, Undo recovery, property reordering, refresh and agent saving across unrelated revisions. All 1,064 tests, browser checks and production build pass.

### Independent scene cell launching

During a scene audition, select a grid cell and use Launch assigned clip. Its duration and immediate/beat/bar timing are independent of the whole-scene cue controls. Assign any pending source/loop changes before launching. The cell replaces only its source track's voices; other tracks and track/bus/master effects keep running. It honors track/region mute and solo. Cell playback is capped by the current scene's end, and a whole-scene handoff replaces all cells. Only one pending launch per track is accepted. Launching a cell during a queued whole-scene switch rejects. Cell launches/stops can now be recorded per track as described below.

`audio-voices.js` separates audio/MIDI voice scheduling from the persistent mixer graph. The audio engine's replaceTrackRegions API schedules a track-local gate on the audio clock, with an optional duration cutoff. collectVoices releases retired groups after the scheduled cut; stopping the graph releases all current/pending voices. Native rendering verifies that a replacement enters the existing delay and bus path, retains old delay history at the handoff, ends at its requested duration, and leaves another MIDI instrument unchanged. Source preflight errors preserve existing playback.

The agent launch_scene_cell action uses the same sceneId/trackId/duration/quantization planner and is available only for observed scene playback. Browser revision/transport epoch checks protect decoding and scheduling. No session edits are made. Checks cover cell plans, mute/solo, recording/pending-scene guards, manual/agent controls, native audio, sampler loops/seeks and pedal-held MIDI split playback. All 1,067 tests and build pass. Live provider inference remains unverified.

### Stop a track's scene clip

During scene audition, Stop track clip uses the selected cell's track and launch-timing choice (immediate/beat/bar). It stops whichever clip is playing or queued on that track, even if the selected scene has an empty cell. It does not require valid source/duration editor fields and does not alter track mute or saved regions. Other tracks continue, and downstream effect tails finish naturally within the scene's overall playback window.

The voice gate cancels future open events at the stop boundary, so a pending clip cannot start afterward. Retired and pending voice groups are released after that boundary; launching again is allowed once cleanup reaches it. The agent stop_scene_cell action takes trackId and quantization, shares destination validation, and checks the transport epoch before applying. Stops during a pending whole-scene handoff reject; cell-level recording is described below.

Native audio testing stops a track before its queued launch, suspends rendering to run cleanup, launches it again, and verifies the preserved delay tail, canceled launch, resumed clip, and unchanged second track. Browser checks cover manual and agent Stop, unchanged session state, and existing scene workflows. All 1,069 tests and build pass; the existing bundle-size warning remains.

### Per-track scene performance capture

Record performance now supports Launch assigned clip and Stop track clip. New takes contain track-scoped passages (`trackId` on each event), so simultaneous tracks can overlap while passages on one track cannot. Initial/queued scenes create one passage per audible cell; one-shots end at their clip duration. Replacing/stopping a cell caps only that track. Canceled future launches produce no passage, and a whole-scene handoff caps every previous track. The recorder uses scheduled audio-clock boundaries and handles delayed UI ticks. Each take is limited to 100 scene launches and 1,000 clip passages; failed limit checks leave current playback/recording intact.

Saving remains one `scene.performance` command and one undo step. It places only the event's track from its source scene, with signatures scoped to that cell. Changing another cell in the same scene does not invalidate an otherwise independent recorded passage. Legacy whole-scene receipts remain supported. Receipt JSON is capped at 500,000 characters; recorded clips retain the existing expansion/track/timeline limits. The recorded length display uses the latest passage end across all tracks rather than the last-starting event.

`scene-performance-log.js` handles passage timing. Tests cover cancellation before launch, stop/relaunch, handoffs, one-shot ends, passage limits, simultaneous tracks, invalid overlaps, cell-scoped source recovery and atomic placement. The browser workflow records a cell launch and stop while another track continues, then saves and undoes it. `browser-experimental-cell-performance-check.cjs` renders a two-track performance, records an independent launch/stop, saves it as clips, and compares replayed samples after removing initial launch latency: zero differing samples in that fixture. This proves the tested static mix and clip timing, not full automation/effect-tail parity. All 1,074 tests, browser/audio checks and build pass. Indefinite cell transport, live automation capture in scenes and richer take management remain unfinished.
...
### Cue markers in exported WAV files

Mix, stem, range and selected-region WAV downloads now include named RIFF `cue ` points and `LIST/adtl/labl` labels. Each file contains session markers within its rendered interval, measured relative to the file start. The end boundary is exclusive; exported effect tails can contain markers. Positions round to the nearest sample frame (clamped to the last valid frame). Markers do not extend the render or change audio samples, and no absolute movie timecode is embedded. The shared export path also covers agent exports.

Integer 16/24-bit and float 32-bit formats preserve their existing audio layout; cue metadata follows the padded data chunk. Labels are UTF-8, with embedded NUL characters removed. Marker support and non-ASCII display depend on the receiving application. Recording and in-place rendering remain unchanged unless markers are explicitly passed to the encoder. WAV marker import support is described in the following section.

References: [Logic audio-file marker exchange](https://support.apple.com/en-ca/guide/logicpro/lgcpadb63ff8/mac), [Microsoft RIFF chunk sizing and padding](https://learn.microsoft.com/en-us/windows/win32/multimedia/resource-interchange-file-format-services), and [FFmpeg WAV cue/label reader](https://github.com/FFmpeg/FFmpeg/blob/master/libavformat/wavdec.c).

All 1,168 tests and build pass. Unit checks verify unchanged audio, chunk sizes, word padding, Unicode labels, frame rounding, interval rebasing and invalid metadata. An independent FFmpeg process recognizes the exported cue positions and names. The agent-export browser check inspects actual mix, stem ZIP, range and region downloads, including omitted outside/end markers and rebased positions; model responses are mocked. The existing bundle warning remains. Receiving-DAW compatibility has not been manually tested.

### Import embedded WAV cue markers

Ordinary audio uploads now read supported WAV cue metadata and add markers at the clip’s import position. The clip and cues are one atomic undoable edit; recording imports retain their existing behavior. Exact same-name/time markers are skipped, so importing aligned stems does not duplicate their shared cues. Markers remain independent snapshots after import.

`readWavMarkers` slices RIFF headers and metadata without copying PCM data again. It supports little-endian RIFF PCM and IEEE float WAV with `cue ` and `LIST/adtl/labl`, including labels preceding cue points. Positions use the source sample rate, independently of browser resampling. Unlabelled points receive “Cue <ID>”; UTF-8 names are limited to 200 characters. Non-WAV inputs and files without cues return no markers. RF64, RIFX, compressed/extensible WAV cue addressing and AIFF markers are not supported yet.

Malformed or unsupported optional metadata leaves the audio import available and reports that cues were skipped. Marker counts, metadata bytes (1 MB), chunk count (10,000), bounds, addressing and duplicate cue IDs are checked. Valid cues exceeding the session’s 1,000-marker or 24-hour limits reject the combined edit before media is stored. Session revisions are checked after decoding/metadata reads and storage to prevent attaching an asynchronous import to a changed document.

The shared `markers.import` command accepts `values.markers` as a JSON name/time array and optional `values.start` in seconds, enabling the agent to import supplied cue lists through the same validation. Source data must be supplied, not invented.

Verification: 1,172 tests pass in the full suite, plus the subsequently added agent-import test passes in the focused suite. Build passes with the existing bundle warning. The WAV-import browser test verifies actual decoding/upload, Unicode names, exact offset placement, atomic Undo/Redo and malformed metadata fallback. Agent provider output is mocked. This completes the import counterpart to the RIFF export documented above, within the stated format limits.

### Named MIDI articulation switches

Instrument inspectors now have an **Articulation switches** editor. Save up to 128 named mappings per track, choosing a keyswitch note, controller change or program change. The UI shows channels 1–16; stored commands use 0–15. Note/controller numbers and program values use 0–127. Keyswitch velocity must be positive, and duration is 1–1,000 ms. Select a region, choose the saved switch, and insert it at a region-relative time. Place it early if the target library needs preparation time.

`midiSwitch.save` targets a MIDI track with optional existing `id`, `name`, `type`, `channel`, `parameter`, `value` and optional `duration` (default .05 seconds). Program changes require parameter 0. `midiSwitch.delete` targets the track with `values.id`; `midiSwitch.insert` targets a region with `values.id` and `values.start`. The agent receives these instructions and must use supplied library mappings. Each command supports shared validation and Undo; saving and inserting can be one atomic batch.

Switches insert ordinary notes/events, retained by existing MIDI export and hardware scheduling. Equal-time keyswitch note-ons precede musical note-ons. A keyswitch cannot overlap another sounding note of the same channel/pitch; program/controller insertions reject conflicting same-position messages. Keyswitch length clips at the region boundary. Preset changes/deletion leave earlier insertions unchanged. Instrument-settings copy replaces the switch library with an independent copy, including clearing mappings when the source has none.

This is explicit MIDI switch authoring, not Logic’s complete articulation-ID engine. Built-in synths do not implement orchestral techniques and may play keyswitches as pitched notes. General note edits also affect them. Latching keyswitches are not restored when seeking past their note: begin playback before the switch. Program/controller events retain existing chase behavior. The following section adds explicit per-note assignment. Remote-switch modes, sampler articulation groups and library-specific mappings are not assumed. [Logic’s articulation editor reference](https://support.apple.com/en-lamr/guide/logicpro/lgcp33a49091/10.7/mac/11.0).

All 1,178 tests and build pass. Tests inspect actual MIDI export/reimport, hardware message ordering and controller chase, unchanged musical notes, Undo, preset copying, collision/protection validation and mocked-provider agent batches. The browser switch-editor check exercises save/insert/edit/delete, bounds and Undo/Redo. Physical hardware and live model behavior remain unverified. Existing bundle-size warning remains.

### Assign articulation switches to selected notes

The articulation editor can now **Assign switch to selected notes** or **Clear note articulations**. Each assignment stores a snapshot of the saved mapping on the selected notes. Musical pitch, timing and velocity stay unchanged, and the piano roll gains no extra keyswitch notes. Preset edits/deletion do not alter assignments. The inspector lists the techniques assigned to the current selection. Oscillator and drum instruments ignore these assignments; MIDI export, compatible external instruments and configured sampler mappings use them.

`notes.articulation` targets a region with explicit comma-separated `values.noteIds` and `values.id` of a saved switch, or `null` to clear. The agent uses this shared undoable command. Switch channels are copied from the mapping, rather than automatically following subsequent note-channel edits.

MIDI export and device playback generate switches before note-ons. Identical simultaneous assignments share one message; different assignments on the same switch channel at the same instant reject. Keyswitch collisions with musical notes also reject. Keyswitch pulses shorten before the next same-pitch pulse; exported pulses last at least one MIDI tick. Hardware playback regenerates assignments for notes still sounding at a seek, including sustained notes, and includes switch channels in existing overlap checks. Assignments are emitted at note onset, without configurable advance time; use explicit early switch insertion for libraries requiring it. Clearing an assignment leaves the device’s latched state alone, so explicitly assign the desired default technique when needed. Standard MIDI exports contain ordinary switch messages, not editable assignment metadata.

This adds note assignments to the explicit-switch foundation above. Full remote switching, automatic defaults, sampler articulation groups and live input articulation control remain unfinished. Tests verify chord deduplication, seek restoration, unchanged musical data, muted notes, conflicting mappings, pulse lengths, independent preset snapshots, Undo and mocked-provider agent batches. The browser check assigns/clears an actual piano-roll selection and inspects the hardware plan at a held-note seek. All 1,182 tests and build pass; physical devices and live model behavior remain unverified. The existing bundle warning remains.

### Sampler sources selected by note articulation

The instrument inspector’s **Articulation samples** panel maps a saved or assigned articulation ID to an uploaded audio file. Each mapping has its own root pitch and loop boundaries. Saving a mapping activates the sampler. Assigned notes use their matching mapping; unassigned or unmapped notes use the main sample. Track envelope, tuning, filter, controller behavior, routing and effects remain shared. The source files are unchanged.

`samplerArticulation.set` targets the instrument track with `id`, `assetId`, optional `root` (60), `loop` (false), `loopStart` (0) and `loopEnd` (null for source end). It accepts existing saved/assigned IDs or updates an existing mapping. `samplerArticulation.delete` removes a mapping by `id`, restoring the main-source fallback. UI saves decode the selected file and validate loop bounds before committing; agent commands validate the mapping structure, and playback validates source availability and loop bounds. The agent must use supplied asset IDs. These commands share Undo.

Playback and offline bounce resolve the source per note. Only samples needed by audible notes are decoded; portable/account storage retains all mapped sources, including currently unused ones, and remaps their references on restore. Instrument-settings copy includes mappings; bouncing a region to an audio track clears that copied track’s sampler mapping references. Mapping selection uses the copied note articulation ID, so renaming or deleting a saved switch does not break existing assignments.

This is one sample per articulation across the keyboard, not yet a full zone/group editor with note ranges, velocity layers or round robin. Live MIDI monitoring uses the selected input articulation described below; per-note assignments affect arrangement/scene playback and exports. [Logic sampler articulation reference](https://support.apple.com/en-ie/guide/logicpro/lgcp4f103162/mac).

All 1,186 tests and build pass. Tests cover per-note selection/fallback, roots/loops, required source validation, archive preservation/remapping, instrument copying and mocked-provider agent commands. The browser test uploads two distinguishable sample signals, maps the second source in the UI, downloads a real stereo WAV and verifies that each note uses the correct signal; Undo reverts the audible result and Redo restores the mapping. The existing bundle warning remains.

### Input articulation for MIDI recording and memory

Choose a saved switch in the articulation editor and click **Use switch for MIDI input**. The track retains a snapshot of that mapping; **Clear input articulation** restores unassigned input. `midiSwitch.input` exposes the same operation to the agent, targeting a MIDI track with `values.id` or null. Renaming/deleting the saved preset does not mutate the snapshot or older notes. Instrument-settings copy includes the selected input articulation.

Recording captures the input articulation at preparation time. MIDI memory captures it when Remember MIDI starts, even with monitoring off, and retains it through pause, input-selection changes and save retries. A sampler monitor uses the corresponding mapped source/root/loop, with existing track envelope/filter/tuning; it does not send switches to an external MIDI device. Takes preserve the snapshot as note articulation metadata instead of generating audible switch notes. The recording import path uses optional `midi.import` `values.articulation` (a validated JSON mapping) to retain this through its MIDI serialization step, atomically with the new region.

One articulation is fixed for each recording or remembered performance. Switching remotely or assigning changing techniques mid-take remains unfinished. Changing the input selection after memory starts does not change its sound or recorded assignments. Recorded notes use the destination track’s current articulation sample mapping on playback; those sample mappings are not frozen into the take.

Verification: 1,188 tests pass in the full suite. Focused tests additionally verify that an input snapshot remains available for sample mapping after deleting its saved preset. Final build passes with the existing bundle warning. The extended sampler-articulation browser check uses simulated Web MIDI with real Web Audio monitoring: it confirms the mapped buffer is played, the recorded take contains one musical note with its assignment, and a monitored-off remembered take preserves its original assignment after clearing the track input selection. Physical MIDI hardware remains unverified.

### See and select articulations in the piano roll

Assigned notes now show a clipped technique label plus the full name in their tooltip and accessible label. **Note color → Articulation** gives each articulation ID a stable color; Velocity retains the previous opacity display. Muted and selected note treatments remain visible. Names are escaped before rendering. Labels do not intercept dragging or resize handles.

**Select notes by condition** includes Any, Unassigned and the region’s assigned techniques, combined with existing pitch/velocity/channel/timing/mute conditions. Selection retains Replace/Add/Remove/Intersect behavior and does not edit the document or add an Undo step. Reset conditions clears the articulation filter. IDs distinguish same-named techniques; duplicate names show IDs in the option label.
...
### Sampler key ranges, velocity layers and overlapping zones

An instrument track’s **Sample zones** editor maps imported audio to inclusive MIDI key and velocity ranges, with a name, root pitch, optional articulation restriction and per-zone loop. The map shows keys horizontally and velocity vertically; zone buttons load the matching form for editing. Save updates the chosen zone or creates a new one; Delete removes it. The inspector stays open after saves. UI saves decode the source and validate loop bounds before committing.

`samplerZone.set` targets a MIDI track and upserts `values.id`, `name`, `assetId`, `root` (default 60), `keyLow`/`keyHigh` (default 0/127), `velocityLow`/`velocityHigh` (default 1/127), `articulationId` (null for any), `loop`, `loopStart`, and `loopEnd`. `samplerZone.delete` removes `values.id`. Both use shared validation and Undo. Sessions allow up to 128 zones per track. Velocity matching rounds normalized note velocity to MIDI 1–127. Matching zones all play together; their levels add. With no zone match, the note uses its articulation mapping or main sample; a zone instrument without either fallback stays silent outside its zones.

Arrangement/scene playback and bounce schedule each matching layer with its own root and loop and shared track envelope/filter/tuning. Live MIDI input snapshots the instrument, preloads eligible sources, selects layers per note/velocity and releases each note’s layers together, including repeated pitches and sustain. The existing live voice limit counts layers (64 by default). Stored projects/archives retain and remap all zone sources, including unused zones; playback decodes only required sources. Instrument-settings copy includes zones and clears stale ones when absent on the source. Bounce-in-place clears zone references on its new audio track.

This adds explicit key/velocity mapping and layering, following [Apple’s sampler model](https://support.apple.com/en-gb/102060). Automatic sample analysis and round robin remain unfinished. Zone velocity crossfades are described below. Basic sampler groups are described below. Direct map editing is described below.

Verification: 1,200 tests and production build pass, with the existing bundle-size warning. Tests cover inclusive boundaries, layering/fallback, articulation restrictions, invalid edits, archives, source validation, Undo/Redo, instrument copy and mocked-provider commands. `browser-experimental-sampler-zones-check.cjs` verifies the editor, actual downloaded WAV sample signs, layering, Undo/Redo, reload, simulated MIDI source selection, and repeated-note/sustain releases using offline Web Audio. The rendered inspector was visually inspected. The separate real-time MIDI-monitor browser check did not pass: the host browser’s AudioContext reported running while its clock failed to advance, preventing timed source-end verification. Physical-device timing and live model inference remain unverified.


### Direct editing in the sampler zone map

Select a zone by its rectangle, button or form dropdown. The selected rectangle rises above overlapping zones and shows four resize handles. Drag its body to move both key/velocity ranges; drag a handle to adjust one edge. Moves retain range width/height and clamp at MIDI boundaries; handles cannot cross their opposite edge. Root pitch, source, articulation restriction and loop stay fixed.

Pointer gestures preview locally and commit one Undo step on release. Escape, pointer cancellation or lost pointer capture restores the previous range. A session repaint during a drag prevents the detached editor from committing. Selection alone does not edit the document. Arrow keys move the focused zone or handle by one MIDI unit; Shift uses twelve. Focus survives keyboard commits, including overlapping-zone promotion. Saved zones stay selected after form saves and range edits.

`samplerZone.range` targets a MIDI track with `values.id` of an existing zone and one or more `keyLow`, `keyHigh`, `velocityLow`, `velocityHigh` values. It validates the resulting zone while preserving all other properties. The agent uses this shared command to change ranges without resupplying sample settings. Creation/deletion continue through the existing commands.

Verification: all 1,204 tests and final build pass with the existing bundle warning. Unit tests cover clamped moves, edge constraints, atomic invalid-command rejection, unchanged source settings, Undo and mocked-provider agent edits. The new zone-map browser check exercises actual pointer movement/resizing, overlap selection, Escape cancellation, keyboard focus, Undo/Redo, fixed root/loop and reload. The prior zone audio-rendering browser check also passes, including actual layered PCM and offline note releases. The inspector screenshot was visually inspected. Physical MIDI timing and live model inference remain unverified.

### Per-zone sample level, pan and tuning

The Sample zones form now includes **Zone level · dB** (-60 to +24), **Zone pan** (-1 left to +1 right), **Zone tune · semitones** (-48 to +48) and **Zone fine tune · cents** (-100 to +100). All default to zero for older sessions. These controls provide the per-zone mixer and pitch adjustments described in [Apple’s zone editor guide](https://support.apple.com/en-lb/guide/logicpro/lgcp7431220e/mac).

Zone tuning adds to key/root mapping and the track’s coarse/fine tuning; pitch bend still applies afterward. Playback and seek offsets use the same combined rate, including looped samples. Zone gain scales the note envelope and zone pan uses a stereo panner before MIDI channel and track controls. Overlapping zones retain independent gain, pan and tuning. Centered zones retain their previous signal path. The live monitor resolves these same settings per layer and cleans up each zone panner when its voice ends or stops.

`samplerZone.set` accepts `gainDb`, `pan`, `tune` and `fineTune`. `samplerZone.adjust` targets the MIDI track with an existing `values.id` and one or more of those four fields, preserving the source, ranges, root, loop and omitted sound fields. This is a shared undoable agent operation. Partial validation explicitly removes creation defaults so adjusting pan cannot silently reset gain or tuning. Instrument copying, session persistence and existing archive serialization retain the new zone fields.

Verification: all 1,208 tests and production build pass; the existing bundle warning remains. Unit tests cover additive tuning/seek math, defaults for old projects, partial-edit preservation, invalid edits, instrument copy, Undo and mocked-provider commands. `browser-experimental-sampler-zone-voice-check.cjs` verifies form editing, Undo/Redo/reload, actual offline PCM amplitudes and channels, independent layered pitches/levels, combined track/zone tuning, pitch bend, monitor audio and looped seek phase. The rendered controls were visually inspected. Tests use offline Web Audio for sound verification; physical-device timing and live model inference remain unverified.


### Sampler groups for related sample zones

**Sample groups** creates named collections of zones with shared level, stereo pan, coarse/fine tuning, mute and solo. Check member zones in the group form, or choose a group in an individual zone’s form. Each zone belongs to at most one group. Membership changes move selected zones out of previous groups; removing a member leaves it ungrouped. Deleting a group keeps its zones. Group edits use Undo, and instrument copying/portable projects retain membership and settings.

Group gain and tuning add to zone gain/tuning and existing track tuning. Zone voices feed a shared group stereo panner per MIDI channel before channel and track controls. Neutral groups preserve the previous sound. Mute suppresses member zones without causing a matched note to use the fallback sample. If any groups are soloed, only nonmuted zones in soloed groups sound; ungrouped zones and fallback sources are suppressed. Multiple groups can be soloed. Live input snapshots these settings and preloads audible zone sources; its render path uses the same sample selection, gain, tuning and group pan. Mapped sources remain in archives even while muted.

`samplerGroup.set` targets a MIDI track, with `values.id`, `name` for creation and optional `gainDb`, `pan`, `tune`, `fineTune`, `mute`, `solo`. Existing IDs accept partial updates without resetting omitted fields. `samplerGroup.members` takes the group `id` and `zoneIds` as a JSON array of existing zone IDs, replacing its membership atomically. `samplerGroup.delete` removes the group and clears its zones’ group references. `samplerZone.set` accepts `groupId` or null. The session validates group references and limits tracks to 64 groups. These shared commands are available to the agent.

This implements the core group organization and mixer workflow described in [Apple’s sampler group guide](https://support.apple.com/en-au/guide/logicpro/lgcp420be8ac/10.7/mac/11.0). Separate output routing and round robin remain unfinished. Group key/velocity conditions and envelope/filter offsets are described below.

Verification: all 1,213 tests and final build pass, with the existing bundle warning. Tests cover combined settings, mute/solo/fallback behavior, membership transfer/deletion, Undo, invalid references, partial updates, instrument copying, portable archive preservation and mocked-provider agent batches. The group browser check exercises form membership, deletion, Undo and reload, and measures actual offline PCM for neutral gain, summed group pan, combined tuning, mute, solo and agreement with the monitor render path. The previous per-zone sound browser check also passes. Group controls were visually inspected. Physical-device timing and live model inference remain unverified.


### Sampler group ranges, envelopes and filters

The group editor’s **Playback range and sound** section adds key/velocity ranges, attack/decay/sustain/release offsets, and a group filter choice with cutoff/resonance offsets. Ranges intersect the zone’s own ranges. A note that matches a zone but fails its group conditions stays silent instead of triggering a fallback sample. Group settings affect arrangement playback, bounce and MIDI monitoring; the original audio files and track controls stay intact.

`samplerGroup.set` accepts `keyLow`/`keyHigh` (0–127), `velocityLow`/`velocityHigh` (1–127), `attackOffset`/`decayOffset` (−10 to +10 seconds), `sustainOffset` (−1 to +1), `releaseOffset` (−30 to +30 seconds), `filterType` (`inherit`, `off`, `lowpass`, `highpass`), `cutoffOffset` (−8 to +8 octaves) and `resonanceOffset` (−50 to +50 dB). Envelope offsets add to track settings and clamp to the existing envelope limits. Cutoff multiplies the track cutoff by 2 to the offset power, then clamps to 20–20,000 Hz; existing per-note filter key tracking/Nyquist limits still apply. Resonance adds to the track value and clamps to −20 to +30 dB. Defaults retain the full range and track sound. Partial updates preserve omitted settings.

Duration and bounce/seek planning account for longer group releases. Monitoring keeps an envelope per voice, including a group’s release length. This work also fixes an existing monitor release bug: starting a new ramp after a finished attack could make the ramp begin at that attack endpoint. Note-off now explicitly sets the calculated held level at the current audio time before scheduling release. Early note-off retains the current attack/decay level.

Verification: all 1,218 tests and final production build pass, with the existing bundle warning. Unit tests cover intersected ranges, no fallback on excluded group notes, clamped sound offsets, partial updates, invalid edits, Undo, group release tails through region/range bounce plans and mocked-provider commands. The group-shaping browser check verifies form save/Undo/reload, actual offline PCM attack/release levels, seeking into a release tail, low-pass attenuation, excluded-note silence and matching monitor behavior, including early release and instant attacks. The form was visually inspected. Physical MIDI timing and live model inference remain unverified.

### Reusable sampler instrument presets

The instrument inspector’s **Sampler instrument presets** panel saves named instrument snapshots with the current project. Save captures the main sample and sampler controls, zones, groups, group shaping, articulation sample mappings, saved switches, selected input articulation and pitch-bend range. Apply installs a separate copy on the selected MIDI track while preserving notes, regions, effects, volume/pan, automation and routing. Update from this track replaces the saved snapshot without changing previously applied tracks. Rename and Delete manage the bank. All edits support Undo.

`samplerPreset.save` targets a sampler track with `values.name` and optional new `id`; `samplerPreset.apply` and `samplerPreset.update` target an instrument track with `values.presetId`. Update requires a sampler source. `samplerPreset.rename` targets the preset ID with `values.name`; `samplerPreset.delete` targets the preset ID with empty values. Projects can retain up to 32 presets with distinct names. Applying a simpler preset clears stale groups/zones/articulation mappings on the destination. Snapshot validation includes group references and sampler settings; presets cannot contain arbitrary track fields.

Preset source references participate in project media retention and portable archive export/remapping, even after all source tracks are deleted. They do not cause unused samples to be decoded during playback. Preset snapshots are project-scoped; standalone instrument exchange is described below. An account-wide instrument library remains unfinished. Portable project archives carry the bank and its source files together.

Verification: all 1,223 tests and production build pass, with the existing bundle warning. Unit tests cover capture of every sampler field, independent application/update, unchanged musical/mixer data, stale-setting removal, invalid names/targets/references, Undo, preset-only media archives/remapping and mocked-provider commands. The browser check saves an instrument, deletes its original and imported source tracks, reloads, applies the retained preset to another track and verifies the downloaded stereo WAV. It also checks rename/delete, Undo/Redo and unchanged MIDI notes. The preset panel was visually inspected. Live model inference remains unverified.


## Standalone sampler instrument files

Experimental → Sampler presets offers **Export instrument** and **Import instrument**. A `.cuestamp-sampler.zip` contains one preset and its referenced original sample files, including zones and articulation sources. It excludes project tracks and recorded takes. Import adds a preset to the current bank without replacing the session or changing tracks; apply it separately to a MIDI track. Duplicate names receive a numeric suffix, and preset/media IDs are regenerated. Import supports Undo/Redo and persistence across reloads.

The general media picker also recognizes this filename suffix. Dedicated import validates the manifest regardless of filename. Archives use the existing bounded ZIP reader (512 MB source media, 10 MB document); malformed archives, missing sources and banks exceeding 32 presets are rejected. Media writes precede the shared `samplerPreset.import` command, with session/revision checks and cleanup of uncommitted files.
...
## Sampler mapping from source filenames

**Sample zones → Map multiple zones → Read roots from filenames** previews interpreted MIDI roots before applying. It reads actual `File.name` values in the local media store, not editable zone names. Choose **C4 = MIDI 60** (default) or **C3 = MIDI 60** to match the sample library's octave notation. Explicit numeric labels are independent of that setting.

Supported labels are bounded note tokens such as `Piano_C4.wav`, `Strings_F#3.wav`, `Bass_Db2.flac`, Unicode sharp/flat equivalents, or explicit `midi60`, `note_60`, and `root=60` labels. Bare numbers do not imply a pitch. Repeated labels must agree; conflicting labels, missing sources, absent labels and notes outside MIDI 0–127 reject the entire operation. Negative octave labels such as C-1 are supported under the C4 convention. This is filename interpretation, not audio pitch detection.

The shared `samplerZone.automap` roots mode now accepts optional `roots`, a JSON string containing exactly one `{id, root}` entry per selected zone. It sets roots and maps ranges atomically, preserving order, source IDs, loop and sound settings. It supports all 128 zones in a single command/Undo step; other mapping modes reject root overrides. Existing group, articulation and velocity-lane separation still applies.

The `map_sampler_filenames` agent tool is gated by `allowSamplerFilenameMapping`, requires observed track/zone IDs, explicit middleC (3 or 4) and low/high key bounds, and is unavailable during editing continuation. The server validates the request; the browser resolves filenames from actual source files at the captured session revision and applies the same plan as the manual form. The model does not supply filenames or inferred roots.

Verification: `test/experimental-sampler-filename-map.test.js` covers note conventions, ambiguity, source lookup, atomic rejection, 128-zone mapping, preserved fields, Undo/Redo and mocked agent capability validation. `scripts/browser-experimental-sampler-filename-map-check.cjs` verifies the live preview, convention changes, mapping, actual two-source WAV PCM, Undo/Redo, mocked agent application and reload. Live model inference remains unverified. Reference: [Apple Sampler mapping menu](https://support.apple.com/en-ca/guide/logicpro/lgcp70bb8459/mac).

## Create multiple sampler zones from imported audio

Experimental → instrument track → Sample zones → **Create zones from audio** lists locally imported audio. Select files, preview their mapping, and choose an existing sample group if needed. **Consecutive keys** assigns one key per file in displayed order, starting at the chosen MIDI key; the root follows the key so each sample plays at its original pitch. **Read roots from filenames** uses the existing C3/C4 convention parser and spreads the new roots across MIDI 0–127. Equal roots remain layers. This appends zones; existing zones, track settings and group restrictions remain in effect and may overlap or limit playback.

Creation validates capacity (128 zones), selection, group, ranges and filenames before doing work, then decodes each source using the existing browser decoder. Only after all sources decode does it commit the shared `samplerZone.addMany` command. Decode failure, cancellation or a changed session prevents the edit. No new media copies are created. The complete batch is one Undo step and persists with the session; normal preset saving and instrument export include the resulting zones and original media.
...
## Sampler zone velocity crossfades

**Sample zones** now has **Velocity fade-in width**, **Velocity fade-out width**, and **Velocity fade curve** controls. Widths are integer MIDI-velocity distances inside the zone's existing inclusive range; zero disables that edge fade. The width must fit within `velocityHigh - velocityLow`. The curve is **Linear gain** (default) or **Equal power**. These are triggered-note velocity fades, not elapsed-time fades or continuously modulated sample selection.

For a smooth two-layer transition, overlap the ranges: e.g. Soft velocities 1–96 with fade-out 64, and Hard velocities 32–127 with fade-in 64. Linear gains sum to one through velocities 32–96; equal-power squared gains sum to one. Equal-power layering of identical/correlated samples can increase amplitude at the midpoint. Normal note velocity, zone/group levels and track mixing still apply.

A fade edge reaches zero at the corresponding range endpoint. Matching zones at zero gain stay silent instead of falling through to the main/articulation sample. Out-of-range notes retain the existing fallback behavior. When both edge fades overlap within one zone, the smaller edge gain applies. Group key/velocity restrictions continue to limit triggering; this does not implement group crossfade controls or sample-select modulation.

`samplerZone.adjust` can patch `velocityFadeIn`, `velocityFadeOut`, and `velocityFadeCurve` while preserving other fields. `samplerZone.set` and bulk creation accept them as zone properties. Older zones default to zero widths and linear curves. Graphical/automatic range edits that would invalidate an existing fade width reject atomically; shorten the fade before narrowing the range. Copies, presets and instrument archives preserve these fields. Manual edits and the agent use the same validation and Undo history.

Arrangement/scene rendering and live MIDI monitoring resolve the fade from the rounded 1–127 note velocity and multiply it into the zone gain before envelope/pan processing. Source selection excludes fully silent layers, avoiding unnecessary decoding for those notes.

Verification: `test/experimental-sampler-velocity-fade.test.js` covers complementary gains across all velocities, equal-power behavior, group gating, fallback boundaries, validation, legacy migration, Undo, instrument archive exchange and mocked agent patches. `scripts/browser-experimental-sampler-velocity-fade-check.cjs` checks controls, Undo/Redo, reload and mocked agent edits, plus native OfflineAudioContext PCM for arrangement playback, seek and the live MIDI monitor at seven velocities. It does not verify physical MIDI output timing or live model inference. Reference: [Apple sampler crossfades](https://support.apple.com/en-euro/guide/logicpro/lgsifc8649f5/mac).
...
The scheduler adds the source start to its pitch-bend-integrated seek offset. Non-looping voices stop at the selected source end using Web Audio's source-duration parameter, so pitch/tuning/bend changes also change the audible duration correctly. Looping voices remain inside the selected loop, and live MIDI monitoring uses the same source selection. Source preflight distinguishes different trims of the same asset. Main and articulation sources retain their existing whole-file behavior; this addition exposes trimming for zones.
...
## Audition a sampler zone or unsaved draft

The sample-zone editor now has **Audition draft** with MIDI pitch, velocity, note length (0.05–10 seconds), **Play zone**, and **Stop preview**. It reads the form without saving, so new or modified zones can be heard before committing. Zone/source selection changes stop the current preview. Starting arrangement playback, editing the session or leaving the workspace also uses the shared stop path. Preview does not move the arrangement playhead or create notes/history entries.

Audition uses the zone's real source, start/end selection, loop, root/tuning, velocity fade, envelope/filter, and zone/group level/pan. Pitch and velocity must fall inside the zone and active group conditions; muted/solo-excluded groups and silent velocity-fade endpoints reject. It deliberately bypasses track effects, track level/pan, buses and the master mixer; the UI states this. Note release can extend after the requested note duration. Source completion or a bounded fallback timer releases nodes and updates status. Only one zone preview plays at a time.

Source loading and AudioContext resume check the captured session/revision and preview epoch before starting; stopping or replacing a pending preview prevents stale playback. Manual preview updates its inline status without repainting the form or discarding draft values. Invalid source bounds use the existing sample validation.

The `audition_sampler_zone` agent tool accepts `{trackId, zoneId, pitch, velocity, duration}` for an observed **saved** zone. It is capability-gated by `allowZoneAudition`, is unavailable during editing continuation, and runs separately from edits/exports. The server and browser share `zoneAuditionPlan` validation; model requests do not see unsaved form values.

Verification: `test/experimental-sampler-zone-audition.test.js` covers draft preservation, zone/group sound selection, missing/excluded/silent note rejection, unsaved zones and mocked agent capability checks. `scripts/browser-experimental-sampler-zone-audition-check.cjs` checks Play/Stop/automatic finish, unchanged saved state and form drafts, real source rejection, mocked agent audition and native OfflineAudioContext PCM for trimmed, leveled and panned source playback. Physical listening/output timing and live model inference remain unverified.


## Zoom and navigate the sample-zone waveform

The zone source waveform now provides **Full source**, **Fit selection**, **Zoom in**, **Zoom out**, **Earlier**, and **Later**. Zoom is centered on the current visible interval; Earlier/Later move by half its width. Fit selection shows the current draft trim, with a minimum visible interval of 32 source frames (or the whole file for shorter buffers). Views stay sample-aligned and clamp to source boundaries. A source-time readout labels the visible interval.

The waveform is regenerated from the visible source samples, including every channel, rather than scaling the full-file thumbnail. Rendering is cached until the view changes. Only the visible portion of the selection band appears, and trim handles outside the view are hidden. Pointer-to-time conversion uses the captured visible interval, so dragging while zoomed edits the corresponding source times. Keyboard boundary changes outside the view bring that edge back into view.

These controls alter only the displayed interval; they do not change the trim draft, saved session or Undo history. Trim edits still require **Save sample zone**. Switching to a different decoded source resets to its full view. Zoom is transient UI state and is not saved in the project or instrument file.

Verification: `test/experimental-sampler-wave-viewport.test.js` checks bounded/frame-aligned zoom, minimum detail, panning, invalid requests and visible-range stereo waveform sampling. `scripts/browser-experimental-sampler-wave-zoom-check.cjs` checks fit, zoom, navigation, exact source-coordinate dragging, unchanged project state, return to full view, Save/Undo/Redo and the existing native trim audio checks. The zoomed controls were visually inspected.

## Draw sampler-zone loops in the waveform

Choose **Loop start/end** under **Waveform edits** to draw or resize a loop using the same waveform, zoom, navigation and keyboard controls as sample trimming. Drawing or adjusting a loop enables the zone's loop checkbox. Loop boundaries stay inside the selected sample portion and snap to source frames; numeric source boundaries align inward for graphical loop editing. An invalid or shorter-than-one-frame source selection must be corrected first.

Gestures update the form draft only. **Save sample zone** validates and commits through the existing undoable operation. Escape, pointer cancellation and lost capture restore both prior loop points and the prior enabled state. Invalid loop handles are hidden so they cannot intercept a replacement selection. Sample mode continues to edit only source trim boundaries. The mode persists across form repaints on the same track and resets when changing tracks; it is not stored in the session.

Verification: waveform unit tests cover constrained forward/backward drawing, extreme handles, inward alignment and invalid sample bounds. `scripts/browser-experimental-sampler-loop-wave-check.cjs` covers initial invalid-loop recovery, Escape restoring disabled looping, clamping, zoomed drawing, keyboard steps, unchanged source trim, Save/Undo/Redo and native rendered audio checks for loops, pitch/bend, seeking and live-monitor parity. The loop controls were visually inspected. Physical device timing and live model inference remain unverified.

## Sampler-zone zero-crossing adjustments

The zone waveform provides **Start to zero crossing** and **End to zero crossing** for the active Sample or Loop mode. These reuse the audio editor's shared-channel search: a crossing must occur in every channel (mono/stereo) within 5 ms of the boundary. Searches respect the opposite edge, retain at least one source frame, and keep loops inside the sample selection. No shared crossing leaves the draft unchanged and displays an inline explanation. This can reduce abrupt boundaries but does not guarantee click-free loops; fades or matching loop phases may still be needed.
...
The operation rejects insufficient MIDI keys, capacity beyond 128 zones and invalid source ranges before committing. Undo restores the original zone. The existing shared `samplerZone.delete` and `samplerZone.addMany` operations provide the persistence/agent command path; the dedicated agent attack-slicing tool is described below.
...
## Agent attack slicing for sampler zones

The `slice_sampler_zone` tool accepts observed `trackId`/`zoneId`, a starting MIDI key and explicit transient settings (`riseDb`, `floorDb`, `minimumGap`). The browser advertises `allowSamplerSlicing`; older callers do not expose the tool. Server validation rejects missing zones, protected tracks and invalid settings. The tool is separate from edit batches/transport/export and unavailable during editing continuations.

The browser decodes the saved zone's source, runs the existing transient worker and feeds actual detected attack times into the same `samplerSlicePlan` used by the UI. The saved source selection is partitioned; unsaved form drafts are not read. The original zone is replaced in one undo step only after detection and slice planning succeed. Source decoding and detection are followed by abort/session identity/revision checks. The worker receives the request's abort signal. Existing capacity, source-bound and empty-cut checks still apply.

Verification: `test/experimental-sampler-slice-agent.test.js` checks capability/continuation gates, observed IDs, settings, protection and nonmutation during validation. `scripts/browser-experimental-sampler-slice-agent-check.cjs` mocks the model response but uses real browser decoding/detection, checks persisted keys/bounds, renders the resulting zones in native OfflineAudioContext and verifies Undo. Full suite: 1,283 passing; build passes. Live model tool selection and physical MIDI latency remain unverified.

## Tune attack detection and preview slice mappings

The sampler attack panel exposes Attack rise (3–24 dB), Noise floor (−96 to −12 dBFS) and Minimum gap (0.01–1 second). Defaults remain 9 dB, −45 dBFS and 0.05 second. These controls use the same validated settings as the worker and agent slicing tool. Editing detection settings aborts pending work and clears old markers/selections, so stale detections cannot be used for slicing. Rerun Detect attacks to generate new results.

A preview shows the prospective slice count and MIDI-key range, or the planner's validation error. It uses the same draft/markers/planner as creation, updates with source-boundary and starting-key changes, and disables Create when the selected zone, bounds, capacity or keys are invalid. Waveform gestures and cancellation also refresh the preview. Previewing creates no session edit or source asset.

Verification: `scripts/browser-experimental-sampler-slice-preview-check.cjs` checks real worker sensitivity changes, marker invalidation, redetection, key overflow disabling, live count changes from trimming, unchanged saved state before Create, rendered slice audio and Undo. The controls were visually inspected and spacing corrected. Focused attack/slice tests and production build pass.

## Create a MIDI pattern with sampler slices

**Create MIDI pattern** optionally adds a region at the entered timeline start when creating playable slices. Each slice gets a note at its source-relative start, lasting its source span; the region duration matches the selected sample portion. Pitches correspond to the new slice keys. Note velocity uses the original zone's velocity-range midpoint, and a referenced articulation is copied when available; missing articulation definitions reject pattern creation. Envelopes, tuning, velocity response, effects, group ranges and overlapping zones still affect sound, so this is an editable timing reconstruction rather than a guaranteed identical render.

The `slice_sampler_zone` agent option `regionStart` accepts timeline seconds to request this pattern or null to omit it. Older requests default to null. Slices and the region/notes share one Undo step. Pattern placement must fit within the 24-hour timeline. The new `notes.addMany` shared command accepts a JSON string array of validated note objects, allowing up to 128 slices without exceeding the edit batch's command limit. Existing session validation rejects duplicate IDs, notes outside their region and oversized collections atomically.

Verification: slice tests cover region placement, relative note times, durations, shared Undo and atomic invalid bulk-note rejection. UI and mocked-agent browser checks verify persisted patterns alongside native slice audio rendering. Full suite: 1,285 passing; build passes. The option's rendered layout was inspected and adjusted to align its checkbox and label.

## Edit sampler attack markers manually

The attack panel now accepts a **Marker position · source seconds** for **Add marker** and **Move selected**, plus **Remove selected**. This supports correcting detections or creating cuts without running detection. The selected marker populates its source time; Previous/Next navigates the editable list. Manual edits are sample-aligned, sorted and bounded strictly inside the source; duplicate sample boundaries are rejected. At most 10,000 temporary markers are allowed, while creating zones retains the existing 128-zone/key limits.
...
## Agent slicing from saved markers

`slice_sampler_zone` now accepts `markerSource: "saved" | "detected"`. Saved mode uses the selected zone's persisted `sliceMarkers` and bypasses detection, preserving manual corrections. Missing saved markers reject rather than silently generating a different set. The browser still decodes the source and validates distinct in-bounds sample frames before planning slices. Detected mode runs the worker with the provided settings; older requests default to detected mode. Source selection, key/capacity checks, optional MIDI pattern and single-step Undo remain shared.

Verification: slice-agent unit tests prove that saved mode bypasses the detector, rejects absent/out-of-source markers and retains detected-mode behavior. `scripts/browser-experimental-sampler-saved-slice-agent-check.cjs` uses a mocked agent request with custom saved points different from the recording's attacks, confirms the persisted slice boundaries and pattern, renders each slice and checks Undo. Full suite: 1,291 passing; build passes. Live model tool selection remains unverified.
...
## In-app score preview

Selected MIDI regions now have a collapsible **Score preview** before the piano roll. Opening it lazy-loads OpenSheetMusicDisplay 2.1.3 and renders the same MusicXML produced by the export path as SVG. Zoom offers 75–150%. The score uses a white page in both themes, inside a bounded scrolling viewport. This is a notation viewer: edit notes in the piano roll or shared commands; score rendering does not alter notes or session history. MusicXML's current pitch/percussion/engraving limitations still apply.
...
### Multi-part score export (2026-09-23)

- **Export score MusicXML** downloads all eligible pitched MIDI tracks, each as a separate named part in track order. Parts share a timeline-zero start/end, preserve gaps as rests, and retain overlapping regions as voices. Each part uses its track clef and the project tempo, meter and key maps.
- This is written-note notation, not an audio render: track/region mute and solo do not hide parts; muted notes are omitted as in region export. Audio, video, empty tracks, drum instruments and tracks containing percussion-channel notes are excluded. The toolbar explains this in its tooltip. Percussion notation, controllers/effects as notation, full-score preview and direct score editing remain incomplete.
- Agent tool `export_score_musicxml` accepts observed `trackIds` in requested part order. It requires export capability, is unavailable during edit continuation, validates the complete export before returning, and never changes the session. Browser execution checks the session revision and cancellation. Live model tool selection has not been verified; provider-response and browser-action tests are mocked.
- Checks: `test/experimental-score-export*.test.js`, browser scripts `browser-experimental-score-export-check.cjs` and `browser-experimental-score-export-agent-check.cjs`. Two-part browser download passed MusicXML 4.0 XSD validation; round-trip in an external notation editor remains unverified.

### Full arrangement score preview (2026-09-23)

- Score preview now offers **Selected MIDI region** and **Full arrangement**. When no MIDI region is selected, eligible tracks still make the full arrangement preview accessible. Parts have instrument names, individual clefs, shared bar positions, and rests for gaps.
- Rendering uses the same multi-part MusicXML generator as download. Pitched MIDI only; track/region mutes do not remove written parts. The existing mid-bar key-change renderer limitation is checked over the full arrangement, including silent gaps.
- Async renderer loads use isolated surfaces and request generations, preventing stale loads from replacing a newer selected view. Zoom works in both scopes. This remains a preview, with piano-roll editing; direct graphical notation editing is still pending.
- Regression: `test/experimental-arrangement-score.test.js`, `scripts/browser-experimental-arrangement-score-check.cjs` (two named staves, bass/treble clefs, zoom, scope switching and unchanged session), plus existing clef browser check. Visually inspected `/tmp/cuestamp-arrangement-score.png`.

### Editing notes from the score (2026-09-23)

- Click a rendered notehead (or Tab to it and press Enter/Space) to edit MIDI pitch, region-relative start, duration and velocity. **Apply note edit** uses the existing validated `note.set` command, persistence and Undo. It edits the underlying note, not an engraving copy; tied segments resolve to the same note.
- Works in selected-region and full-arrangement score views. Mapping uses part, voice, pitch and 1/960-quarter-note position; renderer pitches use an octave offset of 12 relative to MIDI. The shared arrangement-part builder retains original region references. Rests and ambiguous merged unison heads are not editable; use the piano roll for those.
- Graphical dragging, note insertion, engraving layout edits, slurs/articulations and advanced notation remain pending. Existing agent `note.set` shares the same operation; no separate score-only document is introduced.
- Verified `test/experimental-score-note-editor.test.js` (voice/tie/part/local-time mapping) and `scripts/browser-experimental-score-note-edit-check.cjs` (multi-part mouse/keyboard selection, tied-note edit and Undo). Renderer integration follows its notehead accessors; reference: https://github.com/opensheetmusicdisplay/opensheetmusicdisplay/wiki/Exploring-the-Demo .

### Dragging score pitches (2026-09-23)

- Drag an editable notehead vertically by staff steps, or focus it and use Up/Down; Shift+Up/Down moves an octave. Movement follows the project key at the original note onset and retains the note's chromatic deviation from that key. MIDI overflow is rejected. Timing, velocity and other notes stay unchanged.
- Pointer movement previews the head position; release commits one existing `note.set` command. Escape, pointer cancellation and lost capture restore the visual without editing. Tied segments edit their single underlying note. Click/Enter/Space still opens the numeric note editor.
- The gesture accounts for score zoom (one staff step is five renderer units). Stems/ties and accidentals re-engrave after release. Horizontal timing drag, score insertion and advanced engraving remain pending.
- Checks: `test/experimental-score-pitch-drag.test.js` covers major-key sharp/flat spelling, chromatic deviations, octaves, extreme key spellings and MIDI limits. `scripts/browser-experimental-score-pitch-drag-check.cjs` verifies zoomed dragging, unchanged data before release, Undo, Escape cancellation and keyboard octave editing. Existing score note form browser test also passes.

### Written note lengths (2026-09-23)

- The score note editor offers whole through thirty-second notes, straight/dotted/triplet rhythms, and custom seconds. Standard lengths are recognized when selecting an existing note. Changing start recalculates a chosen written length using the project tempo map, including changes inside the note. Editing seconds explicitly switches to custom timing.
...
### Agent context from score selection (2026-09-23)
...
### Transposing-instrument score notation (2026-09-23)

- Tracks accept optional `scoreInstrument`: `concert` (default), `bb` (written +2 semitones), `eb` (E-flat alto, written +9), and `f` (written +7). The selected-track notation dropdown sets it through `track.set`; the agent is instructed to use the same field. Other transposing instruments/ranges and a global concert-score toggle remain pending.
- Preview and MusicXML shift written pitches and key signatures, with the inverse diatonic/chromatic `<transpose>` metadata for sounding playback in compatible notation software. Extreme key signatures use an enharmonic signature within seven sharps/flats. Unknown project keys stay unknown. Written pitches above MusicXML octave 9 reject export instead of producing invalid XML.
...
### Agent concert/written MusicXML export (2026-09-23)

- Both `export_musicxml` and `export_score_musicxml` accept `pitchMode: written | concert`. Tool schemas require it for new model requests; server/browser compatibility defaults missing values to written. The agent is instructed to use written unless concert pitch is requested.
- Server validation generates notation from the selected pitch view; the browser independently applies that view after checking revision/cancellation. Concert filenames include `-concert`. Track settings and MIDI data are unchanged. Multipart export still accepts observed track IDs in requested order.
- Verified `test/experimental-agent-score-pitch-mode.test.js` (both tool schemas, concert versus written range validation, invalid modes, unchanged session), `scripts/browser-experimental-agent-score-pitch-mode-check.cjs` (mocked agent concert download), and MusicXML XSD validation. Live model selection remains unverified without local credentials. The transient score view/part filter is not yet automatically supplied to the agent; requests should specify concert/written mode and desired parts explicitly.

### Visible score context for the agent (2026-09-23)

- Agent requests now capture `scoreView` from an open, successfully rendered score before repaint: session/revision, region-versus-arrangement scope, ordered track IDs, optional region ID and concert/written pitch mode. This allows the model to interpret “export this/displayed score” using visible choices; explicit user choices take precedence.
- Server validation rejects stale session revisions, missing/duplicate parts and invalid region ownership before provider requests. The model receives this as context only, not authorization. Context is omitted after edit continuations because its original revision no longer describes the document. Closed, incomplete or failed score views supply no context.
- Verified `test/experimental-score-view-context.test.js` (validation and provider request payload) and `scripts/browser-experimental-score-view-context-check.cjs` (bass-only concert view captured before repaint, matching mocked-agent export). This supersedes the previous limitation that users must always restate the visible score settings. Actual model inference remains unverified until credentials are configured.

### Score chords checkpoint
...
### Score selection continuity (2026-09-23)

Score selections now persist in transient workspace memory across command repaint, Undo/Redo and score zoom/notation changes. This supersedes the earlier repaint-clears-selection limitation. Restore only selects notes that are actually rendered in the current score and still belong to their source region. Missing/deleted IDs remain in the transient memory for Undo recovery but never enter the visible selection or agent context. Cancel, a new selection, a draft note or moving to another editor clears/replaces that memory. Switching sessions clears it; no selection is saved in project data or local storage. Undo/Redo controls and shortcuts preserve it. A closed score no longer supplies note-selection context to the agent.

Verified 1,341 tests and build, plus targeted closed-score validation. `scripts/browser-experimental-score-selection-memory-check.cjs` verifies repeated phrase edits without reselecting, deletion/Undo recovery, zoom and Cancel. The mocked agent multi-selection check still passes. Live model inference remains unverified.

### MusicXML note import (2026-09-23)

**Import files** now accepts uncompressed partwise `.musicxml` / `.xml` scores, up to 8 MB, 128 parts, 2,048 measures per part and 20,000 note elements. The importer converts written pitches to sounding MIDI notes, combines sound ties, handles chords, overlapping voices/staves, rests, pickup measures, divisions changes, explicit tempos/metronomes and conventional key/time signatures. Overlapping unisons use separate MIDI channels (excluding percussion channel 10) to avoid note-off ambiguity; more than 15 simultaneous instances at one pitch reject. Dynamics import/export now correctly use a percentage of MIDI forte velocity 90, correcting the earlier export scaling.

Import reuses the shared atomic `midi.import` command, inserts at the playhead and supports Undo. Existing timing/signature selectors are labeled MIDI / MusicXML and govern preserve-performance, follow-session or adopt-file behavior. Original layout, clefs, instrument patches, lyrics, articulation/expression markings and note-off velocities are not imported; successful import explicitly says it contains editable MIDI notes. No source audio is created. Repeats/endings/playback jumps, grace notes, percussion, ornaments/tremolo, octave shifts, note attack/release overrides and unsupported transposition/key/time forms reject with an explanation. Cue notes are not played. Compressed `.mxl`, timewise MusicXML and full notation-document import remain pending. Missing tempos use 120 BPM. XML entity/internal-subset declarations reject; no external resources are fetched.
...
### Agent score clipboard paste (2026-09-23)

The agent now receives a count and identity token for copied score notes and can request `paste_score_notes` with a destination MIDI region, zero-based beat offset, beats/seconds timing and explicit extension choice. The copied note payload remains local. The client resolves the request using the same snapshot and shared commands as the manual paste controls. This supersedes the previous lack of clipboard context for agent paste; agent copy/cut into this clipboard remains future work.
...
### Display-only score quantization (2026-09-23)

The score toolbar's **Display grid** can show performed timing or straight/triplet notation grids. It rounds note starts and ends on a temporary, tempo-aware score copy, keeping at least one grid interval where it fits and clipping at the region edge. Original note IDs, pitch and other properties remain linked. Session revision, stored notes and playback are unchanged. The setting persists across redraws in the current workspace/project but is not a saved performance edit.

Selecting or editing a displayed note resolves its original source note: fields, Copy/Cut, pitch changes and shared timing tools still operate on actual performance data. With a display grid active, tied visual segments highlight for their original note's performed interval. **Download displayed score** includes the rounded notation; other MusicXML export paths retain performed timing. The agent's validated score-view context includes the active grid and its instructions distinguish display from playback; agent export tools do not yet reproduce this grid automatically.

Verified 1,381 tests and production build. `test/experimental-score-display-timing.test.js` covers nonmutation, tempo-aware rounding, identity, straight/triplet grids and boundary notes. `scripts/browser-experimental-score-display-timing-check.cjs` checks actual rendered selection, original note fields/highlight times, velocity edit/Undo, view context, a downloaded MusicXML parsed back to rounded timing, and unchanged source tracks. Existing score keyboard clipboard regression also passes.

### Agent MusicXML display-grid exports (2026-09-23)

Both agent notation tools now accept `displayGrid`: off (performed timing) or the same straight/triplet grids as the score toolbar. For “export as shown,” the agent instructions use the captured view's grid, pitch mode and region/parts. This supersedes the previous agent-export display-grid limitation. Server validation and browser generation share `score-notation-export.js`, applying display timing and pitch mode to temporary copies. Saved performance data does not change. Grid-enabled downloads include the grid name in the filename and export report.
...
### Opt-in score note audition (2026-09-23)

Enable **Audition notes** in the score toolbar to hear existing notes when selected by click or Enter. It is off by default and retained across redraws in the current project view. The audition uses original note data, current instrument/sample articulation, MIDI controllers, track/bus/master routing, gain and effects. It isolates that note, caps its key-down length at half a second, bypasses region edge fades, and caps the full audition including tails at four seconds. Muted/silent notes and tracks excluded by the current mix remain silent. It does not move the playhead, start transport or edit the document.

Selecting a new note cancels an older audition or pending sample/context preparation. Loading validates the exact session object/revision before scheduling. Audition is suppressed during transport, recording, loading or agent planning; transport start/stop, score close, toggle-off, relevant recording preparation and disposal stop its audio graph. Sample assets use the existing decoder/cache and sampler routing. Browser audio output must be available.

Verified 1,390 tests and build, plus targeted controller tests after final cleanup refinements. `test/experimental-score-note-audition.test.js` covers isolation/nonmutation, mute, preparation supersession, graph cleanup, revision invalidation and blocked transport. `scripts/browser-experimental-score-note-audition-check.cjs` verifies the real selection/toggle workflow, unchanged session/playhead and actual OfflineAudioContext PCM, including track-gain attenuation and silence after audition. Physical speaker output and every sample/effect combination have not been independently tested.

### Selected score phrase playback

The score note editor now has **Play selection** for one or more existing notes within a MIDI region. Shift-click notes to build the selection. Playback uses the original performance timing (including gaps and pedal holds), the current instrument, region fades, automation, routing and mixer effects. Other notes and regions are excluded. Cycle and metronome are disabled for this preview; the transport stops after the selected phrase and its release/effect tails. Use the transport Stop control to interrupt playback. The saved session is not edited.

The agent can request the same behavior using `audition_selected_notes`, with an explicit region and note IDs or the captured note selection. Empty, duplicate, missing and wholly muted selections are rejected. This action requires selection-audition capability and current transport context; the client checks the session revision and transport epoch again before starting. It is a listening action, not music generation or export.

Verification: 1,393 tests passed and the production build passed. `scripts/browser-experimental-score-note-audition-check.cjs` checks score selection playback/automatic stop, unchanged saved session, and offline rendered audio proving that unselected notes stay silent. Agent tests use a mocked provider; live model inference and physical audio/MIDI devices were not exercised in this checkpoint.

### Monophonic audio-to-MIDI extraction (2026-09-23)

Select an audio region and open **Audio to MIDI · melody** in its inspector. **Analyze melody** measures the source recording in a cancelable worker, respecting source offset and reverse. Review detected note names, relative start/length, tuning offset and periodicity confidence, then choose **Create MIDI track**. The new sine-instrument MIDI region aligns with the original region and can be edited through the existing piano roll, score and agent commands. One Undo removes the entire extraction; the source track, file and audio edits remain untouched. Both tracks remain audible unless you mute one.

This is approximate monophonic transcription, not generative music or polyphonic/Flex Pitch parity. Use a solo voice/instrument playing one note at a time. Detection uses a windowed-sinc decimator and YIN-style normalized differences with refined lag estimates; stereo channels are evaluated independently, avoiding cancellation from opposite polarity. Analysis uses 20 ms steps, so note boundaries are approximate. Minimum-duration filtering rejects short detections. Repeated notes without a gap, vibrato, slides, percussion, chords and noisy recordings can require manual correction. The confidence percentage measures periodicity, not a calibrated probability of the correct note. MIDI rounds to semitones; measured cents are shown but are not encoded as pitch bends. Mixer effects and region gain are not analyzed.

Bounds: up to two minutes of mono/stereo source PCM within 250 MB; split longer recordings first. Configurable MIDI pitch range (24–96), noise floor and shortest note. Cancellation terminates the worker. Source samples stay in the existing cache; transferred worker data is copied. Fractional-sample region lengths clamp detected note ends to the actual region boundary. Analysis results are temporary and invalidated by document changes.

The `analyze_audio_melody` agent action supports `preview` or `createMidi`, explicit/captured audio regions and validated settings. The model requests analysis; pitches come from the worker, never model-authored guesses. Capability/continuation guards and client document-revision checks apply. Creation uses shared track/region/note commands and records one history transaction.

Research: [Apple Flex overview](https://support.apple.com/en-by/guide/logicpro/lgcp15968647/mac) describes audio-to-MIDI extraction within its pitch workflow. Detector reference: [de Cheveigné & Kawahara, YIN (2002)](https://pubmed.ncbi.nlm.nih.gov/12002874/). This implementation is original and is not Apple's algorithm.

Verification: 1,398 tests and production build passed; browser checks cover worker execution, stereo polarity, source trim/reverse, cancellation, fractional-sample boundaries, review/create UI, source preservation, Undo and mocked-agent extraction. Unit fixtures cover several sample rates, harmonic-rich/detuned notes, silence/DC/noise rejection, stale analysis and agent gating. Live model inference and accuracy on a representative corpus of real vocal/instrument recordings remain unverified.

### Review and correct melody drafts (2026-09-23)

After **Analyze melody**, each detected row has **Edit note**. Correct MIDI pitch, region-relative start, length and velocity, or exclude a false detection. **Apply correction** updates only this temporary draft; **Reset corrections** restores all measured values. Measured tuning/confidence remain labeled separately and are never replaced with invented measurements. **Create MIDI track** uses the corrected, included notes; it is disabled when all notes are excluded. Corrections do not retune audio. The resulting MIDI can use the existing score/piano-roll editors.

The table pages through 128 notes at a time, so longer detections are fully accessible. Analysis remains temporary: document edits invalidate it, replacing the workspace history clears it, and reanalysis replaces corrections. Draft changes use reset rather than project Undo; creating MIDI is still one project Undo transaction.

The agent receives only the current region's visible draft page and a version token. `edit_melody_draft` can correct observed note indices, reset all corrections, or create MIDI from the current draft without rerunning detection. Unchanged fields use null. Server and client validate source-region bounds and action shape; the client rejects stale draft tokens. Draft changes rotate that token even though the saved session revision stays unchanged. The tool is unavailable without a captured draft or during editing continuations.

Verification: 1,401 tests and production build passed. `test/experimental-melody-draft.test.js` verifies nonmutation, correction/exclusion/reset, generated notes, stale tokens/revisions, bounds, paging and agent capability guards. `scripts/browser-experimental-melody-draft-check.cjs` exercises actual manual edits/reset, MIDI creation, Undo and mocked-agent correction followed by creation, including a rerun after project-switch cleanup. Live model inference and per-note audio retuning remain unverified/unimplemented respectively.

### Render note-level audio pitch corrections (2026-09-23)

The melody review now offers **Render tuned audio**. It computes a constant pitch shift for each included detected note from its measured pitch/cents to its draft MIDI pitch. Unedited included notes are tuned to their nearest semitone; exclude a note to leave its audio unchanged. Corrections up to ±12 semitones are supported. Reset start/length corrections before rendering: this action changes pitch only, and MIDI velocity edits do not alter the recording's gain.

Rendering runs in the existing cancelable audio worker with SoundTouch processing. Each span includes up to 100 ms of surrounding source context, but writes only inside its detected boundaries; up to 5 ms linear dry/wet transitions reduce edge discontinuities. Samples outside correction spans are unchanged. Duration and mono/stereo channel count are preserved. This applies a constant correction per detected note, retaining within-note pitch variation; it does not implement continuous pitch-curve correction, formant preservation or vibrato/drift controls. Short notes, rough detection boundaries and large shifts may sound less natural.

A successful render saves a new 32-bit float WAV asset and a derived audio track with the source track's editable mixer processing. The original region is muted, its original file is retained, and one Undo restores the prior arrangement. Protected/muted source-region and track-capacity checks match existing stretch/pitch rendering. Source trim/reverse are baked into the derived recording; original fades remain editable. Abort/stale-session failures remove uncommitted assets.

The agent's `edit_melody_draft` action accepts `retuneAudio` with empty edits, requiring current draft token and audio-render capability. It uses actual local measurements and the same renderer/commit path. `region.commitRetune` is the validated internal command; model-authored edit batches cannot fabricate a retuned asset. This supersedes the earlier lack of per-note audio retuning, but full Flex Pitch-style graphical editing and formant/vibrato features remain unfinished.

Verified 1,405 tests and production build. `test/experimental-audio-retune.test.js` verifies shifted frequencies, untouched samples, stereo polarity, duration, invalid spans, source preservation, derived-track Undo and agent capability/asset guards. `scripts/browser-experimental-retune-check.cjs` checks manual and mocked-agent rendering, persisted WAV frequency/duration, excluded notes, original asset retention, Undo and worker cancellation. These are synthetic signal tests; live model use and naturalness across a representative music/vocal corpus remain unverified.

### Graphical melody pitch editor (2026-09-23)

The **Audio to MIDI · melody** editor now appears in the main editing area beneath the arrangement, with a pitch-lane graph for the current 128-note review page. Solid note blocks show draft pitches/timing; dashed outlines retain original detected pitch, cents and timing. Excluded notes use a muted style. The graph follows the current dark/light colors and scrolls when space is constrained.

Drag vertically to change pitch in semitone increments. Arrow Up/Down makes a one-semitone correction; Shift changes an octave. Escape or pointer cancellation discards an unfinished drag. Click or Enter opens the existing numeric correction form. Horizontal dragging does not move notes. Edits remain temporary draft changes until Create MIDI track or Render tuned audio, and use the same validated correction path as the agent. Graph scroll state survives ordinary editor repaint. The table remains available for precise timing/velocity and exclusion edits.

The wider panel also fixes the narrow inspector's stretched analysis button and its CSS overriding the correction form's hidden state. Cancel now actually hides that form. Original pitch outlines keep their measured time positions even when MIDI draft timing is corrected.

Verified 1,408 tests and production build. `test/experimental-melody-graph.test.js` covers pitch/time geometry, original outlines, page indices, MIDI bounds and disabled accessibility markup. `scripts/browser-experimental-melody-graph-check.cjs` verifies keyboard semitone/octave edits, drag, Escape cancellation, form selection/cancel, unchanged saved session and corrected MIDI creation. Actual retune browser regression also passed after moving the panel. Dark/light screenshots were visually inspected. This graph shows per-note summaries, not continuous detected pitch curves or formant/vibrato controls.

### Listen before committing melody pitch corrections (2026-09-23)

The melody panel now has **Play original**, **Preview tuned audio**, and **Stop preview** while either audition is active. Original playback isolates the source region; tuned playback renders the current included pitch corrections into a temporary AudioBuffer. Both run through the source track's mixer processing and buses, include effect tails, disable cycle/metronome and use the normal transport playhead. They ignore unrelated track solo selection, as other isolated audio-range auditions do; muted source tracks/regions cannot be auditioned.

Preview never writes an audio file, adds a track, mutes the original or changes saved session data. Its temporary buffer reference is removed from the cache after scheduling; transport owns scheduled audio until stopped. Rendering can be canceled, and edits/project switches invalidate pending work. The normal render path remains a separate explicit action with its existing new-file/new-track commit and Undo behavior.

Read-only preview is allowed for protected source tracks and at the 128-track limit; permanent rendering still requires an unprotected source and room for a new track. Pitch preview retains the same analysis/timing/shift bounds as retuning. It rerenders on each request rather than caching a long-lived audio result.

Agent `edit_melody_draft` operations `previewOriginal` and `previewAudio` require current transport context in addition to the draft token. The former requires range-audition capability; the latter audio-render capability. Client transport-epoch checks reject requests if playback changed during planning, and summaries identify these as previews rather than saved edits.

Verified 1,411 tests and production build. `test/experimental-melody-audition.test.js` covers original/tuned isolation, mixer tails, source nonmutation, protected/full-session preview and transport/capability guards. `scripts/browser-experimental-melody-audition-check.cjs` verifies both manual previews, explicit/automatic stop, mocked-agent preview, unchanged session and unchanged IndexedDB asset IDs. Live model inference and physical speaker-output verification remain outstanding.

### Save melody analysis and corrections with a region (2026-09-23)

Choose **Save melody edits** to store measured notes, analysis settings and correction patches in the audio region's project document. Saving is one Undo transaction. The panel distinguishes saved and unsaved drafts; changing corrections makes the live draft unsaved again. Saved analysis restores automatically when selecting that region after reload or unrelated project edits. **Remove saved melody** deletes the stored draft and supports Undo. Unsaved live corrections are still temporary and can be discarded by reanalysis, switching projects or document changes; save them before doing those operations.

Stored analysis carries the exact source asset ID, offset, duration and reverse flag. Moving/renaming the region or changing mixer settings keeps it valid. Changing the source span/direction or rendering a new source invalidates the copied analysis; Undo restores the prior saved draft. Protected tracks reject save/removal as content edits. Measured notes and patches are schema-validated and bounded to 4,000 notes and the two-minute source span; raw save commands have a 2 MB JSON limit.

Portable archive import and cloud media remapping update the saved source reference alongside the region's media ID. The metadata therefore remains linked after importing fresh local asset IDs. Saved melody metadata is excluded from scene audio-content signatures, so saving annotations does not invalidate previously recorded scene performances. Existing projects without this optional field remain readable.
...
### Stereo-linked noise gate (2026-09-23)

The mixer now offers **Noise gate** on tracks, buses and master. `effect.add` kind `gate` and normal effect editing/presets/automation support threshold (-96..0 dB), reductionDb (-96..0 dB), hysteresis (0..24 dB below the opening threshold), attack (0..1 s), hold (0..2 s), and release (.001..3 s). All six controls support saved curves and live automation; commands remain atomic and undoable. The agent receives the same parameter descriptions.

The AudioWorklet uses the peak of both input channels to control one shared gain. A 10 ms detector decay reduces zero-crossing chatter. Hysteresis sets a lower closing threshold; hold delays closure after the detector drops below it. Attack/release are one-pole time constants (approximately 63% of the transition after one time constant), not hard completion times. Quiet signals receive the specified attenuation; noise during louder signals still passes. No lookahead, external sidechain, sidechain filtering, or gate-state meter is implemented. Seek/restart clears detector state; the effect adds no signal tail or lookahead latency.

`prepareSessionEffects(context, session)` loads the processor before building playback/export graphs. The app calls it for transport, scenes, score audition, cycle rendering, audio/MIDI recording accompaniment, mix analysis and export. Direct users of synchronous `scheduleSession` must await preparation first when enabled gates exist. Worklet load failure surfaces as an error; there is no silent bypass. Context-owned load state prevents duplicate registration during development reloads. Stop releases the worklet. Existing sessions without gates require no new processor.
...
### External gate sidechains (2026-09-23)

A gate's **Gate sidechain** selector can use **This signal** or an existing audio/instrument track. The shared `effect.set` / `effect.add` field is `sidechainTrackId` (track ID or null); the agent uses the same validated command. Buses and video tracks are rejected. The detector takes the source's raw instrument/audio input before inserts, fader, pan, track mute and solo. Region mute still silences that region. This avoids detector feedback through effect or bus routing. The existing detector filters, envelope controls and meters apply to either source.

A muted or solo-excluded detector track is scheduled for detection only: its channel and all sends remain silent, and its fader/send live automation is not exposed as an audible lane. Required audio and sampler files are decoded before transport/export. Gate bypass avoids decoding otherwise unused detector media. The source track can also play normally when audible; it is never mixed directly into the gated channel by the sidechain connection.

Isolated region/track bounces, stems and note/region auditions retain required detector recordings as muted dependencies. These paths preserve source time positions and prevent trigger leakage. Scene playback uses the source's active scene clips; this checkpoint has not separately verified live sidechain cell switching. Deleting a source track resets referencing gates to This signal in the same undoable transaction. Effect chain presets intentionally reset gate track assignments so portable presets cannot retain foreign track IDs; choose the source after applying a preset.

Verification: repository suite/build passed, with additional regression coverage for bypassed dependencies. `test/experimental-gate-sidechain.test.js` checks muted-source assets, isolated documents, bounce-in-place dependencies, atomic validation, deletion/Undo, preset reset and mocked-agent assignment. Browser worklet PCM checks show a muted trigger opening a quiet target without adding its signal, unchanged behavior when soloing the target, matching stem/full output and silence when the trigger clip is muted. The browser also selects a sidechain in the mixer and starts real transport. Hardware, live model inference and large-session performance remain unverified.

### Sidechain ducking (2026-09-23)

**Noise gate / Ducker** now has a saved **Mode** choice. Gate reduces the signal while the detector is inactive; Ducker reduces it while the detector is active. For music under speech, insert the effect on the music track, choose Ducker, select the voice track as the sidechain, set a reduction such as -12 dB, and adjust threshold, attack, hold and release. The existing gate reduction value is preserved when changing mode; set the desired amount explicitly.

Ducker attack controls the onset of attenuation and release restores the original level. Hold/hysteresis prevent brief trigger gaps from immediately restoring volume. Idle audio starts at unity gain. Detector filtering, stereo linking, automation, pre-insert external sources, isolated rendering, bypass, presets and Undo share the gate implementation. Mode is static and cannot be automated. The meter labels detector activity **Ducking** or **Recovering / idle** and shows actual gain reduction. `meterObservation.gates.open` continues to mean detector activity; in Ducker mode true means attenuation is engaged.
