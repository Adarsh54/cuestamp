...
Tests: `test/experimental-region-audition.test.js` checks isolation, routing, muting/solo, missing IDs and preservation. `scripts/browser-experimental-region-audition-check.cjs` checks controls, stop feedback, unchanged session and native OfflineAudioContext output through a bus while excluding an unavailable unselected source. Agent invocation of this new selection-audition action remains to be wired; the existing waveform-range agent audition is separate.
